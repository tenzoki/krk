...
---
Abgleich 260819-1440 (reconciler): **geschlossen bestätigt, und die Schließung als widerlegte Prämisse ist die richtige Form.** Der Datensatz behauptete drei Ursachenkandidaten am Baum, und alle drei sind am Baumstand `77dcd48` ausgeschlossen: `crates/krk-ui/src/appkit/leiste.rs` baut eine `NSTableView` (`:3`), die den Ersthelferrang von sich aus nimmt; `refusesFirstResponder` steht im ganzen Baum nur an den Schaltern der Bereichsleiste (`crates/krk-ui/src/appkit/bereichsleiste.rs:93`) und an keiner Fläche eines Bereichs. Ein `Resolved:`-Vermerk ohne Codeänderung ist hier kein Fehlgebrauch: geschlossen wurde nicht ein behobener Defekt, sondern ein Datensatz ohne Gegenstand, und die `Resolved:`-Notiz sagt das ausdrücklich.
