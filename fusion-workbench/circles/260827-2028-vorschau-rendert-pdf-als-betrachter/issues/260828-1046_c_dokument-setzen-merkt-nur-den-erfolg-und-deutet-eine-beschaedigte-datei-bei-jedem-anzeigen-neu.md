...
---
Resolved: Der vorgeschlagene Fix, mit einer Zutat. `PdfbetrachterIvars::bytes` heisst jetzt `gedeutet` und traegt `RefCell<Option<(Arc<Vec<u8>>, Deutung)>>`; der `Arc::ptr_eq`-Vergleich liefert die gemerkte `Deutung` statt unbedingt `Gesetzt`, und alle drei Zweige schreiben. Die Zutat ist die eine Schreibstelle: `gedeutet_merken(daten, deutung) -> Deutung` steht als Zeile am Ende jedes Zweigs, damit kein kuenftiger Zweig sie vergessen kann — genau das war der Defekt. `pdf_zeigen` verzweigt unveraendert. Der Doc-Kommentar nennt beide Merkposten und schreibt aus, dass Ansicht und Merkposten dabei absichtlich auseinandergehen: der Merkposten sagt, was zuletzt **gedeutet** wurde, die Ansicht zeigt, was zuletzt **gesetzt** wurde. Eine Probe ohne Fenster ist nicht moeglich, wie der Datensatz sagt; `zoom_und_deutung_tragen_je_genau_drei_werte` bleibt gruen.
