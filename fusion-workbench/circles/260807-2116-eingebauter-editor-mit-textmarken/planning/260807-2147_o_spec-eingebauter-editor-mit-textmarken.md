...
Seit dem 260810-0714 dieselbe Regel wie im Plan: ein Verweis auf einen Datensatz trägt an der Stelle des Zustandsmarkers eine Sternstelle. `decisions/260808-0021_*_was-sagt-der-editor-beim-sichern-ueber-den-unveraenderten-teil-der-datei-zu.md` bleibt richtig, wenn der Datensatz von offen über beantwortet nach umgesetzt wandert. Vorher standen hier feste Marker, und acht davon waren beim Abgleich veraltet; wer einen Pfad daraus in eine Suche gab, fand nichts. Wo der Marker eine Aussage über den Stand ist und nicht Teil eines Pfades, steht er weiter ausgeschrieben.
...
Der Editor ist der einzige Bereich in KRK, der einen Zustand hält, den ein Schließen verlieren kann. C4 trägt elf Abnahmekriterien, und die ersten neun hängen an diesem Bild; die beiden letzten sagen über die Form des geschriebenen Standes zu und nicht über den Weg dorthin. Das Bild zeigt, wie die neun zusammenhängen, und macht prüfbar, ob die Fallunterscheidung vollständig ist:
...
**Abnahmekriterien:**
- [ ] F4 im Dateifenster öffnet den ausgewählten Eintrag im Editor. Die Taste war dafür seit der Runde 1 freigehalten; seit dieser Runde trägt die Funktion `bearbeiten` in `resources/default-keymap.toml` die Tastenliste `["f4"]` und das zugehörige Kommando, und das Feld `reserviert_fuer = "editor"` steht nicht mehr dort. Wer beim Abnahmelauf nach dem Feld sucht, sucht den eingelösten Zustand und nicht den Befund.
...
- **Der Nutzer hat am 260808-0017 die eigene, höhere Grenze gewählt: rund 16 MB, nur Text** (`decisions/260807-2147_*_welche-dateien-oeffnet-der-editor-ueberhaupt.md`). Damit stehen zwei Zahlen für dieselbe Frage in zwei Flächen: die Vorschau nimmt Text bis 1 MB (`TEXTGRENZE` in `crates/krk-ui/src/vorschaumodell.rs:83`), der Editor bis rund 16 MB. Beide tragen dieselbe Regel, nämlich eine Obergrenze für das vollständige Einlesen in den Arbeitsspeicher; verschieden ist allein, wie viel die jeweilige Handlung rechtfertigt. `speculation:` Die 16 MB sind ein Vorschlag und keine gemessene Größe.
...
- **Die einklappbaren Blöcke entfallen in dieser Runde.** Hervorhebung braucht Wortarten, Einklappen braucht Blockgrenzen; das sind zwei Kenntnisse und nicht eine, und die gewählte Kiste liefert die erste. Die Festlegung vom 260807-2139 lautete "Syntaxhervorhebung mit einklappbaren Blöcken" und ist damit zur Hälfte zurückgenommen. Der Datensatz `shared/decisions/260802-0842_*_editor-formatansicht-je-dateityp.md` trägt den Nachtrag vom 260808-0017. Die Blöcke stehen unten unter `## Ausdrücklich außerhalb dieser Runde`.
...
- **Nachtrag vom 260810-0714: es sind zwei Kisten geworden und nicht eine, und der Grund ist gemessen.** Gewählt hat der Planner `syntect` für die Erkennung und Einfärbung. Sein Vorgabesatz führt Rust, Markdown und Shell, aber kein TOML, und das fünfte Abnahmekriterium oben verlangt TOML ausdrücklich; `two-face` trägt den erweiterten Sprachsatz nach. Beide stehen mit geschriebener Begründung in `Cargo.toml`, wie die vier bestehenden, und beide sind ohne ihre Vorgabemerkmale eingebunden: der Vorgabesatz beider zöge eine Bibliothek in C herein und änderte damit die Bauvoraussetzungen des Projekts. Das zwölfte Abnahmekriterium oben gilt für beide. Die Zeile, die den Befund festhält, ist `crates/krk-ui/tests/syntaxkiste.rs::ohne_two_face_fehlt_toml`; sie schlägt fehl, sobald `syntect` TOML nachreicht, und das ist der Anlass, die zweite Kiste neu zu bewerten.
...
- [ ] Dieselbe Nachfrage erscheint, wenn der Editor eine andere Datei aufnehmen soll, gleich auf welchem der drei Wege sie kommt: über F4, über den Übergang aus der Vorschau oder über den Sprung auf eine Textmarke aus C6. C2 zählt dieselben drei an seiner Stelle auf, und der Abnahmelauf prüft alle drei.
...
- [ ] Beim Sichern schreibt der Editor ausschließlich `\n` als Zeilenende, hängt genau einen abschließenden `\n` an, wenn der Stand keinen trägt, und schreibt keine Bytefolgenmarke an den Dateianfang. Das gilt unabhängig von der Form, die die geöffnete Datei mitbrachte.
- [ ] Keine Automatik der Textfläche ändert den getippten Text. Was der Nutzer tippt, einfügt oder ausschneidet, steht beim Sichern Zeichen für Zeichen in der Datei: keine typografischen Anführungszeichen, keine Gedankenstriche, keine Textersetzung, keine Rechtschreibkorrektur, kein eingefügtes oder fortgenommenes Leerzeichen beim Einfügen und Ausschneiden, keine Wortvorhersage und keine Formelergänzung.
...
- **Ein zweiter abgeleiteter Anlass stand hier bis zum 260810 und ist gefallen.** Das sechste Abnahmekriterium verlangte die Nachfrage auch vor dem Einblenden der Vorschau, mit der Begründung, das Verdrängen durch die Vorschau verliere denselben Stand wie das Schließen. Am gebauten Code stimmt diese Begründung nicht: ein Wechsel der Sichtbarkeit setzt `hidden` an den Ansichten und fasst das Editormodell nicht an. Der verdrängte Editor hält seinen Stand weiter, und "Verwerfen" verwarf an dieser einen Stelle nichts. Der Nutzer hat den Anlass am 260810-0250 fallen lassen; der Datensatz mit den drei geprüften Möglichkeiten ist `decisions/260810-0021_*_was-verwirft-verwerfen-wenn-die-vorschau-den-editor-nur-verdraengt.md`. Das sechste Abnahmekriterium ist mit ihm gestrichen, und die Nachfrage steht seither genau dort, wo etwas auf dem Spiel steht.
- **Die Nummern der Abnahmekriterien haben sich dabei um eins verschoben.** Wer in einem Dokument von vor dem 260810 das siebte bis zehnte Abnahmekriterium von C4 zitiert findet, liest es hier als das sechste bis neunte. Der Code ist nachgezogen.
- **Zwei Kriterien sind am 260810-0714 hinten angehängt worden, und die Stelle ist gewählt und nicht zufällig.** C4 trägt seither elf. Ein Einschub in der Mitte hätte die Nummern zum zweiten Mal an einem Tag verschoben, und sechzehn Stellen im Programmtext zitieren ein Kriterium von C4 nach seiner Nummer. Angehängt bleibt jede bestehende Zahl richtig.
- **Das zehnte Abnahmekriterium löst den Datensatz zur Sicherungsform ein.** `decisions/260808-0021_*_was-sagt-der-editor-beim-sichern-ueber-den-unveraenderten-teil-der-datei-zu.md` war beim Schreiben des Specs noch offen, und die Antwort fiel nach seiner Überarbeitung; der Plan hat das Kriterium bei S9 abgeleitet und hierher überwiesen. Der Nutzer ist der Empfehlung des Datensatzes nicht gefolgt und hat die vereinheitlichende Form gewählt. Der Preis ist angenommen und benannt: das Sichern ändert Zeilen, die der Nutzer nicht angefasst hat, und eine Datei aus einem Windows-Projekt kommt mit anderen Zeilenenden zurück. Die Wandlung geschieht beim **Einlesen**, weshalb auch die Rohansicht aus C3 keine Wagenrücklaufzeichen mehr zeigt; das ist die einzige Form, in der Zeilennummern, Suche, Textmarken und Sicherung dieselbe Zeichenkette meinen.
- **Das elfte Abnahmekriterium stand bis zum 260810 in keinem Spec-Satz, obwohl der Code es seit S16 einlöst.** Die Zusage "der gesicherte Stand ist der getippte" lebte im Plan bei S16, im Modulkopf von `crates/krk-ui/src/appkit/editor.rs` und in den Datensätzen. Das Nächstliegende im Spec stand unter C2 und meint etwas anderes: dort geht es um die Binärdatei, die der Editor gar nicht erst annimmt, und nicht um die Ersetzung, die AppKit an gültigem Text vornimmt. Ein Abnahmelauf prüft, was geschrieben steht, und deshalb steht es jetzt unter C4, der Fähigkeit, die über den Weg vom Getippten in die Datei zusagt.
- **Sieben Automatiken sind abgeschaltet und nicht vier, und die Zahl ist zweimal gewachsen.** Vier greifen beim Tippen: Anführungszeichen, Bindestriche, Textersetzung, Rechtschreibkorrektur. Die fünfte greift beim Einfügen und Ausschneiden und blieb an, weil die Aufzählung nach den vier tippenden aufhörte. Die sechste und die siebte, die Wortvorhersage aus macOS 14 und die Formelergänzung aus macOS 15, blieben an, weil die Aufzählung nach einer Namensform fragte, die diese beiden nicht tragen. Das Kriterium oben nennt deshalb die Wirkung und keine Zahl; die Zahl steht im Modulkopf, wo sie nachgezählt werden kann.
- **Die Schreibwerkzeuge aus macOS 15 sind von diesem Kriterium nicht erfasst, und das ist eine offene Frage.** Sie schreiben markierten Text um, unterscheiden sich von den sieben aber darin, dass der Nutzer sie eigens aufruft. Ob C4 sie trotzdem ausschließt, ist eine Lesart und keine Codefrage; sie steht in `issues/260810-0512_*_die-schreibwerkzeuge-aus-macos-15-schreiben-den-text-um-und-sind-nicht-abgewaehlt.md` und hält die Abnahme der übrigen zehn Kriterien nicht auf.
...
- **Das Anlegen einer Textmarke liegt auf dem bestehenden Befehl `lesezeichen_anlegen` und bekommt kein eigenes Kommando.** Nachtrag vom 260810-0714, gebaut in S38. Der Spec zählte das Anlegen unter den neuen Befehlen dieser Runde auf; der Plan hat es auf `cmd+d` gelegt, weil es dieselbe Handlung an derselben Liste ist. Der Fokus wählt die Sorte: im Dateifenster entsteht eine Ordnermarke, im Editor eine Textmarke auf der Zeile der Schreibmarke. Eine Funktion, eine Kombination, ein Eintrag in der Belegungsansicht; ein zweiter Anlegebefehl daneben wäre die zweite Tür zu der einen Liste, die diese Fähigkeit zusagt. Steht der Fokus im Editor und hält er keine Datei, meldet der Befehl das und legt nichts an, statt auf den Ordner des aktiven Dateifensters auszuweichen.
...
- **`Wirkungsbereich` ist auf sieben Werte gewachsen, und drei bestehende Befehle sind mit umgezogen.** Nachtrag vom 260810-0714, gebaut in S3 und S5. Zu den vier Werten der Runde 1 kommen `Vorschau` für den einen Befehl, der allein im Vorschaufenster wirkt, `Editor` für die Befehle aus C3, C5 und C6, und `Navigator` für die Befehle, deren Taste im Editor der Textfläche gehört. Der dritte Wert ist der, den man nicht kommen sieht: `fenster_wechseln` auf `tab`, `auswahl_hoch` auf `up` und `auswahl_runter` auf `down` standen seit der Runde 1 auf `Ueberall`, weil es damals nichts gab, wovon sie auszunehmen wären. Ohne den Umzug bewegte `up` im Editor die Auswahl im Dateifenster statt der Schreibmarke, und das erste Abnahmekriterium oben fiele. `Navigator` ist positiv formuliert und nicht als Verneinung von `Editor`, damit ein stehendes Blatt ausgeschlossen bleibt.
- **Die größte einzelne Zunahme dieser Runde steht damit in der Belegung und nicht im Editor.** Der Preis ist benannt: der Fokusvorbehalt bleibt dafür eine Regel und wird keine Abfrage je Aufrufstelle, und die vorige Festlegung verlangt genau das.
...
- **Was gemessen ist und was nicht.** Gemessen ist, dass `cmd+y` und `shift+cmd+y` am laufenden Bündel nichts auslösen und `f3` schon, festgestellt vom Nutzer am 260807-2112. Nicht gemessen war beim Schreiben dieses Absatzes die Ursache; die beiden Verdächtigen des Datensatzes waren nach Prüfaufwand geordnet und nicht nach Wahrscheinlichkeit.
- **Nachtrag vom 260810-0714: die Ursache ist gemessen, und keiner der beiden Verdächtigen traf zu.** Das Hauptmenü führt sieben Einträge und keinen mit `y`, und die Normalisierung der Zusatztastenmaske vergleicht Wert gegen Wert, was `f3` mit seinem Funktionstastenbit belegt. Die Ursache lag außerhalb des Programms: der Nachschlag ging über die **Stelle** der Taste, und auf einer deutschen Tastatur liegt die Stelle `kVK_ANSI_Y` unter der Aufschrift Z. Damit ist das vierte Abnahmekriterium oben eingelöst, und zwar in der einzigen Form, in der es einlösbar war (S1, Abschlussnotiz in `shared/issues/260807-2112_*_cmd-y-und-shift-cmd-y-loesen-nichts-aus-f3-schon.md`).
- **Der Nutzer hat am 260808-0155 den dritten Weg gewählt, gegen die Empfehlung des Datensatzes.** `decisions/260808-0140_*_die-y-tasten-liegen-auf-einer-deutschen-tastatur-unter-anderen-buchstaben.md` legte drei Wege vor und empfahl den ersten, also die beiden Abnahmekriterien oben auf den Ist-Zustand umzuschreiben. Gewählt und in S2 gebaut ist der dritte: Buchstaben und Ziffern werden über das **gemeldete Zeichen** nachgeschlagen, Funktionstasten weiter über die Stelle. **Die beiden ersten Abnahmekriterien bleiben damit unverändert stehen und sind buchstäblich erfüllbar geworden**, auf jeder Tastaturbelegung und nicht nur auf der deutschen und der amerikanischen. Der vom Plan vorgesehene Nachtrag für den ersten Weg entfällt ersatzlos, weil seine Bedingung nicht eingetreten ist.
...
- **Nachtrag vom 260810-0714: "die Anzeige" im zweiten Abnahmekriterium meint die volle Akzentfarbe.** Das Kriterium verlangt, dass genau ein Bereich sie trägt. Unter der Vorbelegung des Shapers tragen zwei Bereiche eine Markierung, nämlich der fokussierte in voller Akzentfarbe und das aktive Dateifenster ohne Fokus in einer zurückgetretenen Fassung derselben Farbe. Die Lesart ist damit festgelegt und nicht offen: gezählt wird die volle Farbe, und das aktive Dateifenster ohne Fokus zählt nicht mit. Sie ist die einzige, die mit der Vorbelegung zusammengeht, und der Datensatz zum Akzentrahmen sagt sie nicht. Wer jenen Datensatz zugunsten der dritten Möglichkeit beantwortet, bekommt die buchstäbliche Lesart zurück und ändert dazu einen Funktionsrumpf.
- **Die zurückgetretene Form ist dieselbe Systemfarbe mit verringerter Deckkraft, gebaut in S44.** Drei Systemfarben und keine eigene Tafel: die Akzentfarbe für den fokussierten Bereich, dieselbe Farbe mit verringerter Deckkraft für das aktive Dateifenster ohne Fokus, die Trennfarbe für alles übrige. Steht das Fenster im Hintergrund, bekommt auch der fokussierte Bereich die zurückgetretene Fassung; damit tritt die Anzeige zurück, statt zu verschwinden, wie das achte Abnahmekriterium oben es verlangt.
...
- **Bei Markdown decken sich Ansicht und Datei zeilenweise, und das ist entschieden und nicht angenommen.** Die Antwort vom 260808-0155 (`decisions/260808-0140_*_was-heisst-gerendert-bei-markdown-wenn-zugleich-bearbeitet-wird.md`) lautet, dass die Auszeichnungszeichen stehen bleiben und allein ihre Wirkung gezeigt wird. Damit steht in der Formatansicht Zeichen für Zeichen der Stand der Datei, und die Nummern gelten dort unverändert. Wäre die Gegenmöglichkeit gewählt worden, also eine Darstellung wie im Browser, trüge die Formatansicht keine Dateizeilen mehr und C10 hätte für sie keine Zusage.
...
**Gebaut sind daraus drei Werte und nicht einer, und `Wirkungsbereich` trägt seit dieser Runde sieben.** Nachtrag vom 260810-0714. Zu `Editor` sind `Vorschau` und `Navigator` gekommen; die Begründung für jeden der drei steht in den Festlegungen von C7 und im Modulkopf von `belegung.rs`. Drei bestehende Befehle sind dabei von `Ueberall` nach `Navigator` umgezogen, `fenster_wechseln`, `auswahl_hoch` und `auswahl_runter`, weil ihre Tasten im Editor der Textfläche gehören. Es ist die größte einzelne Zunahme dieser Runde, und sie steht in der Belegung und nicht im Editor.
...
- **Die neuen fremden Kisten fügen sich in ein bestehendes Muster ein.** `Cargo.toml` führte vor dieser Runde vier Kisten mit Wirkung auf die Anwendung, jede mit einer geschriebenen Begründung, die sagt, was sie leistet, warum keine Alternative sie ersetzt und welche Vorgabemerkmale abgeschaltet sind. Für die Syntaxhervorhebung sind zwei dazugekommen, `syntect` und `two-face`, beide mit derselben Begründung an derselben Stelle; der Grund für die zweite steht in den Festlegungen von C3. Eine Abhängigkeit ohne geschriebenen Grund entsteht nicht.
...
- **Die beiden Fragen der Runde 1, die an der Lesestelle hängen.** `circles/260802-0842-krk-mac-dateimanager-editor-git/decisions/260807-0010_*_kann-der-auffrischungsaufschub-entfallen-nachdem-die-lesestelle-nicht-mehr-vorab-leert.md` und `circles/260802-0842-krk-mac-dateimanager-editor-git/decisions/260807-0020_*_soll-die-markierung-eine-auffrischung-ueberleben.md` betreffen das Dateifenster. Diese Runde fasst die Lesestelle nicht an: der Editor liest eine einzelne Datei und kein Verzeichnis, und die Auffrischung nach einer Sicherung geht über den bestehenden FSEvents-Weg. Sie binden diese Runde damit nicht.
...
**Was die Erweiterung den Zuschnitt kostet, ist benannt und nicht klein.** Die drei Fähigkeiten bringen einunddreißig Abnahmekriterien mit: acht für die Fokusanzeige, zwölf für die Zeilennummern, elf für den Fenstertitel. Zwei der drei fassen Flächen an, die seit der Runde 1 stehen. Der Gegenwert ist, dass die Runde nicht mit einer Anwendung endet, in der der Nutzer raten muss, wo seine Tasten ankommen.

**Die Zahl der Kriterien von C1 bis C8 steht hier bewusst nicht mehr.** Sie hat sich in vier Tagen dreimal geändert, zuletzt am 260810 in beide Richtungen: das sechste Kriterium von C4 ist gestrichen, zwei Kriterien sind hinten angehängt. Jede Stelle, die eine solche Zahl ausschreibt, veraltet, und drei Stellen dieses Projekts haben es bereits getan. Wer sie braucht, zählt sie, und zwar so, dass die Abschnitte hinter C11 nicht mitgehen:
...
**Ein Zählweg, der die Überschrift nur auf `### C` setzt, misst falsch, und die Falle hat schon zugeschlagen.** Er trägt die letzte Fähigkeit über die Abschnittsgrenze hinaus weiter und schlägt die beiden Kriterien aus `## Verhältnis zu den zehn Zeitzusagen aus C8 der Runde 1` C11 zu. So ist der Defekt `issues/260810-0359_*_die-erweiterungsnotiz-zaehlt-elf-abnahmekriterien-fuer-c11-gebaut-sind-dreizehn.md` entstanden, und so ist er am 260810-0714 auch wieder geschlossen worden: C11 trägt elf, und die einunddreißig oben stimmen.
...
**Die Sitzung hat diesen Spec nicht angefasst.** `git log 38a02b2..HEAD` nennt für diese Datei keinen Commit. Alles unten ist Prüfung, keine Änderung außer diesem Abschnitt.

**Die Zählung von 110 hält, jetzt zum zweiten Mal unabhängig nachgezählt.** 108 Kästchen in den elf Fähigkeiten (Zeilen 213 bis 486) und zwei in `## Verhältnis zu den zehn Zeitzusagen aus C8 der Runde 1` (Zeilen 487 bis 507), zusammen 110, davon **null** abgehakt. Je Fähigkeit: C1 sieben, C2 elf, C3 zwölf, C4 elf, C5 neun, C6 vierzehn, C7 acht, C8 fünf, C9 acht, C10 zwölf, C11 elf. Die Angabe 111 aus `agentstate.yaml`, die der Eintrag vom 260810-0805 widerlegt, steht in dieser Datei nicht mehr; die Datei ist in dieser Sitzung neu geschrieben worden.

**Zwei Stellen dieses Specs behaupten einen Marker, den der Bestand nicht mehr trägt.** Zeile 10 (`## Gatehinweis für den Nutzer`) und Zeile 583 sagen, die Datensätze unter `decisions/` trügen den Marker `_a_`. Im Bestand trägt keiner dieses Speichers ihn: zehn `_i_`, zwei `_o_`, ein `_s_`, gezählt am 260810-1404. Beide Stellen sind Prosa über den Stand und keine Pfade, die Sternstellen-Regel greift dort nicht. Die Aussage, um die es ihnen geht — die fünf Fragen sind beantwortet und tragen eine `Answered:`-Zeile — ist weiter wahr; falsch ist allein der genannte Marker, weil die Antworten inzwischen in Code eingelöst und die Datensätze nach `_i_` gewandert sind. Der `reconciler` schreibt keine Kopf- und Gatehinweiszeilen um; der Nutzer zieht sie nach.
