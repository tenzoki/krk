...
**Datum:** 2026-08-08, 01:40, abgeglichen am 2026-08-10
**Status:** Gebaut. Alle 48 Schritte sind gebaut und am Code nachgeprüft; die Abnahme am laufenden Bündel ist Nutzerarbeit. **Alle 48 Abnahmekriterien werden vom gebauten Code eingelöst.** Die beiden, die es am 260810-0805 nicht taten, sind am 260810-0822 berichtigt worden, und in beiden Fällen war das Kriterium falsch und nicht der Code: S6 verbot zwei Buchstaben aus einem Grund, den S2 beseitigt hat, und S33 verlangte eine Mechanik, die drei von vier zugesagten Wirkungen nicht trägt. Die Einzelheiten stehen unten unter `## Reconciliation Log`, Abschnitt `### 260810-0822`. **Über den Zustandsmarker im Dateinamen ist damit neu zu entscheiden**, gemeinsam mit dem des Specs; die Entscheidung gehört dem Orchestrator, weil sie die Zeile `**Active spec/plan:**` im Circle-Datensatz nachzieht. Die Konvention `rules/fusion-workbench-conventions.md`, Abschnitt `### Planning files`, verlangt die Umbenennung, sobald alle Schritte `[DONE]` tragen.
**Spec:** `circles/260807-2116-eingebauter-editor-mit-textmarken/planning/260807-2147_*_spec-eingebauter-editor-mit-textmarken.md`, acht Fähigkeiten C1 bis C8, vom Nutzer am 260808-0043 abgenommen; dazu seit dem 260809-2043 die drei Anzeigefähigkeiten C9, C10 und C11, die der Abschnitt `## Nachtrag vom 260809` unten baut. **Eine Zahl der Abnahmekriterien steht hier nicht mehr**: sie hat sich in vier Tagen viermal geändert, und diese Zeile trug sie von Anfang an um eins zu hoch. Der Spec sagt unter `## Die vier später hinzugekommenen Fähigkeiten`, wie man sie zählt.
...
Damit sind die Abnahmekriterien von C8 in ihrer heutigen Fassung nicht zu erfüllen: `cmd+y` blendet die Vorschau ein und aus, aber nur auf der Taste mit der Aufschrift Z, und kein Programmteil ändert daran etwas, ohne die Festlegung aus C3 anzufassen. **Die Wahl gehört dem Nutzer** und liegt als `decisions/260808-0140_*_die-y-tasten-liegen-auf-einer-deutschen-tastatur-unter-anderen-buchstaben.md` vor. Ein Punkt daraus gehört hierher, weil er den dritten Weg billiger macht, als der Datensatz von 260803 ihn eingeschätzt hat: **das Hauptmenü schlägt bereits heute über das Zeichen nach.** `NSMenuItem.keyEquivalent` nimmt eine Zeichenkette entgegen (`menue.rs:322-342`), und genau deshalb wirken `cmd+c` und `cmd+v` auf jeder Tastaturbelegung an der beschrifteten Stelle. Eine zeichenbasierte Nachschlagart wäre also keine fremde Mechanik, sondern die, die vier Funktionen dieses Projekts schon tragen.

**Kein Schritt dieser Runde hängt an der Frage, und keiner der neuen Tastenbefehle meidet `y` oder `z`.** Eine Meidung hätte einen Gegenstand gehabt, solange der Ereignisabgriff auch Buchstaben über die Stelle nachschlug: `kVK_ANSI_Y` und `kVK_ANSI_Z` tauschen zwischen der deutschen und der amerikanischen Belegung den Platz, und eine Funktion dort läge nicht unter ihrer Aufschrift. S2 hat die Frage anders beantwortet und den dritten Weg gebaut, den Nachschlag über das gemeldete Zeichen für Buchstaben und Ziffern. Seither wandert keine Stelle mehr, `cmd+y` liegt auf jeder Tastaturbelegung unter der Aufschrift Y, und die Tabelle in `### Frage 11` legt `text_rueckgaengig` und `text_wiederholen` ohne Vorbehalt auf `cmd+z` und `shift+cmd+z`.

**Nachgezogen am 260810-0822.** Bis dahin stand hier "Kein neuer Tastenbefehl dieser Runde liegt auf `y` oder `z`". Der Satz war schon beim Schreiben mit der Tabelle in `### Frage 11` unverträglich und ist seit S2 zusätzlich gegenstandslos. Die beiden Datensätze dazu sind `issues/260809-1527_*_der-plan-verbietet-y-und-z-und-legt-rueckgaengig-selbst-auf-cmd-z.md` und `issues/260809-1746_*_die-probe-auf-die-wandernden-stellen-hat-ihren-grund-verloren.md`; der dort vorgeschlagene Schnitt entlang des Zustellers ist geprüft und trägt nicht, die Begründung steht unten im `## Reconciliation Log`.
...
**`syntect` in seiner reinen Rust-Fassung, und `two-face` daneben, falls die Messung in S32 zeigt, dass `syntect` TOML nicht mitbringt.** Die Wahl ist ein eigener Schritt mit Kriterien (S32), weil zwei ihrer vier Kriterien am gebauten Programm zu messen und nicht am Papier zu entscheiden sind.

Warum `syntect`: es ist die einzige verbreitete Rust-Kiste, die Erkennung **und** Einfärbung mitbringt, ohne eine C-Werkzeugkette zu verlangen. Ihr Merkmalssatz `regex-fancy` tauscht die C-Bibliothek Oniguruma gegen `fancy-regex` in reinem Rust; der Bauzusammenhang steht in ihrer eigenen `Cargo.toml` (Fassung 5.3.0). Die Merkmale, die KRK nennt, sind `parsing`, `default-syntaxes`, `default-themes`, `dump-load` und `regex-fancy`; abgeschaltet bleiben `html`, `plist-load`, `yaml-load` und `metadata`, weil KRK die Einfärbung selbst in Textmerkmale umsetzt und keine HTML-Ausgabe braucht.

**Berichtigt am 260810: `dump-create` stand hier als fünftes abgeschaltetes Merkmal und lässt sich nicht abschalten.** `parsing = [regex-syntax, fnv, dump-create, dump-load]` in der `Cargo.toml` von `syntect` 5.3.0 zieht es mit, und `parsing` ist das Merkmal, ohne das die Kiste nichts tut. Die Angabe war damit sachlich falsch und nicht bloß unerfüllt. Die Wirkung ist gering: `dump-create` bringt `bincode` und `flate2` mit, die über `dump-load` ohnehin im Baum stehen, und kein Ausschlusskriterium von S32 ist berührt. Der Befund steht als `issues/260808-0949_*_s32-nennt-dump-create-unter-den-abgeschalteten-merkmalen-es-laesst-sich-nicht-abschalten.md` und als Kommentar an der Versionsangabe in der Wurzel-`Cargo.toml`.
...
**Der angenommene Preis bleibt angenommen.** `speculation:` Ob die Kiste die Maxime "superschnell" auf dem Referenzgerät von 2018 hält, ist ungemessen. Diese Runde misst es nicht, weil der Abnahmelauf ausgeklammert ist; abgenommen werden stattdessen die beiden Größen, die ohne Messstrecke prüfbar sind, nämlich der Preis der Kisten am Programm und die Fortdauer von `#![deny(unsafe_code)]`. **Berichtigt am 260810:** die Größe trägt S32 nur zur Hälfte. Die Fortdauer von `#![deny(unsafe_code)]` prüft er, den Preis am Bündel kann er nicht prüfen, weil in S32 kein Aufrufer die Kisten nennt und der Übersetzer den eingebetteten Bestand wieder wegwirft. S32 wiegt deshalb ein eigenständiges Prüfprogramm, und das Wachstum des Bündels ist an S33 abgenommen, wo der Bestand zum ersten Mal geladen wird.
...
**Ein `NSTextStorage`, zwei Darstellungen, und die Merkmale an zwei Orten statt an einem.**

Die Fallunterscheidung heißt **"wirkt auf die Auslegung oder nicht"**, und welche Seite ein Merkmal trägt, sagt AppKit selbst und nicht eine Liste in KRK:

> Clients may set any attributes they wish, but the only attributes that the layout manager will recognize for drawing are those that do not affect layout (color, underline, etc.).
>
> `MacOSX.sdk/System/Library/Frameworks/AppKit.framework/Headers/NSLayoutManager.h:351`

Farbe und Unterstreichung ändern die Auslegung nicht und gehen deshalb als vorübergehende Merkmale in den Layoutverwalter, über `NSLayoutManager::setTemporaryAttributes:forCharacterRange:`. Schriftgröße, Schriftschnitt, feste Schrift und Absatzeinzug ändern sie und gehen über `addAttributes:range:` in den `NSTextStorage`. Als vorübergehendes Merkmal gesetzt täten die vier nichts, nicht etwas Falsches, sondern gar nichts. Die Fallunterscheidung ist damit trennscharf und vollständig, weil der Kopf oben beide Seiten benennt.

**Die Zusage hängt nicht an den Merkmalen, sondern an den Zeichen.** Sie lautet: die Auszeichnung kann beim Sichern nicht in die Datei geraten. Gesichert wird `Editormodell::stand`, eine gewöhnliche Zeichenkette aus `NSTextView::string`, also aus den **Zeichen** der Fläche. Kein Merkmal wird auf dem Sicherungsweg auch nur gelesen, gleich in welchem der beiden Speicher es liegt. Daraus folgt dasselbe Dreierlei wie zuvor: die Auszeichnung ist niemals Teil des Dokuments; das Umschalten der Ansicht ist ein Zurücksetzen und Neusetzen von Merkmalen und kein Umbau des Standes; und es gibt keine zweite Textkopie, die auseinanderlaufen könnte. Das ist die Antwort auf das zehnte Abnahmekriterium von C3, "beide Ansichten arbeiten auf demselben Stand und nicht auf zwei Kopien", und sie ist eine Eigenschaft der Bauart und keine Zusage der Sorgfalt.
...
### Die Nachfrage vor den drei Anlässen

Der Plan entwarf vier Anlässe. Der Nutzer hat am 260810-0250 den Anlass "Vorschau einblenden" fallen lassen, weil ein verdrängter Editor seinen Stand behält und "Verwerfen" dort nichts verwarf (`decisions/260810-0021_*_was-verwirft-verwerfen-wenn-die-vorschau-den-editor-nur-verdraengt.md`). Dieser Abschnitt und das Bild darunter sind auf die drei gezogen, die geblieben sind: den Editor schließen, eine andere Datei aufnehmen, die Anwendung beenden.
...
Die Fortsetzung reist in der Schließung mit, so wie bei jedem Blatt der Runde 1 (`anwendung.rs:1822-1852` ist das reinste Beispiel). Ein Feld, das eine noch nicht ausgeführte Absicht über den Rückruf hinaus hält, entsteht nicht; es wäre die zweite Wahrheit, vor der jeder Modulkopf dieses Projekts warnt.

**Der Plan sah hier je Anlass eine eigene Aufrufstelle vor; gebaut ist eine für alle.** Die Anlässe teilen mehr als das Blatt, und was sie am stärksten teilen, ist die Behandlung des gescheiterten Sicherns. Mehrfach aufgeschrieben wären das ebenso viele Stellen, an denen jene Zusage zu halten oder zu brechen ist. Der Grund im Einzelnen und die Aufzählung `Anlass`, an der ein weiterer Anlass den Bau anhält, stehen bei S28.
...
**Die Schnittstellen-Regel, nachgetragen am 260810.** Eine dritte Herleitungsregel kommt dazu, und sie ist die einzige der drei, die auf der bindenden Verbotsseite steht: **ein Schritt, der eine öffentliche Form ändert, zieht ihre Aufrufstellen im selben Schritt nach**, auch wenn ein späterer Schritt sie ohnehin anfassen wird. Die Lese- und Begründungsliste darf weiter unvollständig sein; der Arbeitsbereich darf zwischen zwei Schritten nicht rot stehen. `cargo build --workspace` und `cargo test --workspace` sind nach `CLAUDE.md` die Abnahmekommandos des Projekts, und ein Schritt, nach dem sie scheitern, ist nicht abgenommen, gleich was seine Dateiliste sagt.

Der Anlass ist S11. Er ändert `Lesezeichen` in `krk-core` und weist die zwei Aufrufstellen in `crates/krk-ui/src/leistenmodell.rs` den Schritten S38 und S39 zu, die nach der Abhängigkeitsordnung 26 Positionen später liegen; dazwischen hätte kein Schritt gegen einen übersetzbaren Arbeitsbereich gearbeitet. Der Defekt ist `issues/260808-0930_*_s11-aendert-eine-kernschnittstelle-deren-aufrufstellen-der-plan-erst-s38-und-s39-zuweist.md`; S11 trägt den Nachzug seit dem 260808 in seiner Dateiliste und in seinem Umsetzungsvermerk. Für die sechs Schritte des Nachtrags ist die Regel schon in den Zuschnitt eingegangen (`### Wie diese sechs Schritte geschnitten sind`), und die Formen, die sie ändern, stehen dort vollständig aufgezählt (`### Wo diese Schritte eine öffentliche Form ändern`). Kandidaten derselben Lücke unter den ersten 42 Schritten sind die, die eine öffentliche Form in `krk-core` ändern, während `krk-ui` sie benutzt: bei S3 und S5 nennt der Plan die `krk-ui`-Seite ausdrücklich mit, dort greift sie also nicht.
...
- Abnahmekriterium: `cargo test -p krk-core` beendet mit 0. Die Konflikterkennung aus C3 meldet keinen Konflikt: `Belegung::bauen` macht den ersten Fund zum Fehler (`belegung.rs:789-794`), und ein grüner Bau der Belegung ist damit der Nachweis, dass keine der dreizehn Kombinationen eine bestehende bei demselben Zusteller doppelt. Eine Probe hält fest, dass die Datei 71 Funktionen führt und dass die dreizehn neuen Kennungen darin stehen (`die_auslieferungsbelegung_fuehrt_einundsiebzig_funktionen`).
- **Kriterium nachgezogen am 260810-0822.** Der zweite Halbsatz verlangte, dass "keine Tastenliste die Taste `y` oder `z` neu belegt". Er war mit der Tabelle in `### Frage 11` von Anfang an unverträglich, die `text_rueckgaengig` auf `cmd+z` und `text_wiederholen` auf `shift+cmd+z` legt, und er hat mit S2 seinen Gegenstand verloren: Buchstaben gehen seither über das gemeldete Zeichen, und keine Stelle wandert mehr. Er ist ersatzlos gestrichen, und die Probe `keine_neue_kombination_liegt_auf_den_beiden_wandernden_stellen` ist mit ihm gefallen. Was sie noch trug, tragen `auf_einer_deutschen_tastatur_findet_die_aufschrift_y_die_vorschau` und `jede_ausgelieferte_kombination_traegt_die_kennung_ihrer_tastensorte` in derselben Datei, und zwar an der Sache selbst statt an einer Vorsichtsregel.
...
- Ausführender: `coder`
- Dateien: `crates/krk-core/src/ablage/lesezeichen.rs` (erweitert: `Lesezeichen`, `Ziel`, `gueltig`, der Modulkopf), `crates/krk-core/tests/ablage.rs` (erweitert), `crates/krk-ui/src/leistenmodell.rs` (nachgezogen: `gewaehlt` und `anlegen`, die beiden Aufrufstellen der geänderten Form)
...
- Abhängigkeiten: keine
- Abnahmekriterium: `cargo test -p krk-core` beendet mit 0 und deckt ab: eine `bookmarks.toml` **in der Form vor dieser Runde**, also mit `name` und `ordner` und ohne jedes weitere Feld, wird eingelesen und liefert drei Ordnermarken; eine Rundreise über beide Sorten liefert byteweise dieselbe Datei; eine geschriebene Datei ist von Hand lesbar, was die Probe daran festmacht, dass sie keine geschachtelte Tabelle und keine Sortenkennung enthält; ein Lesezeichen kann nicht beide Sorten zugleich tragen, was der Typ erzwingt und die Probe am fehlenden Konstruktor festhält; `gueltig()` einer Textmarke auf eine bestehende Datei ist wahr, auf einen bestehenden Ordner falsch. Dazu, seit dem Nachtrag vom 260810: `cargo build --workspace` und `cargo test --workspace` beenden mit 0, nicht nur `cargo test -p krk-core`. Die Schnittstellen-Regel aus `### Was die Dateiliste eines Schrittes zusagt` verlangt das, weil dieser Schritt eine öffentliche Form ändert.
- **Umsetzung am 260808:** die beiden Aufrufstellen in `crates/krk-ui/src/leistenmodell.rs` sind im selben Commit (`65c8efa`) minimal auf die neue Form gehoben, ohne Verhalten zu ändern und ohne die zweite Sorte vorzubauen: `Leistenmodell::gewaehlt` liest den Pfad aus `Ziel::Ordner` und liefert für eine Textmarke `None`, `Leistenmodell::anlegen` verpackt den Pfad in `Ziel::Ordner` und behält seine Signatur. Beide Stellen tragen einen Kommentar, dass S38 und S39 sie ablösen. Ohne den Nachzug hielt `cargo build --workspace` mit zwei Fehlern an (`leistenmodell.rs:352` und `:382`), und zwar bis S38, den die Abhängigkeitsordnung 26 Positionen später führt. Der Befund und die daraus abgeleitete Regel stehen als `issues/260808-0930_*_s11-aendert-eine-kernschnittstelle-deren-aufrufstellen-der-plan-erst-s38-und-s39-zuweist.md`.
...
- **Umsetzung am 260809-1547:** `crates/krk-ui/src/editormodell.rs` mit `Editormodell`, `Ansicht`, `Dateityp`, `Stempel`, `Suchlauf`, `Ladevorgang`, `Ladeausgang` und `Sicherungsausgang`; neunzehn Proben stehen als `#[cfg(test)] mod tests` in der Datei. Die vier Abnahmekommandos laufen durch, `grep -c 'objc2'` auf die Datei liefert 0. Sechs Abweichungen von der Schrittbeschreibung:
  ...
  - **Der Modulkopf trägt `#![allow(dead_code)]`.** Zwischen S15 und S16 hat das Modul keinen Aufrufer im Programm, und `make lint` fährt `cargo clippy -- -D warnings`; ohne die Zeile stünde der Arbeitsbereich zwischen den beiden Schritten rot. Der Kommentar nennt S16 als ablösenden Schritt, wie `issues/260808-1413_*_vier-platzhalter-nennen-ihren-abloesenden-schritt-nicht-obwohl-der-plan-ihn-fuehrt.md` es für Platzhalter verlangt. Tot ist nichts: die Proben fassen jedes öffentliche Stück an.
  - **Der ungesicherte Stand ist eine Marke und kein Vergleich mit dem Dateiinhalt.** Die Frage hängt an der Anzeige aus dem zweiten Abnahmekriterium von C4 und wird damit bei jedem Tastendruck gestellt; ein Vergleich hätte bei einer Datei an der 16-MB-Grenze je Tastendruck 16 MB zu lesen. Der angenommene Preis steht im Modulkopf: wer eine Änderung tippt und zurücknimmt, bekommt an den drei Anlässen trotzdem die Nachfrage. Der Zustandsübergang des Specs lautet "tippen oder ersetzen" und nicht "der Stand weicht ab".
...
- **Umsetzung am 260809-1738:** Die vier Abnahmekommandos laufen durch. Geschlossen sind damit `issues/260809-1640_*_der-fokus-kennt-den-editor-nicht-obwohl-der-abgriff-ihn-seit-s4-durchlaesst.md` und `issues/260809-1648_*_die-sprungmarke-geht-ohne-fokuspruefung-in-das-aktive-dateifenster.md`. Vier Vermerke:
...
#### 23. [DONE] **Der Übergang aus der Vorschau in den Editor**
...
- **Umsetzung am 260810-0146**, zusammen mit S30 und S31; alle drei schreiben in `anwendung.rs`. Gebaut sind der Zweig `Kommando::EditorAusVorschau` in `kommando_ausfuehren` und `Anwendungsdelegierter::editor_aus_vorschau`, zusammen dreizehn Zeilen Programmtext. Die vier Abnahmekommandos laufen durch. Drei Vermerke:
  - **Die Zusage aus dem Vorfindungsvermerk hält, und sie ist gemessen und nicht angenommen.** Der Übergang erbt die Nachfrage aus C4 ohne eine Zeile dafür. Der Beleg ist eine Kette mit genau einem Glied je Stufe: `Editormodell::oeffnen` hat in der ganzen Kiste **einen** Aufrufer außerhalb der Proben, nämlich `Editorbereich::datei_oeffnen` (`grep -rn 'borrow_mut().oeffnen' crates/krk-ui/src` findet neben ihm allein die Tabliste); `datei_oeffnen` hat in `anwendung.rs` drei Aufrufstellen, F4 aus S22, diesen Übergang und die Wiederherstellung aus S30. Alle drei laufen damit durch dieselbe Prüfung auf dem Arbeitsfaden und durch dieselbe Rückhaltung; `Ladeausgang::Zurueckgehalten` erreicht `editorausgang_behandeln` und dort `nachfrage_zeigen(Anlass::AndereDatei)`, gleichgültig wer geöffnet hat. Eine zweite Abfrage steht nirgends.
  - **Der Pfad wird abgeschrieben, bevor irgendetwas an der Sichtbarkeit geschieht.** `Vorschaufenster::angezeigter_pfad` liefert einen `PathBuf` und keine Ausleihe; das Verdrängen der Vorschau geschieht erst später in `fokus_holen(Fokus::Editor)`, und zu dem Zeitpunkt hält der Editor die Datei schon. Damit ist die Festlegung des Nutzers vom 260807-2139 nicht bloß eingehalten, sondern von der Bauart her nicht zu verletzen.
  - **`grep -c 'datei::oeffnen' crates/krk-ui/src/appkit/anwendung.rs` liefert 3, und alle drei sind Doc-Kommentare.** Die Prüffunktion wird von `anwendung.rs` gar nicht gerufen, sondern allein genannt; gerufen wird sie im Modell. Das Abnahmekriterium oben zielt auf den Funktionsnamen des Öffnens, und der ist `datei_oeffnen` mit seinen drei Aufrufstellen.
...
- **Umsetzung am 260809-2322, zusammen mit S26.** Der Grund für die Zusammenlegung steht am Defekt `issues/260809-2148_*_s25-sichern-schriebe-den-plattenstand-weil-die-rueckschreibung-erst-s26-baut.md`: beide Schritte fassen dieselben zwei Dateien an, und S25 dazwischen hätte den Plattenstand zurückgeschrieben und eine gelungene Sicherung gemeldet. Vier Vermerke:
...
- **Umsetzung am 260809-2322, zusammen mit S24.** Vier Vermerke:
  ...
  - **Der ungemessene Preis der Bauart steht als eigener Datensatz da:** `issues/260809-2322_*_der-ganze-stand-geht-je-tastendruck-durch-bearbeiten.md`. Bei einer Datei nahe 16 MB kostet jeder Anschlag eine vollständige Kopie und einen Durchlauf. Der Ausweg ist benannt und nicht zu suchen; gebaut wird er nicht auf Verdacht.
...
#### 28. [DONE] **Zwei der drei Anlässe: schließen, andere Datei**

- Ausführender: `coder`
- Dateien: `crates/krk-ui/src/appkit/anwendung.rs` (erweitert: die Aufrufstellen, `offenes_blatt`)
- Änderungen: Aufrufstellen, je mit eigener Fortsetzungsschließung, alle über das eine Blatt aus S27:
  **Editor schließen** (`opt+cmd+e`): die Fortsetzung blendet den Editor aus und gibt seine Datei frei.
  **Eine andere Datei aufnehmen** (F4, der Übergang aus der Vorschau oder der Sprung auf eine Textmarke aus C6): die Fortsetzung nimmt die neue Datei auf. Die Prüfung der neuen Datei steht **vor** der Nachfrage, wie S22 es vermerkt hat.
  **Ein dritter Anlass stand hier bis zum 260810 und ist gefallen:** das Einblenden der Vorschau. Der Nachtrag unten sagt, warum, und was im Code an seine Stelle getreten ist, nämlich nichts.
  Jede Schließung hält den Anwendungsdelegierten **schwach** über `objc2::rc::Weak::from_retained`, wie alle elf bestehenden Blattaufrufer; der Ring Delegierter → Blatt → Rückruf → Delegierter schlösse sich sonst. Jede setzt den `Blattgriff` nach `ivars.offenes_blatt` und leert ihn im Rückruf als erstes.
...
- Abhängigkeiten: S27
- Abnahmekriterium: `cargo build --workspace` beendet mit 0. Der Diff zeigt die Aufrufstellen, jede mit `Weak::from_retained` und mit `offenes_blatt`. Kein Feld hält einen Anlass über den Rückruf hinaus. **`Nutzerarbeit`:** an einer geänderten Datei erscheint bei beiden Anlässen dieses Schrittes dieselbe Nachfrage, und "Abbrechen" lässt den Editor offen und die Änderungen stehen.
  **Die Zählung `grep -c 'ungesichert::zeigen'` stand hier mit vier und ist gestrichen.** Sie war der Versuch, "je Anlass eine Aufrufstelle" messbar zu machen; gebaut ist eine Aufrufstelle für alle, und der erste Vermerk unten rechnet vor, warum das strenger hält als die Zahl. Eine Zahl, die den gebauten Stand für falsch erklärt, ist kein Abnahmekriterium.
- **Umsetzung am 260810-0021, zusammen mit S27 und S29.** Die drei fassen dieselben Dateien an, S28 hängt an S27 und S29 an S28; einzeln gefahren hätten sie sich überschrieben. Die vier Abnahmekommandos laufen durch. **Zwei Zusagen dieses Schrittes sind eingelöst, das gezählte Abnahmekriterium ist es nicht**, und der Grund steht im ersten Vermerk. Fünf Vermerke:
  - **Der Bau hat eine Aufrufstelle des Blattes und nicht je Anlass eine, und das ist eine bewusste Abweichung vom Schnitt oben.** Die Anlässe teilen sich mehr als das Blatt: den schwachen Griff auf den Delegierten, das Setzen und Leeren von `offenes_blatt`, die dreiwertige Antwort und, das Entscheidende, die Behandlung des gescheiterten Sicherns aus dem neunten Abnahmekriterium von C4. Mehrfach aufgeschrieben wären das ebenso viele Stellen, an denen diese Zusage zu halten oder zu brechen ist. Gebaut ist deshalb `Anwendungsdelegierter::nachfrage_zeigen` als die eine Aufrufstelle, dazu die Aufzählung `Anlass` mit ihren zwei vollständigen Fallunterscheidungen `anlass_ausfuehren` und `anlass_unterbleibt`. **Die Absicht des Abnahmekriteriums hält damit strenger als seine Zahl:** kein Feld hält einen Anlass über den Rückruf hinaus, denn der Wert wird in die Schließung hineinkopiert und fällt mit ihr, und ein weiterer Anlass hält an beiden Fallunterscheidungen den Bau an, statt still den Zweig des Nachbarn zu bekommen. `grep -c 'ungesichert::zeigen'` liefert deshalb **1**.
  ...
  - **Die Regel im Modell hat einen Gewinn, den drei Abfragen bei drei Aufrufern nicht hätten:** F4, der Übergang aus der Vorschau (S23) und der Sprung auf eine Textmarke aus C6 erben sie, ohne sie zu kennen. Das ist zugleich die Zusage aus dem fünften Abnahmekriterium von C4 und die aus dem neunten von C2 („Beide Einstiege legen dieselbe Prüfung an"). S23 hat dafür nichts zu tun. **Der Wortlaut jenes fünften Kriteriums nannte bis zum 260810-0714 zwei Einstiege, wo drei Wege eine Datei aufnehmen**; der Bau war nie enger als die drei, weil die Regel im Modell sitzt. Der Spec ist nachgezogen (`issues/260810-0422_*_das-fuenfte-abnahmekriterium-von-c4-nennt-zwei-einstiege-drei-wege-nehmen-eine-datei-auf.md`).
  - **Der Anlass „Vorschau einblenden" trug drei Befehle, und alle drei sind ihn wieder los.** Gebaut war er zunächst für `f3`, `cmd+y` und `shift+cmd+y`, letzteres, weil ein Fokusbefehl seit dem Nutzerentscheid vom 260807 einen ausgeblendeten Bereich hervorholt und ein hervorgeholtes Vorschaufenster den Editor genauso verdrängt. Mit dem Nutzerentscheid vom 260810-0250 ist der Anlass gefallen; die drei Befehle blenden die Vorschau ohne Nachfrage ein. Der nächste Vermerk sagt, was im Code dabei entfernt wurde.
  ...
  - **Was „Verwerfen" beim dritten Anlass verwarf, war nichts, und daraus wurde die Frage, die den Anlass gefällt hat.** Ein Ausblenden des Editors fällt seinen Stand nicht, wie der Defekt vom 260809-2029 an anderer Stelle belegt hat, während der Spec unter C4 annahm, das Verdrängen durch die Vorschau verliere „denselben Stand wie das Schließen". Die Fortsetzung tat hier deshalb genau das, was der Schritt oben vorschrieb: sie blendete die Vorschau ein, und der Stand blieb. Der Datensatz `decisions/260810-0021_*_was-verwirft-verwerfen-wenn-die-vorschau-den-editor-nur-verdraengt.md` führt die Frage; sie hat keinen Schritt aufgehalten.
  - **Nachtrag vom 260810: der dritte Anlass ist gefallen, und dieser Schritt ist damit auf zwei Anlässe zurückgenommen.** Der Nutzer hat die Frage aus dem letzten Vermerk am 260810-0250 mit der zweiten Möglichkeit beantwortet: die eingeblendete Vorschau fragt nicht mehr, weil sie nichts verliert. Im Code sind die beiden Werte `Anlass::VorschauUmschalten` und `Anlass::VorschauFokus` gefallen, dazu die Vorbedingung `vorschau_verdraengt_den_editor` und die beiden Hüllen `vorschau_umschalten` und `fokus_vorschau_holen`; `f3`, `cmd+y` und `shift+cmd+y` blenden die Vorschau ohne Nachfrage ein. Im Spec ist das sechste Abnahmekriterium von C4 gestrichen, wodurch sich die Nummern der folgenden um eins verschoben haben; die Zitate in diesem Plan sind mitgezogen. Wer diesen Schritt später liest, baut die Vorschau-Anlässe **nicht** nach. Der Datensatz steht seit dem 260810-0402 auf umgesetzt.

#### 29. [DONE] **Der dritte Anlass: das Beenden**
...
- **Umsetzung am 260810-0021, zusammen mit S27 und S28.** `applicationShouldTerminate:` steht im Protokollblock, wo bis hierher drei Methoden standen; die Entscheidung selbst trägt `Anwendungsdelegierter::beenden_erlauben`, die Antwort `beenden_beantworten`. Das Kennzeichen `beenden_ohne_nachfrage` hat genau einen Schreiber (`ohne_tastenabgriff_beenden`) und genau einen Leser (`beenden_erlauben`). Vier Vermerke:
  ...
  - **`applicationWillTerminate:` ist unverändert geblieben, und das ist die Zusage.** Es läuft nach der Zustimmung und nicht vor ihr; ein abgebrochenes Beenden erreicht es nie, und die getaktete Sitzungssicherung trägt den ungesicherten Stand weiterhin nicht mit (sechstes Abnahmekriterium von C4, Datensatz `260807-2147_*_wie-greift-die-nachfrage-bei-der-sitzungssicherung.md`).

#### 30. [DONE] **Die Sitzung merkt sich die geöffnete Datei**
...
- **Umsetzung am 260810-0146**, zusammen mit S23 und S31. Die drei Proben aus dem Abnahmekriterium stehen in `crates/krk-core/tests/ablage.rs`; `beispielsitzung` trägt das neue Feld, damit die bestehende Rundreise es mitprüft. Fünf Vermerke:
  - **Die Dateizeile oben ist unvollständig: `crates/krk-ui/src/fenstermodell.rs` ist mitgezogen, und es geht nicht ohne.** `Sitzung` ist eine Struktur ohne Auffangzweig, und `Fenstermodell::sitzung` baut sie als Literal; ein neues Feld hält dort den Bau an. Genommen ist der Weg, den die Funktion für die Tabs schon geht und in ihrem Doc-Kommentar begründet: der Wert kommt **von außen** dazu, als zweiter Parameter. Der Grund ist derselbe — die Datei wohnt im `Editormodell`, und das Fenstermodell kennt vom Editor allein Breite und Sichtbarkeit. Sie dort aus einer zweiten Quelle zu erfragen wären zwei Orte für eine Angabe.
  - **Das Feld steht vor den drei Tabellen, und das ist keine Geschmacksfrage.** TOML verlangt, dass die Werte einer Tabelle vor ihren Untertabellen stehen; eine Zeile hinter `[breiten]` ließe `toml::to_string` scheitern und damit jede Sitzungssicherung. Der Vermerk steht am Feld, weil die Feldreihenfolge sonst beim nächsten Umsortieren fällt. C7 der Runde 1 bleibt eingehalten: `session.toml` ist von Hand lesbar, und ein nicht gesetzter Pfad steht gar nicht da.
  - **Die Wiederherstellung geht denselben einen Weg wie F4 und braucht trotzdem ein unterscheidendes Kennzeichen.** `editor_wiederherstellen` ruft `Editorbereich::datei_oeffnen` wie beide Einstiege, also mit derselben Prüfung. Zwei der vier Ausgänge verlangen danach aber etwas anderes als bei einem Befehl, und das ist keine Bequemlichkeit: **`Geoeffnet` darf den Fokus nicht holen**, weil er beim Start nach `fokus::BEIM_START` in das aktive Dateifenster gehört, und **`Abgewiesen` ist die Antwort auf keinen Tastendruck** und gehört deshalb auf Rang 3 statt auf Rang 1, wo der erste Tastendruck sie wegräumte. Gebaut ist dafür `AnwendungsIvars::editor_aus_sitzung` nach dem Zuschnitt, den S29 für `beenden_ohne_nachfrage` genommen hat: ein Feld, ein Schreiber, ein Leser, und der Leser verbraucht es beim ersten Ausgang. Die beiden übrigen Ausgänge sind beim Start nicht erreichbar — der Editor hält nichts, also weder `SchonOffen` noch ungesicherten Stand.
  - **Eine abgewiesene Datei blendet den Editor aus und lässt ihn nicht leer stehen.** Hatte die Sitzung ihn sichtbar, nähme er den Dateifenstern sonst Platz für nichts; das ist dieselbe Begründung, aus der `Sichtbarkeit::default` ihn ausgeblendet ausliefert. Gerufen wird dafür `editor_ausblenden`, die Stelle, die S28 schon gebaut hat, und keine zweite daneben.
  - **Die Sichtbarkeit des Editors wird bei der Wiederherstellung nicht angefasst.** Sie kommt aus der Sitzung und steht seit `Fenstermodell::aus_sitzung`. Ein ausgeblendeter Editor mit gehaltener Datei ist ein gültiger Zustand — der Fokusbefehl aus C1 holt ihn genau so hervor —, und ihn hier einzublenden hieße, die gemerkte Sichtbarkeit zu übergehen.

#### 31. [DONE] **Die von außen geänderte Datei**
...
- Änderungen: **Die bestehende Dateisystembeobachtung wird erweitert; ein zweiter Strom daneben entsteht nicht.** `auffrischung::sichtbare_ordner` liefert heute höchstens zwei Ordner, die der sichtbaren Tabs der sichtbaren Dateifenster; es liefert künftig höchstens drei, mit dem Ordner der vom Editor gehaltenen Datei als drittem. `dateisystemwache_nachziehen` wird zusätzlich beim Öffnen und Schließen im Editor angestoßen, so wie es heute bei jeder Navigation und beim Ein- und Ausblenden des zweiten Dateifensters läuft; ein `FSEventStream` ändert seine Pfadliste nach dem Anlegen nicht.
  ...
  **Zwei Momente, ein Stempel.** Der Vergleich läuft beim gemeldeten Ordnerereignis und noch einmal unmittelbar vor jedem Sichern (S25). Das ist kein zweiter Mechanismus, sondern dieselbe Frage an zwei Stellen; der erste Moment meldet, der zweite verhindert das Überschreiben. Das achte Abnahmekriterium von C4 verlangt beides.
...
- **Umsetzung am 260810-0146**, zusammen mit S23 und S30. `fsevents.rs` ist nicht angefasst worden: der eine `FSEventStreamCreate(`-Aufruf steht unverändert an einer Stelle, und `kFSEventStreamCreateFlagFileEvents` bleibt ungesetzt. Die vier Abnahmekommandos laufen durch. Sechs Vermerke:
  - **Die Dateizeile oben ist unvollständig: `crates/krk-ui/src/appkit/editor.rs` ist mitgezogen.** Der Editorbereich hält das Modell, also braucht der Anwendungsdelegierte einen Weg dorthin; dazugekommen ist genau eine Zugriffsfunktion, `Editorbereich::fremdaenderung_melden`, die die Frage hineinreicht und den Satz herausreicht.
  - **Die Sicht auf den Editor kommt über die bestehende Schnittstelle und nicht über einen Zugriff aus AppKit.** `Dateifenstersicht` hat eine Methode dazubekommen, `editordatei`. Damit bleibt `auffrischung.rs` ohne eine Zeile AppKit, und beide neuen Entscheidungen sind ohne Fenster prüfbar — genau der Grund, aus dem diese Schnittstelle überhaupt existiert. Gefragt wird nach der **Datei** und nicht nach ihrem Ordner: aus dem Pfad folgt der Ordner, aus dem Ordner folgt die Datei nicht, und beide Fragen des Moduls hängen an ihr.
  - **Zwei Fragen, zwei Funktionen, eine Quelle.** `sichtbare_ordner` liefert jetzt höchstens drei Ordner statt zwei, und `betrifft_editordatei` beantwortet, ob ein gemeldeter Stapel den Editor angeht. Beide gehen durch dieselbe private `editorordner`, die aus der gehaltenen Datei ihren Ordner macht; zwei Ableitungen nebeneinander wären die erste Gelegenheit für zwei verschiedene Antworten.
  - **Der Stempelvergleich wird einmal je Stapel gestellt und nicht einmal je Pfad**, und er steht **außerhalb** des Auffrischungsaufschubs. Beides ist Absicht. Die Frage lautet „geht mich dieser Rückruf etwas an", und sie je Pfad zu stellen hieße, denselben `stat(2)` in einem Stapel von tausend Meldungen tausendmal zu machen. Der Aufschub beantwortet eine andere Frage, nämlich ob eine **Dateiliste** neu zu lesen ist; ein Stapel-Umbenennen, das die Datei des Editors erwischt, soll gemeldet werden, auch während die Liste stehen bleibt.
  - **Gefragt wird über `fremd_geaendert` und nicht über den Stempel selbst — und damit hat S31 die Ankündigung an `Editormodell::stempel` nicht eingelöst.** S25 hat die Regel gezogen: eine Frage, eine Stelle. Der Vergleich steht deshalb weiterhin im Modell, und der Stempel geht nicht nach außen. Folge: `Editormodell::stempel` bleibt ohne Aufrufer, und der Defekt `issues/260810-0212_*_drei-stuecke-des-editormodells-haben-keinen-aufrufer-und-der-plan-nennt-keinen.md` trägt seit dem 260810 einen Nachtrag, der aus drei Stücken vier macht.
  - **Gemeldet wird der Übergang und nicht der Zustand, und die Meldung steht auf Rang 3.** `Editormodell::fremd_gemeldet` merkt, dass derselbe Befund schon einen Satz hatte; ohne das Feld stünde bei einem fremden Programm, das im Sekundentakt schreibt, derselbe Satz bei jeder Meldung neu in der Zeile und verdrängte alles andere. Die Marke löscht sich selbst, sobald der Vergleich wieder aufgeht, also an jeder der drei Stellen, die den Stempel setzen — keine von ihnen trägt dafür eine Zeile. **Der Preis steht am Doc-Kommentar und in einer Probe:** eine zweite fremde Änderung meldet sich nicht, solange der Stempel nicht neu gesetzt ist. Die Aussage bleibt wahr, und das Sichern hält das Überschreiben ohnehin zurück. Der Rang ist der der Fenstermeldung und nicht der der Befehlsantwort, weil niemand die Änderung angefordert hat; auf Rang 1 räumte der nächste Tastendruck sie weg.
...
- Änderungen: **Dieser Schritt entscheidet die Kiste an vier Kriterien und trägt die Begründung ein.** Der Nutzer hat am 260808-0017 den Weg gewählt, eine fertige Kiste einzubinden, und nicht die Kiste; der Spec überlässt die Wahl dem Planner, und der Planner überlässt zwei ihrer vier Kriterien der Messung, weil sie am Papier nicht zu entscheiden sind.
  **Die vier Kriterien, in dieser Reihenfolge:**
  ...
  4. Der Preis beider Kisten am Programm liegt unter 10 MB. **Zu messen** an einem eigenständigen Prüfprogramm, das die Sprachdefinitionen und die Farbtafeln wirklich lädt, und **nicht** am Bündel: in S32 ruft kein Modul der Anwendung die Kisten auf, der Übersetzer wirft den gesamten eingebetteten Bestand wieder weg, und `target/KRK.app` wächst deshalb um eine Zahl, die nichts über den Preis sagt. Das Wachstum des Bündels ist in S33 abzunehmen, wo der Bestand zum ersten Mal geladen wird.
  **Berichtigt am 260810.** Dieses Kriterium verlangte bis dahin die Größe von `target/KRK.app` vor und nach der Einbindung, gemessen in S32. Gemessen am 260808-0948 wuchs `target/release/krk` durch beide Kisten um 128 Byte, weil kein Aufrufer sie nennt; die Zahl ist echt und sagt nichts. Der Befund steht als `issues/260808-0948_*_das-vierte-kriterium-von-s32-ist-in-s32-nicht-messbar.md`. Der Zahlenrahmen von 10 MB bleibt unverändert; berichtigt ist allein, wo und woran er geprüft wird.
  **Der Vorschlag des Planners ist `syntect` in der Fassung 5.3.0**, mit `default-features = false` und den Merkmalen `parsing`, `default-syntaxes`, `default-themes`, `dump-load` und `regex-fancy`. `regex-fancy` tauscht die C-Bibliothek Oniguruma gegen `fancy-regex` in reinem Rust und erfüllt damit Kriterium 2. Abgeschaltet bleiben `html`, `plist-load`, `yaml-load` und `metadata`, weil KRK die Einfärbung selbst in Textmerkmale umsetzt und keine HTML-Ausgabe braucht. **`dump-create` stand hier bis zum 260810 als fünftes und ist gestrichen: `parsing` zieht es mit, es lässt sich nicht abschalten.** Die Begründung steht bei `### Frage 2`, der Befund als `issues/260808-0949_*_s32-nennt-dump-create-unter-den-abgeschalteten-merkmalen-es-laesst-sich-nicht-abschalten.md`.
...
- Abhängigkeiten: keine
- Abnahmekriterium: `cargo build --workspace` beendet mit 0. Ein kurzer Prüflauf zählt die geladenen Sprachdefinitionen auf und findet Rust, TOML, Markdown und Shell; er zählt die geladenen Farbtafeln auf und findet mindestens eine helle und eine dunkle. `grep -rEln '^[[:space:]]*#!?\[allow\(unsafe_code\)\]' crates/krk-core/src crates/krk-ui/src` nennt weiterhin genau zwei Dateien. Derselbe Prüflauf wiegt sich selbst mit und ohne die beiden Kisten und belegt damit Kriterium 4; die beiden Zahlen stehen im Sitzungsbericht. `Cargo.toml` trägt für jede neue Kiste eine Begründung mit den drei geforderten Angaben.
- **Messung am 260808-0948, zu Kriterium 4:** das Prüfprogramm, das `two_face::syntax::extra_newlines()` und `syntect::highlighting::ThemeSet::load_defaults()` wirklich lädt, wiegt 1.591.544 Byte; dasselbe Programm ohne die beiden Kisten wiegt 418.968 Byte. Der Preis beider Kisten beträgt damit rund **1,12 MiB**, gut ein Zehntel der zugestandenen 10 MB. Der Prüfcode ist Wegwerfcode und liegt nicht im Baum. Als Ausgangspunkt für die Bündelmessung in S33 festgehalten: `target/KRK.app` trug am 260808-0948 **3.502.046 Byte** über vier Dateien.
...
- Änderungen: Ein `NSTextStorage`, zwei Darstellungen, und die Merkmale an zwei Orten. Die Begründung steht in `### Frage 7` und gehört in den Modulkopf: **die Fallunterscheidung heißt "wirkt auf die Auslegung oder nicht".** Farbe und Unterstreichung gehen als vorübergehende Merkmale über `NSLayoutManager::setTemporaryAttributes:forCharacterRange:` in den Layoutverwalter; Schriftgröße, Schriftschnitt, feste Schrift und Einzug gehen über `addAttributes:range:` in den `NSTextStorage`, weil der Layoutverwalter sie als vorübergehendes Merkmal nicht beachtet (`NSLayoutManager.h:351`). Daraus folgt dreierlei ohne weitere Vorkehrung: die Auszeichnung kann beim Sichern nicht in die Datei geraten, weil `Editormodell::stand` aus den Zeichen der Fläche kommt und kein Merkmal liest; das Umschalten ist ein Zurücksetzen und Neusetzen statt eines Umbaus; und es gibt keine zweite Textkopie, die auseinanderlaufen könnte. Das zehnte Abnahmekriterium von C3 ist damit eine Eigenschaft der Bauart und keine Zusage der Sorgfalt, und das elfte, die Schreibmarke stehe nach dem Umschalten an derselben Stelle, fällt ohne eigenen Bau an, weil die Zeichen unberührt bleiben.
...
- Abnahmekriterium: `cargo build --workspace` beendet mit 0. Der Diff zeigt genau einen `NSTextStorage` und keinen zweiten Textbestand; `grep -c 'setString' crates/krk-ui/src/appkit/editor.rs` liefert genau die Stelle, die eine neue Datei einsetzt, und keine im Umschaltweg. Der Diff zeigt beide Merkmalswege und die Grenze zwischen ihnen: Farbe und Unterstreichung über `setTemporaryAttributes` und `addTemporaryAttribute` im Layoutverwalter, Schriftgröße, Schriftschnitt, feste Schrift und Einzug über `addAttributes_range` im `NSTextStorage`. Kein layoutwirksames Merkmal steht auf dem vorübergehenden Weg, denn dort täte es nichts (`NSLayoutManager.h:351`). Dass die Auszeichnung beim Sichern nicht mitgeht, zeigt der Sicherungsweg und nicht der Ort der Merkmale: er liest `NSTextView::string` (`editor.rs:1099`) und damit die Zeichen der Fläche, und kein Merkmal wird auf ihm gelesen. **Dazu, seit dem Nachtrag vom 260810, das vierte Kriterium aus S32:** `cargo xtask bundle` läuft durch, und `target/KRK.app` wächst gegenüber den 3.502.046 Byte, die S32 vor der Einbindung gemessen hat, um weniger als 10 MB. Der Schritt trägt es und nicht S32, weil der eingebettete Bestand hier zum ersten Mal geladen wird und das Wachstum erst dadurch entsteht; die Begründung steht bei Kriterium 4 von S32. **`Nutzerarbeit`** für sechs der zwölf Kriterien von C3: dass der Umschalter bei jedem Dateityp etwas Sichtbares tut, dass Markdown Überschriften, Listen und Links zeigt, dass Code seine vier Wortarten gegeneinander absetzt, dass eine unbekannte Sprache auf Text zurückfällt, dass eine in der Formatansicht getippte Änderung nach dem Umschalten in der Rohansicht steht, und dass die Schreibmarke ihre Stelle behält.
- **Messung am 260810-0918, zum nachgetragenen Kriterium:** `cargo xtask bundle` läuft durch und signiert; `target/KRK.app` trägt **7.191.902 Byte** über vier Dateien, davon 7.180.576 im Programm selbst. Gegenüber den 3.502.046 Byte vor S32 sind das **3.689.856 Byte Zuwachs**, rund 3,52 MiB und damit gut ein Drittel der zugestandenen 10 MB. Die Zahl ist eine **obere Schranke** für den Preis der beiden Kisten und keine Zuordnung: gemessen ist der Stand nach allen 48 Schritten gegen den Stand vor S32, der Zuwachs enthält also auch den Code von S33 bis S48. Für das Kriterium genügt die Schranke, denn sie liegt unter der Grenze.
...
- Änderungen: Ein Blatt mit Textfeld fragt nach einer Zeichenfolge; `cmd+g` geht zum nächsten Treffer, `ctrl+cmd+g` zum vorigen. Die Suche sagt in der Statuszeile, wie viele Treffer die Datei enthält und der wievielte gerade angesteuert ist. Ohne Treffer meldet sie das und lässt die Schreibmarke stehen.
  ...
  **Sie bezieht sich auf den Text der Datei und nicht auf seine Darstellung**, und wirkt deshalb in beiden Ansichten aus C3 gleich. Auch das fällt von selbst an, weil S33 allein Merkmale setzt und keine Zeichen: die Suche läuft über den gehaltenen Stand, und der ist eine gewöhnliche Zeichenkette.
...
#### 38. [DONE] **Eine Textmarke anlegen**
...
- **Am 260810-0036 zur Hälfte ausgeführt, zusammen mit S40. Der Schritt bleibt offen.** Gebaut ist die Kette vom Befehl bis in `bookmarks.toml`: `anwendung.rs::lesezeichen_anlegen_ausfuehren`, `Leistenquelle::lesezeichen_anlegen` und `Leistenmodell::anlegen` nehmen seither das fertige `Ziel` entgegen und fragen an keiner Stelle nach der Sorte. Damit ist der zweite der beiden Platzhalter aus `65c8efa` abgelöst; der erste (`Leistenmodell::gewaehlt`) gehört S39 und steht.
  **Was fehlt, ist der Editor-Zweig des Befehls**, und er hängt an einer Auskunft, die es in `appkit/editor.rs` nicht gibt: Nummer und Inhalt der Zeile, in der die Schreibmarke steht. Sie im Aufrufer zu rechnen hieße, die Umrechnung von AppKits UTF-16-Einheiten in Byteversätze ein zweites Mal aufzuschreiben; sie steht schon als `anfaenge_in_utf16` in `appkit/nummernspalte.rs`. Der Zuschnitt der fehlenden Funktion, die offene Frage nach dem Ende der Auswahl und die zwei verbleibenden Schritte stehen in `issues/260810-0036_*_dem-editor-fehlt-die-auskunft-ueber-die-zeile-der-schreibmarke.md`.
- **Die zweite Hälfte ausgeführt am 260810-0204, zusammen mit S39.** Gebaut wurde der Editor-Zweig in `appkit/anwendung.rs`: `lesezeichen_anlegen` fragt jetzt `Anwendungsdelegierter::anlegeziel`, und das ist die eine Stelle, an der der Fokus die Sorte wählt. Zwei Abweichungen von der Schrittbeschreibung:
  - **Die Fallunterscheidung steht in einer eigenen Funktion und nicht im Rumpf des Befehls.** `anlegeziel` liefert das fertige `Ziel` und den Namensvorschlag als ein Paar, weil beide von der Sorte abhängen und sonst zwei Verzweigungen über dieselbe Frage nebeneinander stünden. Der Befehl selbst zeigt danach für beide Sorten dasselbe Blatt und ruft dieselbe Ausführung.
  - **Der Fokus im Editor ohne gehaltene Datei meldet und legt nichts an.** Auf den Ordner des aktiven Dateifensters auszuweichen wäre der stillschweigend andere Befehl: der Nutzer bekäme eine Ordnermarke, wo er eine Textmarke verlangt hat. Gemeldet wird derselbe Satz, den `editorblatt_moeglich` seit S25 für diesen Fall führt.
  - Der Namensvorschlag ist `dateiname:zeile`, der offen gelassene Vorschlag aus `issues/260810-0036`.

#### 39. [DONE] **Der Sprung auf eine Textmarke**
...
- **Ausgeführt am 260810-0204, zusammen mit der zweiten Hälfte von S38.** `leistenmodell.rs` (`Auswahl` trägt das `Ziel` statt eines Ordners, `Auswahl::pfad`, `gewaehlt` ohne Platzhalter), `appkit/editor.rs` (`Editorbereich::marke_anspringen`, `Editormeldung::MarkenstelleGeaendert` trägt die Zeilenlage), `appkit/anwendung.rs` (`leistenauswahl_ausfuehren` verzweigt nach der Sorte, `textmarke_anspringen`, `textmarke_ausfuehren`, das Feld `vorgemerkte_marke`). An `appkit/leiste.rs` war **nichts** zu ändern: sie reicht die `Auswahl` durch und liest allein ihren Namen. Vier Abweichungen und Entscheidungen:
  - **`Auswahl` trägt `krk_core::ablage::Ziel` und keinen eigenen Sortenwert.** Ein zweiter Aufzählungstyp in `krk-ui` wäre die zweite Wahrheit darüber, was eine Zeile öffnet, und liefe von dem weg, was in `bookmarks.toml` steht. Ein Gerät und ein Standardort tragen `Ziel::Ordner`, denn genau das öffnen sie — dieselbe Regel, die S40 dem Sinnbild gegeben hat.
  - **Der Sprung geschieht beim Ladeausgang und nicht beim Befehl.** `datei_oeffnen` kehrt seit S24 zurück, ohne gelesen zu haben, und wohin die Schreibmarke gehört, entscheidet sich am Text. Die gemerkte Stelle wartet deshalb in einem Feld des Anwendungsdelegierten, das `editorausgang_behandeln` beim ersten Ausgang herausnimmt. Sie überlebt genau einen Umweg, nämlich die Rückhaltung aus C4: `Ladeausgang::Zurueckgehalten` legt sie zurück, und bricht der Nutzer die Nachfrage ab, fällt sie mit der zurückgehaltenen Datei.
  - **Der offene Punkt aus S21 ist entschieden, und `issues/260809-1631` ist geschlossen.** Es gibt keine Vorrangregel zwischen Fund und Zeilenlage und auch keinen dritten, zusammengesetzten Wert: die beiden Auskünfte sind nicht unabhängig. `marke::wiederfinden` liefert `Getroffen` und `Verschoben` nur für eine Nummer, deren Zeile es gibt, und eine von `Zeilenlage::Getroffen` verschiedene Lage kommt deshalb allein mit `Fund::NichtGefunden` vor. Der Fund entscheidet **ob** gemeldet wird, die Lage **wohin** die Schreibmarke gekommen ist; ein Satzanfang, drei Enden.
  - **Die drei Ausgänge des Abnahmekriteriums sind nur zum Teil in `krk-ui` prüfbar.** Die fehlende Datei ist es (zwei neue Proben in `leistenmodell.rs`), der Sprung selbst ebenfalls (vier in `appkit/editor.rs`, über die Werte und ihre Sätze). Die Abweisung nicht: sie hängt an einem laufenden Fenster mit Editorbereich, und die Prüfung dahinter deckt `krk-core` seit S10 ab. Sie steht deshalb unter `Nutzerarbeit` und nicht als Probe, die ein Fenster bauen müsste.
...
- **Ausgeführt am 260810-0011. Die Vermutung des Schrittes hat gehalten: kein Programmteil war zu ändern.** Vier Proben sind hinzugekommen, alle vier grün im ersten Lauf, und sie stehen in `crates/krk-ui/src/belegungsmodell.rs` statt in `crates/krk-core/tests/belegung.rs` — die Zusage gilt der **Ansicht**, und allein `Belegungsmodell::zuweisen` bildet eine Zeilennummer der Ansicht auf ihre Funktion ab. Eine Probe im Kern prüfte `Belegung::zuweisen` und ließe die Gliederung ungeprüft.
  ...
  **Ein Defekt ist gefiled und gehört `ontocoder`:** `issues/260810-0011_*_zwei-kommentarbloecke-der-belegungsdatei-behaupten-den-nachschlag-ueber-den-tastencode.md`. `resources/default-keymap.toml` begründet an zwei Stellen das Meiden von `y` und `z` mit dem Nachschlag über den Tastencode, den S2 abgelöst hat.

#### 42. [DONE] **Abgleich, Spec-Nachträge und Abnahme am laufenden Bündel**
...
- Abnahmekriterium: `make check` läuft durch, also `cargo build --workspace`, `cargo test --workspace`, `cargo clippy --workspace --all-targets` und `cargo fmt --all --check`, jeweils mit Rückgabewert 0. `cargo xtask bundle` baut und signiert. Alle 48 Schritte tragen `[DONE]`. **`Nutzerarbeit` für die eigentliche Abnahme:** der Nutzer geht die Abnahmekriterien der elf Fähigkeiten und die beiden aus `## Verhältnis zu den zehn Zeitzusagen` am laufenden Bündel durch, mit KRK im Vordergrund. Kein Agent kann das, und dieser Plan behauptet es nicht.
- **Ausgeführt am 260810-0714.** `make check` und `cargo xtask bundle` laufen beide mit Rückgabewert 0; signiert ist mit der Entwicklungsidentität aus dem Schlüsselbund, gefunden über die zweite der drei Stufen. Alle 48 Schritte tragen `[DONE]`. Sieben Vermerke:
  - **Aus vier Spec-Nachträgen sind drei geworden, und der vierte ist ersatzlos entfallen.** Der zweite hing an der Bedingung "falls Weg 1 aus S2 gewählt wurde". Der Nutzer hat am 260808-0155 den dritten Weg gewählt, und damit sind die beiden ersten Abnahmekriterien von C8 buchstäblich erfüllbar geworden, statt umgeschrieben werden zu müssen. C8 hat stattdessen drei Festlegungen bekommen, die die gemessene Ursache, die Wahl und ihre Folge festhalten.
  - **Ein fünfter Nachtrag ist dazugekommen, den dieser Schritt nicht vorsah, und er ist der wichtigste von allen.** C4 sagte nirgends zu, dass keine Automatik den getippten Text ändert. Die Formel "der gesicherte Stand ist der getippte" stand im Plan bei S16, im Modulkopf von `appkit/editor.rs` und in den Datensätzen, aber in keinem Abnahmekriterium; das Nächstliegende im Spec stand unter C2 und meint die abgewiesene Binärdatei. Sieben abgeschaltete Automatiken hatten damit kein Kriterium, an dem sie abzunehmen wären. C4 trägt die Zusage seit dem 260810-0714 als elftes Kriterium.
  - **Beide neuen Kriterien von C4 stehen hinten und nicht an ihrem sachlichen Platz.** Sechzehn Stellen im Programmtext zitieren ein Kriterium von C4 nach seiner Nummer, und ein Einschub in der Mitte hätte sie zum zweiten Mal an einem Tag verschoben. Angehängt bleibt jede bestehende Zahl richtig.
  - **Die Verweise auf Datensätze tragen in Spec und Plan jetzt durchweg eine Sternstelle statt eines festen Markers.** Im Spec waren vierzehn Verweise mit festem Marker eingetragen und acht davon veraltet; der Plan hatte die Regel im Kopf stehen und fünfzehn Verweise, die ihr nicht folgten. Ein Pfad mit veraltetem Marker findet in einer Suche nichts, und genau daran hängt die Abnahme, die Datensätze nachschlägt.
  - **Ein gezähltes Abnahmekriterium von S28 ist gestrichen statt nachgezogen.** Es verlangte vier Aufrufstellen des Blattes; gebaut ist eine, und der erste Vermerk bei S28 rechnet vor, warum das strenger hält. Eine Zahl, die den gebauten Stand für falsch erklärt, ist kein Abnahmekriterium.
  - **Von den vier Defekten dieses Durchgangs sind drei geschlossen und einer widerlegt.** `260810-0359` behauptete, C11 trage dreizehn Kriterien statt der elf, die die Erweiterungsnotiz nennt. Nachgezählt trägt C11 elf; der Zählweg des Defekts trug die Überschrift von C11 über die Abschnittsgrenze hinaus weiter und schlug ihr die beiden Kriterien aus `## Verhältnis zu den zehn Zeitzusagen` zu. Der Spec stand richtig und ist an dieser Stelle nicht geändert worden; die Falle steht jetzt dort benannt.
  - **Die zehn Zeitzusagen aus C8 der Runde 1 sind unberührt.** Keine der zehn Zahlen ist geändert, gelockert oder umgedeutet, und diese Runde setzt keine eigene. Das ist das zweite der beiden Kriterien, die an die Stelle einer Zeitzusage treten, und es ist an dieser Stelle abgenommen.
...
Die Runde hat eine Lehre hinterlassen, und sie steht in drei Defekten: fünf Schritte mussten Stellen außerhalb ihres Umfangs mitziehen, weil der Plan nach Sachthema schnitt statt nach Übersetzbarkeit (`issues/260808-0931_*_s13-laesst-sich-nicht-allein-uebersetzen-die-speicherstelle-des-editors-kommt-erst-in-s14.md`, `issues/260809-1640_*_der-fokus-kennt-den-editor-nicht-obwohl-der-abgriff-ihn-seit-s4-durchlaesst.md`, `issues/260808-1413_*_vier-platzhalter-nennen-ihren-abloesenden-schritt-nicht-obwohl-der-plan-ihn-fuehrt.md`). **Die sechs neuen Schritte sind deshalb nach Übersetzbarkeit geschnitten:** jeder von ihnen übersetzt für sich, jeder lässt `make check` grün, und keiner hinterlässt eine Zeile, die auf ihren Ablöser wartet.
...
| Die Kiste macht das Bündel unvertretbar groß oder zwingt `unsafe` in `krk-ui`. | Zwei der vier Kriterien von S32 messen genau das: das `unsafe` an `grep` über die `allow`-Ausnahmen, die Größe an einem eigenständigen Prüfprogramm, und das Wachstum von `target/KRK.app` seit der Berichtigung vom 260810 an S33, wo es entsteht. Fällt eines, ist die Kiste die falsche, und die Wahl wird im selben Schritt wiederholt. |
...
**Die Formatansicht setzt Merkmale und keine Zeichen** (S33). Damit gibt es keinen zweiten Textbestand, der auseinanderlaufen könnte, und das zehnte Abnahmekriterium von C3 ist eine Eigenschaft der Bauart statt einer Zusage der Sorgfalt. Die Merkmale liegen an zwei Orten, weil der Layoutverwalter allein die auslegungsneutralen beachtet; für diese Zusage ist der Ort gleichgültig, weil der Sicherungsweg die Zeichen liest und kein Merkmal.
...
- `decisions/260808-0140_*_die-y-tasten-liegen-auf-einer-deutschen-tastatur-unter-anderen-buchstaben.md` — die Voraussetzung von C8 ist am Code widerlegt, und die Wahl zwischen drei Wegen gehört dem Nutzer. Sie hält S1 nicht auf und S2 als einzigen Schritt.
- `decisions/260808-0140_*_was-heisst-gerendert-bei-markdown-wenn-zugleich-bearbeitet-wird.md` — zwei Abnahmekriterien von C3 lassen sich nicht beide vollständig einlösen, und die Auslegung des Planners ist die eine, die mit beiden zusammengeht. Sie hält keinen Schritt auf und bindet S33. **Am 260810-0822 überholt**: die Antwort gilt unverändert weiter, die Begründung nicht. Der ablösende Datensatz ist `decisions/260810-0822_*_wie-die-formatansicht-ihre-auszeichnung-setzt-und-warum-an-zwei-orten.md`.

**Ein dritter ist am 260810 mit der Umsetzung von S28 dazugekommen**, und er ist die einzige Frage, die diese drei Schritte aufgeworfen haben: `decisions/260810-0021_*_was-verwirft-verwerfen-wenn-die-vorschau-den-editor-nur-verdraengt.md`. Der dritte Anlass aus C4 ist aus einer Annahme über den Code entstanden, die der Code nicht trägt — ein verdrängter Editor verliert seinen Stand nicht —, und damit verwirft „Verwerfen" an dieser einen Stelle nichts. Der Datensatz hat keinen Schritt aufgehalten; gebaut war zunächst der Weg des Plans, weil er der einzige der drei ist, der nichts verlieren kann. **Beantwortet am 260810-0250 und am selben Tag in Code eingelöst:** der Nutzer hat die zweite Möglichkeit gewählt, der Anlass ist gefallen, und die eingeblendete Vorschau fragt nicht mehr. Der Datensatz steht auf umgesetzt.

**Ein vierter ist am 260810-0822 mit der Berichtigung von `### Frage 7` und S33 dazugekommen**: `decisions/260810-0822_*_wie-die-formatansicht-ihre-auszeichnung-setzt-und-warum-an-zwei-orten.md`. Er hält den gebauten Zustand fest und ist deshalb nicht offen, sondern unmittelbar umgesetzt. Er trägt die gemessene Eigenschaft von AppKit als Grundlage und löst den Datensatz von 260808-0140 ab, dessen Annahme über den Weg am SDK-Kopf widerlegt ist. Die Antwort des abgelösten gilt unverändert weiter.
...
- [x] **Beantwortet am 260808-0155.** Der Nutzer hat beide Datensätze entschieden: in der y-Frage den dritten Weg, gegen die Empfehlung des Planners, und bei der Markdown-Auslegung die Möglichkeit des Planners. Beide stehen auf umgesetzt.
- [x] **Angenommen und in S6 ausgeliefert.** Die dreizehn vorgeschlagenen Kombinationen aus `### Frage 11` stehen in `resources/default-keymap.toml` und sind über die Belegungsansicht umbelegbar. Der Spec legt allein F4 fest und überlässt den Rest dem Verfahren der Runde 1: der Planner schlägt vor, der Nutzer entscheidet.
...
- [x] **Beantwortet am 260810-0250: die erste Möglichkeit, drei Zustände auf einem Kanal.** Der Akzentrahmen (`decisions/260809-2043_*_bedeutet-der-akzentrahmen-kuenftig-den-fokus-oder-das-aktive-dateifenster.md`) war vor der Abnahme von C9 zu beantworten. S44 baut die Vorbelegung des Specs; die Kostentabelle für die beiden anderen Möglichkeiten steht in `### Frage 14`, und keine von ihnen ändert mehr als einen Funktionsrumpf und höchstens einen zusätzlichen Schritt.
- [ ] Der Schnitt der Fokusabfrage aus S43 ist eine Ableitung des Planners gegen den Vorschlag des Defekts `issues/260809-1738_*_der-rueckfall-in-fokus-antwortet-dateifenster-fuer-jede-unteransicht-eines-randbereichs.md`, der erst messen und dann schneiden wollte. Der Grund ist das vierte Abnahmekriterium von C9, das die richtige Antwort zusagt, und Befund 8, der zeigt, dass der einzige benannte Preis des Schnitts an dieser Stelle nicht anfällt. Wer die Reihenfolge lieber hält, verschiebt S43 hinter eine Messung am laufenden Bündel; S44 und S45 laufen dann mit dem heutigen Rückfall und erfüllen das vierte Abnahmekriterium nicht.
...
Geprüft am Code, nicht an den Markern. Grundlage: `make check` mit Rückgabewert 0 (721 bestandene Proben in fünfzehn Läufen, kein Fehlschlag, `clippy` ohne Meldung, `fmt` sauber), dazu eine Einzelprüfung jedes der 48 Schritte gegen sein kennzeichnendes Artefakt im Baum.

**Die 48 `[DONE]` halten am Code.** Jeder Schritt hat sein Artefakt an der angesagten Stelle. Die vier vollständigen Fallunterscheidungen tragen die versprochenen Werte: `Wirkungsbereich` sieben (`crates/krk-core/src/tasten/belegung.rs:171-235`), `Fokus` fünf (`crates/krk-ui/src/kommandos/fokus.rs:75-107`), `Bereich` fünf (`crates/krk-ui/src/fenstermodell.rs:62-87`), `Anlass` drei (`crates/krk-ui/src/appkit/anwendung.rs:243`). Der Ereignisabgriff stellt die Nämlichkeitsfrage vor der Klassenprüfung (`crates/krk-ui/src/appkit/ereignisse.rs:497-513`), und `isKindOfClass` steht unverändert dreimal. `target/KRK.app` ist gebaut und mit der Entwicklungsidentität signiert, geprüft mit `codesign -dv` am 260810-0805.

**Der Nachtrag an `CLAUDE.md` aus S42 ist eingelöst.** Die Datei nennt den Projektstand 260810-0714 (`CLAUDE.md:23`), die 48 Planschritte (`:48`) und trägt unter „Was man nicht sieht, wenn man es nicht weiß" sieben Einträge statt fünf, darunter den sechsten über die Nämlichkeit des Ersthelfers und den siebten über `makeFirstResponder:`.

#### Zwei Schritte tragen `[DONE]` über einem gebrochenen Abnahmekriterium

Das ist der einzige sachliche Befund dieses Abgleichs, und er ist der Grund, warum der Zustandsmarker dieser Datei auf offen bleibt.

**S6.** Das Abnahmekriterium verlangt, dass „keine Tastenliste die Taste `y` oder `z` neu belegt". `resources/default-keymap.toml:663` legt `text_rueckgaengig` auf `cmd+z` und `:672` `text_wiederholen` auf `shift+cmd+z`. Die Probe `crates/krk-core/tests/belegung.rs:1577` schränkt die Zusage nachträglich auf Kombinationen ein, die KRK selbst zustellt; beide neuen tragen `gehalten_von = "menue"` und fallen aus der Prüfung heraus. Offen geführt als `issues/260809-1527_*_der-plan-verbietet-y-und-z-und-legt-rueckgaengig-selbst-auf-cmd-z.md`, und die Auflösung ist eine Nutzerentscheidung.

**S33.** Das Abnahmekriterium verlangt, dass die Einfärbung „über `setTemporaryAttributes` und nicht über `addAttributes`" läuft. `crates/krk-ui/src/appkit/editor.rs:1876` schreibt die layoutwirksamen Markdown-Auszeichnungen über `addAttributes_range` in den `NSTextStorage`. Der Modulkopf von `hervorhebung.rs:20-27` begründet die Zweiteilung sachlich — Farben als vorübergehende Merkmale, Schriftgröße und Einzug im Textspeicher —, aber das Kriterium sagt sie nicht. Offen geführt als `issues/260810-0053_*_der-plan-legt-die-markdown-auszeichnung-in-voruebergehende-merkmale-und-die-tragen-sie-nicht.md`.

#### Warum der Marker deshalb auf offen bleibt

Die Konvention (`rules/fusion-workbench-conventions.md`, `### Planning files`) verlangt die Umbenennung auf geschlossen, sobald alle Schritte `[DONE]` tragen. Sie tragen es. Ein Abgleich prüft aber nicht Marker gegen Marker, sondern Marker gegen den Code, und zwei dieser 48 Marker stehen über einem Kriterium, das der Code nicht einlöst. Ein Plan, der sich geschlossen nennt, während zwei seiner Schritte einen offenen Defekt an ihrer eigenen Zusage tragen, sagt etwas Falsches über den Stand.
...
- **Sechs Abnahmekriterien messen über eine `grep`-Zahl, die inzwischen veraltet ist.** Betroffen sind S16 (`addSubview`: seit S44 ein Aufruf in einer Schleife über `Bereich::ALLE` statt fünf, `crates/krk-ui/src/appkit/aufteilung.rs:203`), S21 (`Statuszeile`: sechs Treffer statt null, alle sechs Doku), S31 (`FSEventStreamCreate`: zehn Treffer statt eins, davon ein Aufruf), S44 (`holt_hervor`: fünf Treffer statt null, alle fünf Doc-Verweise), S45 (`fokusanzeige_nachziehen`: sechs Treffer, davon ein Rumpf und zwei Aufrufstellen) und S48 (`setTitle`: sieben Treffer, davon zwei Fenstertitel). In allen sechs Fällen hält die Sache und zählt die Formel falsch. Eine Zählformel als Abnahmekriterium altert an jedem Kommentar, der das gesuchte Wort enthält.
- **S13 ist anders gebaut als angesagt.** Der Schritt sagt, `ist_beweglich` werde von `matches!` zu einer erschöpfenden Fallunterscheidung. Gebaut ist eine Weiterleitung (`crates/krk-ui/src/fenstermodell.rs:216-218` fragt `seite().is_some()`), und die erschöpfende Fallunterscheidung sitzt in `seite` (`:120-126`). Die Wirkung stimmt, der Wortlaut nicht.
- **Sieben Verweise im Plan tragen weiter einen festen Marker** statt der Sternstelle, die der Kopf dieses Plans verlangt (Zeilen 478, 670, 681, 696, 833, 864, 1330), alle in der Kurzform mit Auslassungspunkten. Der Spec trägt null. S42 hat die Umstellung also im Spec vollständig und im Plan zu 39 von 46 durchgezogen.
  - **Nachtrag vom 260811-1155: alle sieben Stellen sind ausgeschrieben, und zwei Angaben dieses Punktes waren falsch.** Die erste ist die Zahl. Gezählt sind Zeilen und keine Verweise: die Zeile 1330 trug drei Verweise und nicht einen, sieben Stellen sind also neun Verweise gewesen. Die zweite ist der Satz „Der Spec trägt null". Der Spec trug zwei, beide auf derselben Zeile unter `## Ausdrücklich außerhalb dieser Runde` und beide auf Entscheidungsdatensätze der Runde 1. Die Erhebung konnte sie nicht sehen, weil ihr Suchmuster die Endung `.md` verlangte und die Kurzform sie nicht trägt; der Befund darüber ist `shared/issues/260810-1851_*_acht-verweise-in-spec-und-plan-der-runde-2-stehen-in-kurzform-und-entgehen-jeder-suche.md`. Behoben ist die Zeile 1330 mit `c0b96a6` (verzeichnet im Abgleich 260810-1404, heute Zeile 1352), die übrigen sechs Zeilen und die zwei des Specs mit dieser Behebung. Der volle Name ist in jedem Fall gegen den Dateibestand aufgelöst und nicht geraten; der Verweis auf Zeile 696 (heute 716) war daneben im Marker falsch und ist mit der Sternstelle erledigt. Mit ausgeschrieben ist eine neunte Stelle, die dieser Befund nicht führt und nicht führen konnte, weil sie die Sternstelle schon trug und allein im Namen gekürzt war (heute Zeile 1587).
- **44 der 48 Schritte tragen `[DONE]` ohne einen Ausführungsvermerk im Plan.** Nur S39, S40, S41 und S42 tragen einen. Die Ausführung ist dennoch belegt: 51 Sitzungshistorien unter `history/` führen sie einzeln. Der Plan der Runde 1 hat es umgekehrt gemacht, ohne Vermerke je Schritt, dafür mit einem `## Reconciliation Log` am Ende.
- **`crates/krk-ui/src/main.rs:17` verzählt sich.** Der Modulkopf sagt „Zehn Module liegen ausdrücklich neben `appkit`"; eingebunden sind elf (`:37-48`, zwölf `mod`-Zeilen abzüglich `appkit`). Nicht mitgezählt ist `hervorhebung`, das mit S33 entstanden ist.

#### Was dieser Abgleich an Markern gezogen hat

Nichts an diesem Plan außer der Statuszeile und diesem Abschnitt. Die Marker, die gezogen wurden, stehen in den Entscheidungs- und Defektspeichern und sind in `history/260810-0805-reconciliation.md` einzeln aufgeführt.

### 260810-0822 — beide Kriterien berichtigt

Beide Befunde des Abgleichs von 260810-0805 sind geschlossen, und in beiden Fällen war das Kriterium falsch und nicht der Code. Am Programm ist nichts geändert worden außer einer gefallenen Probe.

**S6: der vorgeschlagene Schnitt entlang des Zustellers trägt nicht, und die Einschränkung fällt ersatzlos.** `issues/260809-1527` hatte vorgeschlagen, die Regel auf das einzuschränken, was „KRK selbst über den Tastencode zustellt". Der Vorschlag ist an drei Stellen geprüft und hält an keiner:

1. **Seine Voraussetzung war schon beim Schreiben falsch.** Er beruft sich darauf, KRK belege nach C3 der Runde 1 den virtuellen Tastencode. Seit S2 ist das für Buchstaben und Ziffern nicht mehr so: `Taste::kennung` (`crates/krk-core/src/tasten/parser.rs:192-198`) liefert für jeden einbuchstabigen Namen aus einem ASCII-Kleinbuchstaben oder einer Ziffer eine Zeichenkennung, und `Belegung::nachschlag` (`belegung.rs:821-838`) vergleicht Maske und Kennung.
2. **Am heutigen Code gelesen ist der Vorschlag leer.** `y` und `z` sind einbuchstabig, ihre Kennung ist damit immer eine Zeichenkennung und nie ein Code. Die Menge „von KRK über den Tastencode zugestellt und auf `kVK_ANSI_Y` oder `kVK_ANSI_Z`" ist deshalb für jede denkbare Belegung leer. Ein Kriterium, das keine Belegung verletzen kann, misst nichts.
3. **Die Grenze, entlang der er schneidet, trennt nichts mehr.** Der Vorschlag zieht sie an `gehalten_von`. Beide Zusteller schlagen Buchstaben seit S2 über das Zeichen nach, das Hauptmenü über `NSMenuItem.keyEquivalent` und der Abgriff über `charactersByApplyingModifiers:`. Der Zusteller ist für diese Frage ohne Unterschied.

Was stattdessen gilt, sagt `issues/260809-1746`: die Einschränkung auf `y` und `z` entfällt ersatzlos, weil seit S2 keine Stelle mehr wandert. Das Abnahmekriterium von S6 nennt jetzt allein, was die Probe `die_auslieferungsbelegung_fuehrt_einundsiebzig_funktionen` hält, und der Satz in Befund 4 ist auf denselben Stand gezogen.

**Die Probe ist gefallen und nicht umgebaut.** `keine_neue_kombination_liegt_auf_den_beiden_wandernden_stellen` hätte auf den vorgeschlagenen Schnitt umgebaut eine leere Menge geprüft. Sie war die einzige Änderung am Prüfbestand; an ihrer Stelle steht ein Kommentar, der sagt, was sie hielt und wer es seither hält, nämlich `auf_einer_deutschen_tastatur_findet_die_aufschrift_y_die_vorschau` und `jede_ausgelieferte_kombination_traegt_die_kennung_ihrer_tastensorte`.

**S33: das Kriterium nennt jetzt beide Merkmalswege und macht die Zusage an den Zeichen fest.** Der gebaute Code ist richtig. Falsch war die Annahme, vorübergehende Merkmale des Layoutverwalters trügen alle vier zugesagten Wirkungen; `NSLayoutManager.h:351` sagt, erkannt werde für das Zeichnen allein, was die Auslegung nicht ändert. `### Frage 7`, die Änderungszeile und das Abnahmekriterium von S33 sind auf den Schnitt „wirkt auf die Auslegung oder nicht" gezogen. Der Entscheid `260808-0140_*_was-heisst-gerendert-bei-markdown-wenn-zugleich-bearbeitet-wird.md` stand auf umgesetzt und ist deshalb nicht zurückgedreht, sondern auf überholt gezogen; der ablösende ist `decisions/260810-0822_*_wie-die-formatansicht-ihre-auszeichnung-setzt-und-warum-an-zwei-orten.md`.

**Zwei weitere Stellen im Plan führten dieselbe Aussage** und sind mitgezogen: die Begründung bei S36, Suchen und Ersetzen bezögen sich auf den Text und nicht auf die Darstellung, „weil die Einfärbung nach S33 im Layoutverwalter liegt und den Textspeicher nicht anfasst"; und der Punkt unter `## Wie dieser Plan die Maxime "supersimpel" einlöst`, der die Einfärbung im Layoutverwalter als Grund für den einen Textbestand führte. Beide tragen jetzt den Grund, der wirklich trägt: S33 setzt Merkmale und keine Zeichen.

**Zwei Stellen außerhalb dieses Plans führen die falsche Aussage von S6 weiter** und sind nicht angefasst, weil sie in `resources/default-keymap.toml` stehen und damit nicht in der Schreibgrenze des `coder` liegen. Sie stehen seit dem 260810-0011 als `issues/260810-0011_*_zwei-kommentarbloecke-der-belegungsdatei-behaupten-den-nachschlag-ueber-den-tastencode.md` für den `ontocoder` bereit; der Datensatz sagt selbst, er sei mit den beiden hier geschlossenen zusammen zu erledigen, und er bleibt als einziger der drei offen.

**Der Marker dieser Datei bleibt trotzdem auf offen.** Der Grund unter `#### Warum der Marker deshalb auf offen bleibt` ist mit diesen beiden Berichtigungen weg, aber die Entscheidung über die Umbenennung gehört nicht dem `coder`: sie zieht die Zeile `**Active spec/plan:**` im Circle-Datensatz nach sich und damit eine Änderung außerhalb dieses Plans.

### 260810-0918 — drei Textberichtigungen aus offenen Defekten

Drei Defekte aus Turn 1 betrafen den Plantext und nicht den Code. Sie sind hier eingearbeitet. **Kein `[DONE]` und kein Status im Kopf ist angefasst**, und am Programm ist nichts geändert: die drei Berichtigungen sind Prosa, an drei Stellen mit einer Messung belegt.

**1. Eine Schnittstellen-Regel fehlte, und S11 nennt seinen Nachzug jetzt.** `issues/260808-0930` hielt fest, dass S11 `Lesezeichen` in `krk-core` ändert und die zwei Aufrufstellen in `crates/krk-ui/src/leistenmodell.rs` den Schritten S38 und S39 zuweist, die 26 Positionen später liegen. Der Abschnitt `### Was die Dateiliste eines Schrittes zusagt` sagte, was eine Dateiliste bedeutet, aber nicht, dass der Arbeitsbereich zwischen zwei Schritten übersetzen muss. Er trägt jetzt als dritte Herleitungsregel die **Schnittstellen-Regel** und verweist auf die beiden Abschnitte des Nachtrags, die sie für die sechs neuen Schritte schon anwenden. S11 nennt die nachgezogene Datei in seiner Liste, verlangt in seinem Abnahmekriterium `cargo build --workspace` statt allein `cargo test -p krk-core` und trägt einen Umsetzungsvermerk mit dem Commit `65c8efa`.

**2. Das vierte Kriterium von S32 stand an einer Stelle, an der es nichts messen kann, und ist geteilt.** `issues/260808-0948` hatte es gemessen: in S32 nennt kein Aufrufer die Kisten, der Übersetzer wirft den eingebetteten Bestand wieder weg, und `target/release/krk` wuchs um 128 Byte. Kriterium 4 fragt jetzt den Preis der Kisten an einem eigenständigen Prüfprogramm (1.591.544 gegen 418.968 Byte, rund 1,12 MiB), und das Wachstum des Bündels ist an S33 abgenommen, wo der Bestand zum ersten Mal geladen wird. **Dort ist es gemessen und nicht bloß gefordert:** `target/KRK.app` trägt am 260810-0918 7.191.902 Byte gegen 3.502.046 Byte vor S32, also 3.689.856 Byte Zuwachs für die ganze Runde und damit eine obere Schranke unter der Grenze von 10 MB. Der Zahlenrahmen von 10 MB ist unverändert. Mitgezogen sind der Satz in `### Frage 2` über die beiden Größen, die S32 abnimmt, und die Zeile in `## Risiken und Gegenmaßnahmen`, die beide Messungen S32 zuschrieb.

**3. `dump-create` lässt sich nicht abschalten und steht nicht mehr unter den abgeschalteten Merkmalen.** `issues/260808-0949` hatte den Grund in `syntect` 5.3.0 belegt: `parsing = [regex-syntax, fnv, dump-create, dump-load]` zieht es mit, und ohne `parsing` tut die Kiste nichts. Die Angabe war sachlich falsch und nicht bloß unerfüllt. `### Frage 2` und S32 nennen jetzt vier abgeschaltete Merkmale und sagen dazu, warum es nicht fünf sind. Die Wirkung ist gering: `dump-create` bringt `bincode` und `flate2` mit, die über `dump-load` ohnehin im Baum stehen.

Nichts an diesem Plan außer den genannten Stellen und diesem Abschnitt. Über den Zustandsmarker der Datei ist auch mit diesen drei Berichtigungen nicht entschieden; der Grund steht einen Abschnitt höher.

### 260810-1404 — Abschluss-Abgleich der Sitzung 260810-0845
...
**Nachtrag vom 260811-1155: die 40 sind nicht alle Verweise gewesen, und einer der ungezählten war tot.** Sieben weitere standen zu diesem Zeitpunkt in der Kurzform mit Auslassungspunkten, sechs davon mit festem Marker und einer mit Sternstelle; sie sind in diese Prüfung nicht eingegangen, weil ihr Suchmuster die Endung `.md` verlangte und die Kurzform sie nicht trägt. Die Aussage „Kein toter Verweis" gilt damit für die 40 und nicht für den Plan: der Verweis auf Zeile 696 der damaligen Zählung (heute 716) nannte `260808-1413_o_`, während die Datei `_c_` trug. Beides ist mit der Behebung von `shared/issues/260810-1851_*_acht-verweise-in-spec-und-plan-der-runde-2-stehen-in-kurzform-und-entgehen-jeder-suche.md` erledigt: alle sieben tragen jetzt den vollen Namen mit Sternstelle, und ein Verweis mit Sternstelle kann an einem Markerwechsel nicht mehr sterben. Die Lehre gehört nicht dem Plan, sondern der Erhebung, und der Befund schreibt sie auf: jedes Suchmuster dieses Projekts, das `\.md` verlangt, hat denselben blinden Fleck.
...
**Was am Plan gegen den Code geprüft ist.** Die 52 in dieser Sitzung geschlossenen Defekte sind einzeln daraufhin gelesen worden, ob ihre Behebungsbehauptung im Baum steht. Keine ist erfunden: 45 vollständig gedeckt, 7 mit einer abgewanderten Zeilenangabe oder einer nachgezogenen Nachbarangabe, 0 ohne Deckung. Der Bericht steht in `history/260810-1404-reconciliation.md`.
