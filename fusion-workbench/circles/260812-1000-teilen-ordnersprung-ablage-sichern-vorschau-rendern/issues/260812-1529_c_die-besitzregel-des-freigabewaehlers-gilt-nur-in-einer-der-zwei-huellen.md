...
---
Resolved: Zuschnitt 2 des Abschnitts „Was zu tun ist", also der ohne unbelegbare Annahme: `eintrag_anfuegen` haelt seinen `NSSharingServicePicker` fest wie `anbieten` den seinen. Er bekommt dafuer einen **zweiten** Schlitz (`MENUEWAEHLER`) und nicht denselben — ein Kontextmenue kann aufgehen, waehrend ein Freigabedialog steht, und ein geteilter Schlitz naehme dem Dialog seinen Besitzer. Die Regel „einer und keine Reihe" gilt seither je Weg. Beide Nebenbefunde sind mitbehoben: der Kommentar an der Reihenfolge in `anbieten` sagt jetzt, dass die zwei Zeilen den Fall **nicht** tragen, und nennt die Voraussetzung, die ihn traegt (die Verfolgungsschleife von `showRelativeToRect:`, als `inference:` gekennzeichnet); und dass kein Waehler je freigegeben wird, steht an beiden Schlitzen samt dem sauberen Weg und dem Grund, aus dem er nicht gegangen wird.
