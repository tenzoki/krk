...
---
Resolved: Behoben, soweit der Befund reicht, und auf demselben Weg wie sein Schwesterbefund `260812-2140`: "Der Befund ist, dass zwei gleiche Zeilen der Quelle verschieden herauskommen, und dass diese Ungleichheit mit diesem Turn neu ist und **nirgends steht**." Sie steht jetzt. Der Doc-Kommentar von `Zerlegung::luecke_bis` (`crates/krk-ui/src/markdown.rs`) traegt einen eigenen Abschnitt mit der gemessenen Quelle und ihrer Ausgabe, mit dem Satz, dass der behaltene Einzug keiner ist, den CommonMark traegt, und mit dem Hinweis, dass er von dem Einzug, der wirklich Inhalt ist, ueber `trim` allein nicht zu unterscheiden ist. Neue Probe `auf_dokumentebene_verliert_die_erste_zeile_einer_luecke_ihren_einzug` mit genau der Quelle aus dem Datensatz; sie steht neben `auf_dokumentebene_bleibt_der_einzug_einer_zeile_stehen`, deren Quelle die Grenze nicht traf. Der Zuschnitt (kuerzester fuehrender Lauf ueber alle nichtleeren Zeilen) ist **nicht** gebaut: er ist in beiden Datensaetzen ausdruecklich nicht gewaehlt, er beruehrt beide Seiten der Grenze `self.offen.is_empty()`, und was er mit einer Luecke tut, deren erste Zeile leer ist, ist an keiner der beiden Stellen geprueft. Geprueft: cargo test -p krk-ui Exit 0.
