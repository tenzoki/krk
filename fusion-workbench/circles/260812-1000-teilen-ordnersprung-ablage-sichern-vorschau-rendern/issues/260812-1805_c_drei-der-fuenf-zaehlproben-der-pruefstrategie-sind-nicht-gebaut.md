...
---
Resolved: Alle fuenf Zaehlproben der Pruefstrategie stehen jetzt im Baum. Zwei der drei fehlenden waren beim Nachmessen am 260906 bereits gebaut, ohne dass jemand diesen Datensatz angefasst haette: `die_huelle_um_die_zwischenablage_steht_in_genau_einer_datei` (`crates/krk-ui/src/appkit/zwischenablage.rs:824`, C1.8) und `genau_drei_pruefordner_fassungen_stehen_im_baum` (`crates/krk-core/tests/baum.rs:120`, C6.6). Die dritte, C4.5, ist heute gebaut: `keine_datei_des_baums_baut_eine_web_ansicht` im Pruefmodul von `crates/krk-ui/src/appkit/vorschau.rs`, wo die Zusage wohnt. Sie zaehlt drei zusammengesetzte Nadeln ueber `quelldateien()` — die Klasse, das Rahmenwerk und die objc2-Kiste — und trennt damit Nennen von Benutzen, wie `allein_diese_datei_baut_den_freigabewaehler` es vormacht. Gemessen vor dem Bau mit `grep -rn "WKWebView\|WebKit" crates/ xtask/ --include=*.rs`: keine Fundstelle. `make check` exit 0.
