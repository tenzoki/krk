...
---
Resolved: Behoben auf dem im Datensatz genannten Zuschnitt, ohne die dort erwogene Zusage ueber die Sortierung von `offen`. `Zerlegung::merkzeichen_einloesen` (`crates/krk-ui/src/markdown.rs`) fuehrt keinen inneren Durchlauf mehr: welche Eintraege nachruecken, steht schon vor dem ersten Schreiben fest, naemlich die mit `anfang == stelle beim Eintritt`, und ihr Endwert ist der Lesestand beim Eintritt in ihre eigene Stufe. Die Herleitung steht am Doc-Kommentar; eine Annahme ueber die Sortierung braucht sie nicht. Gemessen am 260908 auf diesem Geraet, `--release`, Bestzeit aus sieben Laeufen ueber `"- " x Tiefe`: 12 kB von 161,6 ms auf 42,6 ms, 20 kB von 408,0 ms auf 120,1 ms, 2 kB von 2,60 ms auf 0,79 ms. Die Grenze von L7 wandert damit von rund 12 kB auf rund 19 kB zurueck, also auf den Stand vor `2c0b2a6`. Geprueft: cargo test -p krk-ui Exit 0.
