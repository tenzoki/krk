...
---
Resolved: Die Entscheidung ist aus der Ansicht in `Tabliste::auswahl_auf_namen` (`crates/krk-ui/src/tabs.rs`) gezogen, wo Lesevorgang und `wunschauswahl` beieinander liegen, und fragt dort **zuerst** `Tabinhalt::liest()`: steht ein Lesevorgang aus, wird der Name vorgemerkt und die Antwort ist `Vorgemerkt`. `DateifensterQuelle::eintrag_waehlen` ist nur noch die AppKit-Seite und setzt bei `Gewaehlt(zeile)` die Zeile. Genommen wurde `liest()` und nicht `Ordnermodell::ersetzt_beim_naechsten_stapel`: letzteres ist die Frage der Ansicht ("muss der nächste Stapel als `reloadData` kommen"), enthält deshalb den Term `!sichtreihenfolge.is_empty()` und wäre auch dann falsch, wenn der erste Stapel schon da ist, der Lauf aber noch läuft — dann räumt nicht der Ersatz, sondern `wunschauswahl_anwenden` beim Abschluss die frisch gesetzte Auswahl weg. Probe: `der_erste_neue_name_eines_stapel_umbenennens_wird_vorgemerkt` in `crates/krk-ui/src/tabs.rs` hält die Umnummerierung nach oben fest; sie schlägt gegen die alte Reihenfolge fehl.
