...
**Jeder Bericht trägt seine Bedingungen.** Kopf jeder Messdatei: Zeitpunkt, `sysctl -n hw.model`, `sw_vers`, Bildwiederholrate, Cache-Zustand, Zahl der Wiederholungen, Startwert des Prüfordners. Ausgewiesen wird das 95. Perzentil über zwanzig Wiederholungen, wie C8 es verlangt, und daneben Minimum und Median, damit ein Ausreißer erkennbar bleibt. Diese Disziplin übernimmt der Plan von der Fn-Messung, wo genau sie den Auswertungsfehler sichtbar gemacht hat.
...
- Änderungen: Workspace mit vier Mitgliedern anlegen. `rust-toolchain.toml` schreibt 1.97.1 fest. `.cargo/config.toml` setzt im Abschnitt `[env]` `MACOSX_DEPLOYMENT_TARGET = "15.0"`. `krk-core/src/lib.rs` beginnt mit `#![deny(unsafe_code)]`, nicht mit `forbid`, weil das spätere Modul `verzeichnis::sys` die beiden Systemaufrufe `getattrlistbulk` und `copyfile` binden muss und dafür `#[allow(unsafe_code)]` trägt. `krk-ui/src/main.rs` beginnt mit `#![warn(unsafe_code)]`; das spätere Modul `appkit` trägt `#[allow(unsafe_code)]`. Abhängigkeiten: `objc2`, `objc2-app-kit`, `objc2-foundation` in `krk-ui`; `serde` und `toml` in `krk-core`. `krk-ui` und `krk-bench` sind zunächst leere Rümpfe.
- Abhängigkeiten: keine
- Abnahmekriterium: `cargo build --workspace`, `cargo build --workspace --target x86_64-apple-darwin`, `cargo build --workspace --target aarch64-apple-darwin` und `cargo test --workspace` beenden jeweils mit Rückgabewert 0. Der Diff zeigt `#![deny(unsafe_code)]` in `krk-core/src/lib.rs`, `#![warn(unsafe_code)]` in `krk-ui/src/main.rs` und `MACOSX_DEPLOYMENT_TARGET = "15.0"` in `.cargo/config.toml`.

#### 2. **Verzeichnisleser und Ordnermodell**
