...
**Datum:** 2026-08-02, 14:28; nachgezogen 2026-08-02, 15:01, 17:46 und 18:59, dann 2026-08-03, 12:00, 12:08 und 15:30, zuletzt 2026-08-03, 18:19
...
- Ausführender: `coder`
- Dateien: `crates/krk-core/src/operation/{mod.rs,auftrag.rs,kopieren.rs,verschieben.rs,loeschen.rs,anlegen.rs,umbenennen.rs,fortschritt.rs}`, `crates/krk-core/src/lib.rs` (einbindend: `pub mod operation;`), `crates/krk-core/src/verzeichnis/sys.rs` (erweitert: die Bindungen an `copyfile(3)` und `rename(2)` kommen in dieses vorhandene Modul, siehe Änderungen), `crates/krk-core/tests/operation.rs`, `crates/krk-ui/src/appkit/papierkorb.rs`, `crates/krk-ui/src/appkit/mod.rs` (einbindend: `mod papierkorb;`)
...
- Ausführender: `coder`
- Dateien: `crates/krk-ui/src/appkit/leiste.rs`, `crates/krk-ui/src/appkit/mod.rs` (einbindend), `crates/krk-ui/src/appkit/volumes.rs` (erweitert: die Aufzählung über `NSFileManager.mountedVolumeURLs…` kommt neben die `NSWorkspace`-Beobachtung aus S14, siehe unten), `crates/krk-ui/src/leistenmodell.rs`, `crates/krk-ui/src/main.rs` (einbindend: `mod leistenmodell;`), `crates/krk-core/src/ablage/lesezeichen.rs` (erweitert, aus S10)
...
Sieben Punkte sind beim Planen und beim Nachziehen aufgefallen und liegen als eigene Dateien, nicht in diesem Plan vergraben. Zehn weitere kamen aus den Umsetzungen zurück, sechs aus den Schritten 2 bis 5 und vier aus Schritt 8. Vier davon sind mit dem Nachzug vom 260802-1859 geschlossen, zwei mit dem vom 260803-1200 und zwei mit dem vom 260803-1819:
...
Die sieben Punkte aus der Planung und dem Nachziehen selbst:
...
- `issues/260803-1819_o_dateilisten-von-s9-bis-s23-noch-nicht-unter-der-erweiterten-regel-durchgegangen.md` — **offen, bindet den nächsten Schritt nicht.** Die erweiterte Dateilisten-Regel ist bisher nur auf S8 angewandt. Der Nachzug über die übrigen Schritte ist bewusst nicht in dieselbe Bearbeitung gefallen, weil sie die Nutzerentscheidung zum Abnahmemaß einzuarbeiten hatte.
