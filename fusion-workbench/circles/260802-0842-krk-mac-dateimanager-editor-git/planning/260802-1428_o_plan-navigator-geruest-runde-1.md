...
**Datum:** 2026-08-02, 14:28; nachgezogen 2026-08-02, 15:01 und 17:46
...
Dieser Plan setzt die Fähigkeiten C1 bis C9 des Specs in 23 einzeln abnehmbare Schritte um. Er beantwortet die sieben technischen Fragen aus `## Offen für den Planner` und legt damit erstmals eine Verzeichnisstruktur, ein Bauverfahren und eine Teststrategie für KRK fest.
...
**Jeder Bericht trägt seine Bedingungen.** Kopf jeder Messdatei: Zeitpunkt, `sysctl -n hw.model`, `sw_vers`, Bildwiederholrate, Cache-Zustand, Zahl der Wiederholungen sowie Pfad und Startwert jedes verwendeten Prüfordners. Ausgewiesen wird das 95. Perzentil über zwanzig Wiederholungen, wie C8 es verlangt, und daneben Minimum und Median, damit ein Ausreißer erkennbar bleibt. Diese Disziplin übernimmt der Plan von der Fn-Messung, wo genau sie den Auswertungsfehler sichtbar gemacht hat.
...
Zweitens: **die Signaturidentität muss über Bauläufe stabil sein.** Eine Ad-hoc-Signatur mit `codesign -s -` ändert ihren Hash bei jedem Bau, und TCC hält jeden Bau für eine andere Anwendung. Der Nutzer bekäme bei jedem Lauf dieselbe Rückfrage erneut. Für die Entwicklung erzeugt S5 deshalb eine lokale, selbstsignierte Code-Signing-Identität im Schlüsselbund und verwendet sie durchgängig. Für die Auslieferung braucht es eine Developer-ID-Identität, siehe S23.
...
#### 5. **Bündelbau und lokale Signierung**

- Ausführender: `coder`
- Dateien: `xtask/src/{bundle.rs,sign.rs}`, `xtask/src/main.rs`, `README.md`
- Änderungen: `cargo xtask bundle` legt `target/KRK.app/Contents/{MacOS,Resources}` an, kopiert das Binärprogramm und `resources/Info.plist`, schreibt `PkgInfo`, und ruft `codesign` mit der Identität aus der Umgebungsvariablen `KRK_SIGN_IDENTITY`. Fehlt sie, sucht der Schritt eine lokale selbstsignierte Identität namens `KRK Entwicklung` und bricht mit einer Anleitung zu ihrer Erzeugung ab, wenn auch die fehlt. Kein stillschweigendes Ausweichen auf eine Ad-hoc-Signatur, weil TCC den Nutzer sonst bei jedem Bau erneut fragt. `README.md` beschreibt Bau, Signierung und die Erzeugung der Entwicklungsidentität.
- Abhängigkeiten: S4
- Abnahmekriterium: `cargo xtask bundle` erzeugt `target/KRK.app` mit der genannten Struktur. `codesign --verify --strict target/KRK.app` beendet mit 0. `codesign -dv target/KRK.app` nennt die Identität, nicht `adhoc`. `vtool -show-build-version target/KRK.app/Contents/MacOS/krk` meldet `minos 15.0`. Ohne setzbare Identität bricht der Aufruf mit Rückgabewert ungleich 0 und einer Anleitung ab.

#### 6. **Fenster, Menü und echte Dateiliste, erster Teil des Durchstichs**

- Ausführender: `coder`
- Dateien: `crates/krk-ui/src/main.rs`, `crates/krk-ui/src/appkit/{mod.rs,anwendung.rs,fenster.rs,tabelle.rs,menue.rs}`
...
- Abnahmekriterium: `cargo xtask bundle && open target/KRK.app` öffnet ein Fenster mit den echten Einträgen von `$HOME` in vier Spalten mit korrekten Größen und Daten. Cmd+Q beendet, Cmd+W schließt das Fenster. Ein mit S3 erzeugter Ordner mit 100.000 Einträgen lässt sich flüssig durchblättern. `grep -rn 'unsafe' crates/krk-ui/src --include='*.rs' -l` liefert ausschließlich Dateien unterhalb von `src/appkit/`.

#### 7. **Tastenereignisse und Pfeiltasten, zweiter Teil des Durchstichs**

- Ausführender: `coder`
- Dateien: `crates/krk-ui/src/appkit/ereignisse.rs`, `crates/krk-core/src/tasten/{mod.rs,normalisierung.rs}`, `crates/krk-core/tests/tasten.rs`
...
- Ausführender: `coder`
- Dateien: `crates/krk-bench/src/messen.rs`, `crates/krk-ui/src/messmodus.rs`, `messungen/<datum>-durchstich.txt`
...
- Ausführender: `coder`
- Dateien: `crates/krk-ui/src/kommandos/{navigation.rs,auswahl.rs,pfadeingabe.rs}`, `crates/krk-core/src/verzeichnis/sprungmarke.rs`, `crates/krk-core/tests/navigation.rs`
...
- Ausführender: `coder`
- Dateien: `crates/krk-core/src/operation/{mod.rs,auftrag.rs,kopieren.rs,verschieben.rs,loeschen.rs,anlegen.rs,umbenennen.rs,fortschritt.rs}`, `crates/krk-core/tests/operation.rs`
...
- Ausführender: `coder`
- Dateien: `crates/krk-core/src/umbenennen/{regel.rs,vorschau.rs,kollision.rs}`, `crates/krk-ui/src/blaetter/stapelumbenennen.rs`, `crates/krk-core/tests/umbenennen.rs`
...
- Ausführender: `coder`
- Dateien: `crates/krk-ui/src/appkit/vorschau.rs`, `crates/krk-ui/src/vorschaumodell.rs`
...
- Abnahmekriterium: `cargo xtask messen --alle --ordner-a <pfad> --ordner-b <pfad> --ordner100k <pfad>` erzeugt einen Bericht, der alle zehn Zusagen L1 bis L10 mit 95. Perzentil, Median und Minimum ausweist und den vollständigen Bedingungskopf trägt. Der Kopf nennt die drei Prüfordner mit Pfad und Startwert und beschreibt die Prüfsitzung, auf der L4 und L5 gemessen wurden. Der Bericht kennzeichnet die L1-Messung ausdrücklich als Spanne bis zum Ende des Zeichendurchgangs und nicht als Bildschirmmessung.

#### 22. **Vollständige Messreihe auf dem Referenzgerät**

- Ausführender: `coder`
- Dateien: `messungen/<datum>-MacBookPro15-1-abnahme.txt`, `CLAUDE.md`
- Änderungen: die vollständige Messreihe auf dem `MacBookPro15,1` fahren, warm und kalt, mit den drei Prüfordnern aus S3: A und B mit je 10.000 Einträgen an verschiedenen Pfaden und der Ordner mit 100.000 Einträgen. Ergebnis versionieren. `CLAUDE.md` um die nun vorhandenen Bau-, Test- und Messkommandos ergänzen, die der Abschnitt `## Projektstand` bisher ausdrücklich als noch nicht vorhanden führt.
- Abhängigkeiten: S21
