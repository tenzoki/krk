...
**Datum:** 2026-08-02, 14:28; nachgezogen 2026-08-02, 15:01, 17:46 und 18:59
...
- Änderungen: `cargo xtask bundle` legt `target/KRK.app/Contents/{MacOS,Resources}` an, kopiert das Binärprogramm, kopiert `resources/Info.plist` mit eingesetzter Version (siehe unten), schreibt `PkgInfo`, und ruft `codesign` mit der Identität aus der Umgebungsvariablen `KRK_SIGN_IDENTITY`. Fehlt sie, sucht der Schritt eine lokale selbstsignierte Identität namens `KRK Entwicklung` und bricht mit einer Anleitung zu ihrer Erzeugung ab, wenn auch die fehlt. Kein stillschweigendes Ausweichen auf eine Ad-hoc-Signatur, weil TCC den Nutzer sonst bei jedem Bau erneut fragt. `README.md` beschreibt Bau, Signierung, Versionspflege und die Erzeugung der Entwicklungsidentität.
...
- Ausführender: `coder`
- Dateien: `crates/krk-ui/src/main.rs` (einbindend: `mod appkit;`), `crates/krk-ui/src/appkit/{mod.rs,anwendung.rs,fenster.rs,tabelle.rs,menue.rs}`, `crates/krk-ui/Cargo.toml` (einbindend: `krk-core = { path = "../krk-core" }`, weil S6 das Ordnermodell aus S2 anbindet und `krk-ui` den Kern bisher nicht führt)
...
Drei Punkte sind beim Planen aufgefallen und liegen als eigene Dateien, nicht in diesem Plan vergraben. Vier weitere kamen aus den Umsetzungen der Schritte 2 bis 4 zurück und sind mit dem Nachzug vom 260802-1859 geschlossen:
...
Die drei Punkte aus der Planung selbst:
