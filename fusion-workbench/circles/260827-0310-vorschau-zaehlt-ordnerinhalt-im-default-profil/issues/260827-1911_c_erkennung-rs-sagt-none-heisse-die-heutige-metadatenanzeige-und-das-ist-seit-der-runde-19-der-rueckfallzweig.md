...
---
Resolved: Drei Stellen in `crates/krk-core/src/leseprofil/erkennung.rs` nachgezogen und nicht zwei: neben dem Ablaufbild im Modulkopf und dem Doc-Kommentar von `erkennen` trug auch der einleitende Absatz des Modulkopfs den Satz, in der Beugung "der heutigen Metadatenanzeige", die die Schliessbedingung des Datensatzes (`grep -n 'heutige Metadatenanzeige'`) nicht gefunden haette. Alle drei sagen jetzt, was `erkennen` liefert, naemlich `None`, und zeigen fuer die Folge auf `bausteine::zusammenfassen_gezaehlt`, statt sie zu wiederholen; genau so, wie der Datensatz es verlangt, damit derselbe Satz beim naechsten Rueckfallzweig nicht ein drittes Mal falsch wird. Der Doc-Kommentar von `erkennen` sagt dazu, was bis zur Runde 19 dort stand und warum es falsch wurde. Der `#[must_use]`-Text ist unangetastet. Schliessbedingung geprueft: `grep -n 'heutige Metadatenanzeige' crates/krk-core/src/leseprofil/erkennung.rs` ohne Fundstelle, `RUSTDOCFLAGS="-D warnings" cargo doc -p krk-core --no-deps` Exit 0 (der Verweis auf `super::bausteine::zusammenfassen_gezaehlt` loest auf, `bausteine` ist `pub mod`). Die vierte Stelle liegt in `resources/default-readers.toml` und gehoert dem `ontocoder`; ihr Datensatz bleibt offen.
