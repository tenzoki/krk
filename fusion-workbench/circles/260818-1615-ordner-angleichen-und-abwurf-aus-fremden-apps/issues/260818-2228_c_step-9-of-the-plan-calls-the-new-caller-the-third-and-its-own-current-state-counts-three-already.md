...
---
Resolved: Als Nachsatz am Plan berichtigt (`planning/260818-1633_c_plan-…`, Punkt 2), nach der Nutzerentscheidung zu `shared/decisions/260906-0203_*`. Der Nachsatz nennt statt einer Ordnungszahl das Zaehlkommando, wie `CLAUDE.md` es fuer `Kommando`, `Wirkungsbereich` und `Art` haelt — und aus demselben Anlass: die Zahl war beim Schreiben des Plans falsch ("dritter" bei drei vorhandenen), beim Ablegen dieses Datensatzes als "vierter" richtig und ist heute wieder falsch. Gemessen mit `grep -rn "self\.auftrag_starten(" crates/krk-ui/src` → **sechs** Rufer (`crates/krk-ui/src/appkit/anwendung.rs:6215`, `:6475`, `:6651`, `:6782`, `:6868`, `:6919`).
