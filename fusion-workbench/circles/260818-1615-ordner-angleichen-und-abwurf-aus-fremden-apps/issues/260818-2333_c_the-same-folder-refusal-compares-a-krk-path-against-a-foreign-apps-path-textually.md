...
---
Resolved: Behoben, aber nicht wo der Datensatz es vorschlug — und die Einschaetzung "kein Datenverlust" dieses Datensatzes ist widerlegt. Sie stuetzte sich auf copyfile(3) und rename(2) einzeln gemessen; der Baum erreicht beide mit bestehendem Ziel nie, weil operation::ziel_klaeren vorher die Konfliktfrage stellt und "Ueberschreiben" sie mit loeschen::baum_entfernen(ziel) beantwortet, einem echten remove_file und nicht dem Papierkorb. Ist das Ziel unter zweiter Schreibweise die Quelle, loescht das die Datei des Nutzers; gegen den unreparierten Baum gefahren endete es mit "die Quelle ist weg: NotFound". Dieselben drei Zeilen liessen einen Ordner ueber den textuellen starts_with-Schutz in seinen eigenen Baum absteigen, 139 Eintraege weit. Die Behebung sitzt an der entscheidbaren Stelle statt in abwurf_pruefen: operation::zielpfad vergleicht (st_dev, st_ino), mit lstat fuer "ist das Ziel die Quelle" und stat fuer "liegt das Ziel in der Quelle", weil das Ueberschreiben den Namen entfernt und ein Pfad durch seine Verknuepfungen laeuft. Der textuelle Vergleich in abwurf_pruefen bleibt und heisst im Doc-Kommentar jetzt Vorhersage; was durchrutscht, kostet einen annehmenden Zeiger und eine Zeile in der Abschlussliste.
