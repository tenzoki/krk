...
---
Resolved: Als Nachsatz am Spec berichtigt (`planning/260830-1251_p_spec-…`, Punkt 1), nach der Nutzerentscheidung zu `shared/decisions/260906-0203_*`. Der Nachsatz sagt, was C1.1 sagen muesste: genau eine der vier Stellen haelt den Bau an, naemlich `Bereichsleiste::bereichsschalter` ueber `Bereich::ALLE.map(…)`, und der Unterschied ist die Bauform und nicht die Feldbreite. Er nennt die drei anderen mit ihrem tatsaechlichen Ausgang (zweimal `index out of bounds` beim Start, einmal still) und die Warnung, die der Befund traegt: wer nach dem Eintrag in `ALLE` die Fehlerliste des Uebersetzers abarbeitet und dann fertig ist, liefert ein Buendel, das beim Start abstuerzt. Am Baum gegengeprueft mit `awk "/pub enum Bereich/,/^}/" crates/krk-ui/src/fenstermodell.rs` → **sechs** Werte; die Erweiterung ist gefahren, die vier Bauformen stehen unveraendert nebeneinander. Die Frage, welche Bauform kuenftig gilt, bleibt offen in `shared/decisions/260826-1811_*`.
