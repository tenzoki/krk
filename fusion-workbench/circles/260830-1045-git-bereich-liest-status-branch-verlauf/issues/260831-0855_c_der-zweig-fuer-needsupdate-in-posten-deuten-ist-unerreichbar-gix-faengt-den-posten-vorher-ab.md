...
---
Resolved: Moeglichkeit 1, wie empfohlen: der Zweig bleibt stehen, die drei Prosastellen sagen jetzt, wer den Posten abfaengt. Die Messung des Datensatzes ist am 260906 an der Quelle nachgeprueft und stimmt: `Iter::maybe_keep_index_change` (`gix-0.87.1/src/status/iter/mod.rs:296-305`) legt einen `EntryStatus::NeedsUpdate`-Posten in `self.index_changes` und gibt `None` zurueck, und `Iter::next` (`:241`) ueberspringt ihn daraufhin; er erreicht den Rufer nicht. Nachgezogen: `crates/krk-core/src/git/leser.rs`, Modulkopf und der Doc-Kommentar von `posten_deuten` (der Zweig traegt die Zusage nicht und kann es nicht, er haelt allein die Fallunterscheidung vollstaendig, falls eine spaetere gix-Fassung den Posten wieder durchreicht), und `crates/krk-core/src/git/mod.rs`, Modulkopf. Beide sagen jetzt, dass allein der fehlende Aufruf von `Outcome::write_changes` die Zusage traegt. Der Name steht dabei an keiner Prosastelle in der Aufrufform mit angehaengter Klammer, denn genau diese Suche ist das Pruefmittel: ein erster Entwurf dieser Berichtigung zitierte das Kommando woertlich und machte die Suche damit selbst nicht-leer. Nachgemessen nach der Berichtigung: die Suche ueber `crates/` bleibt ohne Fundstelle. Der zweite Riss in demselben `grep` steht weiter in `260830-1614_o_...` und ist nicht Gegenstand dieses Datensatzes.
