...
---
Resolved: Beide Haelften sind erledigt, jede auf ihrem Weg. **Die zweite Probe steht seit `210e4c1` im Baum**: `im_zettel_steht_keine_der_vier_ausgeschlossenen_faehigkeiten` (`crates/krk-ui/src/appkit/blaetter/zettel.rs`) sucht die vier Nadeln zusammengesetzt in den Codezeilen und laesst den Modulkopf, der die ausgeschlossenen Faehigkeiten beim Namen nennt, ausdruecklich aussen vor — genau die Bauform, die dieser Befund verlangt hat. **Die erste ist keine Probe geworden, und das ist eine Entscheidung des Baums und keine Unterlassung**: die Zusage "genau eine Ausnahme" gilt seit der Runde 14 nicht mehr, es sind zwei (Editor und Vorschau), und `die_menge_der_eigenen_textflaechen_steht_an_genau_einer_stelle` (`crates/krk-ui/src/appkit/ereignisse.rs`) schreibt in ihrem Doc-Kommentar aus, warum sie die Zahl der Ausnahmen bewusst **nicht** zaehlt: eine dritte eigene Flaeche waere ein dritter Vergleich an derselben Stelle und eine zulaessige Aenderung, keine zweite Fassung. Der Nachsatz am Spec (`planning/260813-2348_o_spec-…`, Punkt 2) haelt beides fest. Gemessen mit `grep -c "isEqual(Some(" crates/krk-ui/src/appkit/anwendung.rs` → 4, davon zwei in `ist_eigene_textflaeche`.
