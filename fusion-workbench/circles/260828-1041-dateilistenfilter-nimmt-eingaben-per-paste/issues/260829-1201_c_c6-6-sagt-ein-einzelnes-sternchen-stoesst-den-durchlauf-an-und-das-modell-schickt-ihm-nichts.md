...
---
Resolved: Als Nachsatz am Spec berichtigt (`planning/260829-1052_c_spec-…`), nach der Nutzerentscheidung zu `shared/decisions/260906-0203_*` — der Datensatz hatte die Berichtigung unter Berufung auf die Ortsregel ausgeschlossen; die Nutzerentscheidung vom 260906 laesst sie als Nachsatz zu. Der Nachsatz trennt die beiden Haelften von C6.6 und B6: der Kurzschluss ueber den Namen in `Ordnermodell::zeilengrund_von` (`grep -n "fn zeilengrund_von" crates/krk-core/src/verzeichnis/modell.rs` → `:812`) steht vor dem Unterbaumzweig, bei `*` bleibt `auftraege()` leer, und der Durchlauf bekommt nichts; die zweite Haelfte haelt mit von Hand gegebenen Auftraegen. Beides so, wie es ist, haelt `ein_einzelnes_sternchen_stoesst_den_durchlauf_an_und_entscheidet_jeden_ordner_mit_dem_ersten_eintrag` (`crates/krk-core/tests/verzeichnis.rs:2484`). Das Verhalten ist guenstiger als beschrieben.
