...
---
Resolved: Behoben auf dem Weg, den der Doc-Kommentar von `ohne_die_eigenen_ziele` schon nannte: geordnet wird nach der gefalteten Laenge und nicht nach der geschriebenen. Die neue Funktion `faltlaenge` (`crates/krk-ui/src/kommandos/kontextmenue.rs`) misst genau das, was `gleicher_eintrag` vergleicht, naemlich den kleingeschriebenen letzten Bestandteil; die Sortierzeile nimmt sie. Damit ist die Ordnung wieder ein Beweis: der Zielname ist der um `ENDUNG` gekuerzte Archivname, die vier Bytes von `.zip` sind ASCII und ueberstehen das Kleinschreiben unveraendert, und die einzige laengenrelevante Kontextabhaengigkeit der Umschrift, das Schluss-Sigma, liefert mit `ς` und `σ` zwei gleich lange Zeichen; der Beansprucher ist gefaltet immer genau vier Bytes laenger als der Beanspruchte. Die zweite Frage des Datensatzes — ob die Ordnung ueberhaupt an der Laenge haengen muss — ist damit beantwortet: sie darf, sobald sie dieselbe Laenge misst wie der Vergleich. Neue Probe `drei_kelvinzeichen_kehren_die_ordnung_des_entpackschnitts_nicht_um` mit dem Fall aus dem Datensatz (`KKK.zip` mit drei `U+212A` neben `kkk.zip.zip`), mutationsgeprueft: mit der alten Sortierzeile wird sie rot. Geprueft: cargo test -p krk-ui Exit 0.
