...
2. [DONE] **`Art::Zippen` und der Packlauf**
   - Executor: `coder`
   ...
   - *Nachtrag 260825-0838: `make check` Exit 0; zwölf neue Proben in `tests/operation.rs`, eine in `auftrag.rs`. **Zwei Vorgaben des Schritts halten in `zip 8.6.0` nicht wie geschrieben.** Erstens setzt `unix_permissions(0o120777)` das Kennzeichen einer Verknüpfung **nicht**: der Rumpf der Kiste maskiert mit `& 0o777` und wirft die oberen Modusbits fort, ein so abgelegter Eintrag wäre eine gewöhnliche Datei mit dem Pfad als Inhalt. Gesetzt wird `S_IFLNK` von `ZipWriter::add_symlink`, und dieser Weg ist genommen; die Wahl nennt `0o120777` weiter, weil `add_symlink` die Rechte nur ergänzt, wenn keine dastehen. Zweitens trägt jeder Eintrag den 1. Januar 1980 statt des Änderungsdatums der Quelle, weil `DateTime::default_for_write` ohne das Merkmal `time` einen festen Wert liefert; das ist als Defekt abgelegt (`issues/260825-0838_*_jeder-gepackte-eintrag-traegt-den-1-januar-1980-*`) und nicht in diesem Schritt behoben, weil die Umrechnung eine Zeitzone braucht, die keine der eingebundenen Kisten liefert. Ein selbst gepacktes Archiv öffnet `/usr/bin/unzip` fehlerfrei und legt die Verknüpfung als Verknüpfung an. Beleg: `history/260825-0838-coder-art-zippen-und-der-packlauf.md`.*

3. [DONE] **`Art::Entpacken` und der Entpacklauf**
   - Executor: `coder`
   ...
   - *Nachtrag 260825-0955: `make check` Exit 0; vierzehn neue Proben, zwölf in `tests/operation.rs` und zwei in `auftrag.rs`. **Drei Vorgaben dieses Schritts halten nicht, wie sie geschrieben stehen.** Erstens trägt `Art::Entpacken` eine **Liste** `ziele: Vec<PathBuf>` und keinen einzelnen `ziel: PathBuf`: die jüngere Nutzerentscheidung `decisions/260825-0727_*_nimmt-unzip-die-betroffenen-eintraege-*` (Möglichkeit 3) lässt einen Vorgang mehrere Archive tragen, und drei markierte Archive brauchen drei Ziele. Die Form ist die des Stapel-Umbenennens: `Auftrag::entpacken(paare)` trennt Paare auf, `entpackziel(stelle)` liest sie zurück, `einen_abarbeiten` hat die Stelle bereits. Zweitens ist die Reihenfolge umgekehrt: **erst das Archiv öffnen, dann den Zielordner klären**, denn „überschreiben" räumt hier einen ganzen Ordnerbaum in den Papierkorb, und ein Archiv, das keines ist, hätte ihn für nichts gekostet; die Zusage „die Rückfrage kommt, bevor ein Eintrag geschrieben wird" hält unverändert. Drittens genügt `enclosed_name` allein nicht: zwei Einträge, deren Namen je für sich im Zielordner liegen (`hinaus -> ..`, dann `hinaus/draussen.txt`), kämen daran vorbei, und deshalb steht daneben `kette_anlegen`, das jeden Weg durch eine Verknüpfung abweist. Zwei Proben halten beide Ausbruchswege. Beleg: `history/260825-0955-coder-art-entpacken-und-der-entpacklauf.md`.*

4. [DONE] **Die Regel des Kontextmenüs, ohne AppKit**
   - Executor: `coder`
   ...
   - *Nachtrag 260825-0927: `make check` Exit 0; achtzehn neue Proben in `kontextmenue.rs`, fünf in `operationen.rs`, zusammen 780 statt 757. **Sieben Vorgaben dieses Schritts halten nicht, wie sie geschrieben stehen.** Erstens gibt es `krk_core::operation::namen_teilen` unter diesem Pfad nicht; der Baum trägt die Trennung als `krk_core::operation::umbenennen::namen_teilen`, und dieser Pfad ist genommen statt einer zweiten Wiederausfuhr. Zweitens trennt `archivname` gar nichts: die anhängende Regel des Nutzers braucht Stamm und Endung nicht auseinander, und die eine Trennung des Paares steht allein in `ordnername_zum_archiv` und `ist_zipname`. Drittens ist `Entpackbefund::Archiv(PathBuf)` zu `Archive(Vec<(PathBuf, PathBuf)>)` geworden, passend zu `Art::Entpacken { ziele }` aus Schritt 3; `Mehrere` gilt seitdem allein der Ersatzregel. Viertens heißt die Umrechnung `menuemarke` und nicht `marke`: die kurze Fassung ließ die bestehende Probe `abwurfregel::tests::die_marke_hat_genau_einen_aufrufer` mit sechs Aufrufern rot werden, weil jene Zählung über den Namen im Quelltext läuft und zwei Begriffe desselben Namens nicht trennt. Fünftens trägt das Modul bis Schritt 7 `#![cfg_attr(not(test), expect(dead_code, …))]` und tragen die vier Funktionen in `operationen.rs` je ein `expect` — `krk-ui` hat kein Bibliotheksziel, also meldet der Übersetzer jedes noch ungerufene Stück, und `-D warnings` hält den Bau an. Sechstens hängt `erzeugt_genau_ein_ziel` beim Entpacken an `ziele.len() == 1` und nicht am Wert, weil ein Vorgang seit der dritten Nutzerentscheidung mehrere Archive tragen kann. Siebtens trug der Modulkopf von `kommandos/mod.rs` die Zahl „Zehn Module", die mit diesem Modul falsch geworden wäre; sie ist gefallen und durch die Anweisung ersetzt, sie mit `grep -c '^pub mod'` zu zählen. **Die drei bindenden Datensätze behalten ihren Marker `_a_`:** ihre Antworten sind hier gerechnet, aber erst mit dem Menüeintrag aus Schritt 6 und seinem Ausführungszweig aus Schritt 7 in Code umgesetzt. Beleg: `history/260825-0927-coder-die-regel-des-kontextmenues.md`.*

5. [DONE] **Ein Weg zur benannten Anwendung, für Terminal und Finder**
   - Executor: `coder`
   ...
   - *Nachtrag 260825-1048: `make check` Exit 0, mit der gleichzeitigen Änderung des zweiten Coders in `tabelle.rs` im Baum. `ordner_oeffnen` ist Zeile für Zeile unverändert. **Drei Vorgaben dieses Schritts halten nicht, wie sie geschrieben stehen.** Erstens sind es acht Fundstellen des alten Namens und nicht vier: vier ist die Zahl der Dateien, `operationen.rs` allein trägt fünf (Definition, Doc-Verweis der Probe, drei Aufrufe). Alle acht sind umgezogen; die Abweichung ist folgenlos, weil ein übersehener Name nicht übersetzt hätte. Zweitens nannte der Doc-Kommentar der Funktion das Terminal sehr wohl, anders als der Schritt annimmt — seine Kopfzeile lautete „den der Terminal-Befehl uebergeben soll", und sein letzter Absatz sprach von der Terminal-Sitzung; für die zwei gelieferten Sätze stimmt die Annahme, für den Kommentar nicht, und beide Stellen sind mitgezogen. Drittens heißt die Probe weiter `ein_fehlender_terminalordner_nennt_den_pfad`: der Schritt nennt sie nicht, ihr Prüfordner heißt so, und der geschlossene Datensatz `archive/260819-1613-safe-cleanup-tier-1/shared/issues/260810-1753_c_*` führt sie unter diesem Namen. **`ordner_fehlt` hat nach diesem Schritt weiter genau einen Aufrufer**; der zweite kommt mit Schritt 7. Modulkopf und Doc-Kommentar nennen deshalb keine Zahl, sondern die Frage. Beleg: `history/260825-1048-coder-ein-weg-zur-benannten-anwendung.md`.*

6. [DONE] **Der Menübau in der Dateiliste**
   - Executor: `coder`
   ...
   - *Nachtrag 260825-1058: `make check` Exit 0, 785 Proben in `krk-ui` statt 784; der Lauf schließt die inzwischen als `dd80e81` eingecheckte Arbeit des Schritts 5 mit ein. **Eine Vorgabe dieses Schritts steht gegen eine stehende Zusage der Runde 7, und die Zusage hat gewonnen.** Der Schritt verlangt, `tabelle.rs` belege `NSMenuItem::initWithTitle:action:keyEquivalent:`, rufe den Erzeuger also selbst; C2.10 der Runde 7 sagt zu, dass genau **eine** Stelle im Quellbaum ein `NSMenuItem` anlegt, und die Probe `appkit::menue::tests::es_gibt_eine_stelle_je_anlage_und_uebersetzung` ließ den ersten Bau nach dem Wortlaut mit zwei Fundstellen rot werden. Genommen ist die vorhandene Hülle: `appkit::menue::ohne_kuerzel` steht von dateiprivat auf `pub(super)` und wird gerufen — jene Probe schreibt in ihrem Kopf ausdrücklich, dass die Hüllen nicht mitzählen. **Damit ist `crates/krk-ui/src/appkit/menue.rs` eine zweite angefasste Datei**, beschränkt auf die Sichtbarkeit und zwei Erläuterungen. Der Modulkopf belegt entsprechend `NSMenuItem` selbst, `tag`, `target` und `NSMenu::addItem:`; der Erzeuger und `setAction:` stehen mit Grund **nicht** darin, weil der Selektor durch die Hülle geht. Daneben zwei kleinere Abweichungen: die Rückrechnung heißt `von_menuemarke` und nicht `von_marke` (schon im Nachtrag zu Schritt 4 vermerkt), und der Setzer `kontextmelder_setzen` trägt bis Schritt 7 ein `#[expect(dead_code, …)]` ohne `cfg_attr(not(test), …)`, denn eine Probe kann ihn nicht rufen: sie bräuchte den Hauptfaden. **Eine Probe ist entgegen der Abnahmezeile doch entstanden**, und sie prüft nicht die Form des Menüs, sondern die Falle: `sel!` fragt nicht, ob eine Methode den Namen beantwortet, und `der_kontextmenue_selektor_hat_einen_empfaenger_und_einen_setzer` zählt die Erklärung und das Setzen des Selektors einzeln über den Quellbaum. Beleg: `history/260825-1058-coder-der-menuebau-in-der-dateiliste.md`.*

7. [DONE] **Die Ausführung beim Anwendungsdelegierten**
   - Executor: `coder`
   ...
   - *Nachtrag 260825-1129: `make check` Exit 0; 792 Proben in `krk-ui` statt 790, die zwei neuen im Modul `kontextproben`. Die Kette schließt: der neunte Rückruf der Dateiliste steht in `oberflaeche_aufbauen`, und `kontextbefehl_ausfuehren` verzweigt über die drei Werte ohne Auffangzweig auf `zipauftrag_stellen`, `entpackauftrag_stellen` und `im_finder_zeigen`. **Vier Vorgaben des Schritts halten nicht, wie sie geschrieben stehen.** Erstens ist `crates/krk-ui/src/appkit/tabelle.rs` eine zweite angefasste Datei: `entpackziel` verlangt das `Ordnermodell` der sichtbaren Zeilen, das in den dateiprivaten `QuelleIvars` liegt, und der Delegierte kommt ohne einen zweiten Weg daran vorbei nicht heran; gebaut ist `DateifensterQuelle::entpackbefund()` nach dem Vorbild des vorhandenen `betroffene_eintraege()`, also eine Ausleihe und keine zweite Regel. Zweitens sind es acht Rückrufe neben dem neuen und nicht sechs; die Form ist von ihnen übernommen, die Abweichung folgenlos. Drittens sind die zwei Zählungen andere: `vorgang_laeuft` steht jetzt auf **sechs Wegen, fünf und einem** — Zip und Unzip fragen je für sich, weil der Finder-Zweig nicht fragen darf —, und `auftrag_starten` bekommt den fünften **und** sechsten Rufer; daneben ist in `loeschen_nach_rueckfrage` „für alle drei Frager" auf „für jeden ihrer Frager" gestellt. Viertens verfallen fünf `expect`-Marken und nicht eine: neben der an `kontextmelder_setzen` die am ganzen Modul `kommandos/kontextmenue.rs` und je eine an `nichts_zu_packen`, `kein_archiv`, `mehrere_archive` und `kein_finder`; alle fünf sind weg, an ihrer Stelle steht je ein Satz mit dem Rufer. `ordner_fehlt` hat seinen zweiten Aufrufer bekommen; Prosa war dort keine nachzuziehen, weil Schritt 5 bewusst keine Zahl gesetzt hat. **Die fünf Entscheidungsdatensätze behalten ihren Marker `_a_`:** dieser Schritt hat auftragsgemäß nicht committet, und ein `Implemented:` ohne Hash wäre kein Beleg. Beleg: `history/260825-1129-coder-die-ausfuehrung-beim-anwendungsdelegierten.md`.*

8. [DONE] **Das Konfliktblatt bei genau einer Zieldatei**
   - Executor: `coder`
   ...
   - *Nachtrag 260825-1140: `make check` Exit 0; 790 Proben in `krk-ui` statt 785, davon fünf neue in `konflikt.rs`. Die Antwort auf den Datensatz ist Möglichkeit 2, und sie steht so im Baum: `schaltflaechen(genau_ein_ziel)` legt beide Gestalten als **eine** Angabe an, `antwort(stelle, genau_ein_ziel, name)` trägt die zwei Tafeln, und jede Tafel hat ihre Probe, die die Beschriftung an der Stelle gegen die zurückgerechnete `Konfliktantwort` hält statt gegen eine zweite Aufzählung im Probenrumpf. Die Erläuterung ist als reine Funktion `tastenhinweis` herausgezogen, damit der Satz je Gestalt genau einmal dasteht und prüfbar ist. **Vier Abweichungen von der Schrittbeschreibung.** Erstens ist `crates/krk-ui/src/kommandos/operationen.rs` eine dritte angefasste Datei: `erzeugt_genau_ein_ziel` trug seit Schritt 4 ein `#[cfg_attr(not(test), expect(dead_code, …))]`, und `unfulfilled_lint_expectations` hielt unter `-D warnings` den Bau an, sobald der Aufrufer dastand; das Attribut ist gefallen, der Doc-Kommentar nennt jetzt seinen einen Rufer. Zweitens bekommt `konflikt_fragen` die `Art` als zweiten Übergabewert, weil es sie nicht hatte; gefragt wird `erzeugt_genau_ein_ziel` wie vorgesehen dort und nicht am Blatt, das die `Art` nicht kennen soll. Drittens fragt `fuer_alle_weiteren` nicht mehr die Stelle (`stelle != 3`), sondern die Antwort: die Stelle des Abbruchs ist je Gestalt eine andere. In der vollen Gestalt ist das verhaltensgleich bis auf eine Stelle jenseits der vierten, die es nach `Blatt::zeigen_mit_wahl` nicht geben kann und die vorher ein angekreuztes Kästchen an einen Abbruch gehängt hätte. Viertens trägt die gekürzte Gestalt an keiner Schaltfläche `Taste::Escape`; die Escape-Taste erreicht „Abbrechen" über den Abbruchbefehl und `Blattgriff::abbrechen`, wie es die Löschbestätigung seit der Runde 12 vormacht, und `abbruchstelle` und `bestaetigungsstelle` fallen dort auf dieselbe Schaltfläche. **Der Datensatz behält vorerst seinen Marker `_a_`:** die Umsetzung steht im Baum, aber uncommitted, und ein `Implemented:` ohne Commit-Hash wäre kein Beleg. Beleg: `history/260825-1140-coder-das-konfliktblatt-bei-genau-einer-zieldatei.md`.*
