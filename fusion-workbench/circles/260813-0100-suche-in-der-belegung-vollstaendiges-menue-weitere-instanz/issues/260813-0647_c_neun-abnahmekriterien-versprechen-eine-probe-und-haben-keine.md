...
---
Resolved: Drei der neun haben jetzt ihre Probe, und fuer die uebrigen sechs steht im Nachsatz des Plans (`planning/260813-0205_c_plan-…`, Punkt 3) je der Grund, warum keine entsteht. Gebaut: `die_anwendungsweiten_befehle_wirken_aus_jedem_bereich_heraus` (`crates/krk-core/tests/belegung.rs`) fuer C3.3 — sie nimmt `Beenden`, `WeitereInstanz`, `Notizzettel`, `BelegungAnsehen` und `BelegungsdateiAnsehen` zusammen, weil die fuenf im Quelltext in demselben Zweig und aus derselben Begruendung stehen; `das_hauptfenster_entsteht_an_genau_einer_stelle` (`crates/krk-ui/src/appkit/fenster.rs`) fuer C3.12 — sie zaehlt, dass `hauptfenster` und `oberflaeche_aufbauen` je genau einen Rufer haben, also den Weg vom Programmstart zum Fenster; `beide_sperrgriffe_der_ablage_tragen_must_use_mit_begruendung` (`crates/krk-core/tests/baum.rs`) fuer C4.8. Die sechs ohne Probe: C2.12 und C2.13 verlangen das laufende Buendel, C2.15 hat der Plan selbst auf (Buendel) verschoben, C4.1 und C4.3 sind Aussagen ueber einen Diff jener Runde und heute nicht mehr zu stellen, C4.4 haengt an `shared/decisions/260811-2050_*_wird-die-untergrenzen-angabe-pruefbar-gemacht.md`. Von den acht teilweise gedeckten Zeilen ist C2.5 inzwischen ganz gedeckt (`die_ausnahmeliste_fuehrt_dieselben_drei_befehle_wie_vor_dieser_runde`). `make check` exit 0.
