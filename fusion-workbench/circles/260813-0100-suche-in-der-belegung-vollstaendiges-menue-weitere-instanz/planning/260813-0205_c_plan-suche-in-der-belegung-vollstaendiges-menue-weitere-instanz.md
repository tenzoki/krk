...
**Spec:** `shared/planning/260813-0053_*_spec-suche-in-der-belegung-vollstaendiges-menue-zweite-instanz.md` (Fassung nach dem Nachzug vom 260813-0130)
...
- **Setzt voraus:** die Empfehlung von `shared/decisions/260813-0053_*_schluckt-der-abgriff-den-zulaessigen-befehl-oder-den-ausgefuehrten.md` (Möglichkeit 1), soweit sie die drei Bestandteile und die Ausnahmeliste betrifft.
...
- **Setzt voraus:** die Empfehlung von `shared/decisions/260813-0053_*_schluckt-der-abgriff-den-zulaessigen-befehl-oder-den-ausgefuehrten.md` (Möglichkeit 1), diesmal in ihrem Kern.
...
- **Setzt voraus:** die Empfehlung von `decisions/260813-0159_*_darf-das-menue-die-eine-gliederung-umsortieren-und-umbenennen.md` (Möglichkeit 1).
...
- **Aufzählung:** keine wächst.
- **Setzt voraus:** die Empfehlung von `shared/decisions/260813-0053_*_wie-viele-obermenues-traegt-die-menueleiste-fuer-81-funktionen.md` (Möglichkeit 1: neun Obermenüs) und die Empfehlung aus S4.
...
- **Kommentar mitlesen:** der Absatz an `Blatt::mit_schaltflaechen` (`crates/krk-ui/src/appkit/blaetter/mod.rs:401-404`) nennt eine Variante `Taste::Keine`, die die Aufzählung nicht führt. Der Defekt ist gemeldet (`issues/260813-0201_*_ein-kommentar-in-blaetter-mod-rs-nennt-eine-taste-variante-die-es-nicht-gibt.md`) und gehört nicht in diesen Schritt; wer die Zeile ändert, liest ihn und lässt ihn stehen.
- **Aufzählung:** keine wächst.
- **Setzt voraus:** die Empfehlung von `shared/decisions/260813-0053_*_welche-tasten-behalten-die-schaltflaechen-der-belegungsansicht-wenn-jedes-zeichen-sucht.md` (Möglichkeit 1).
...
- **Setzt voraus:** die Empfehlung von `shared/decisions/260813-0053_*_was-teilen-sich-zwei-instanzen-an-der-ablage-und-wer-schreibt-die-sitzung.md` (Möglichkeit 1).
...
- [ ] Welche Tasten behalten die Schaltflächen der Belegungsansicht? `shared/decisions/260813-0053_*_welche-tasten-behalten-die-schaltflaechen-der-belegungsansicht-wenn-jedes-zeichen-sucht.md`, Empfehlung Möglichkeit 1. Betrifft S9.
- [ ] Wie viele Obermenüs trägt die Menüleiste? `shared/decisions/260813-0053_*_wie-viele-obermenues-traegt-die-menueleiste-fuer-81-funktionen.md`, Empfehlung Möglichkeit 1. Betrifft S5 und S6.
- [ ] Was teilen sich zwei Instanzen an der Ablage? `shared/decisions/260813-0053_*_was-teilen-sich-zwei-instanzen-an-der-ablage-und-wer-schreibt-die-sitzung.md`, Empfehlung Möglichkeit 1. Betrifft S11 bis S14.
- [ ] Schluckt der Abgriff den zulässigen oder den ausgeführten Befehl? `shared/decisions/260813-0053_*_schluckt-der-abgriff-den-zulaessigen-befehl-oder-den-ausgefuehrten.md`, Empfehlung Möglichkeit 1. Betrifft S1 bis S3 und S6.
- [ ] Darf das Menü die eine Gliederung umsortieren und einen Bereich umbenennen? `decisions/260813-0159_*_darf-das-menue-die-eine-gliederung-umsortieren-und-umbenennen.md`, Empfehlung Möglichkeit 1. Betrifft S4, und über S4 auch S5 und S6.
...
Ein Defekt ist während der Planung aufgefallen und gemeldet: `issues/260813-0201_*_ein-kommentar-in-blaetter-mod-rs-nennt-eine-taste-variante-die-es-nicht-gibt.md`. Er gehört nicht in diese Runde; S9 fasst die Stelle an und lässt ihn stehen.
...
- **Kein Nachziehen der veralteten Zahlen in `CLAUDE.md`.** Die Datei nennt 68 Varianten für `Kommando` und 33 Dateien unter `appkit/`; beide Befunde haben eigene offene Datensätze (`shared/issues/260812-2253_*_claude-md-nennt-fuer-kommando-68-varianten-der-baum-traegt-75.md` und `260812-1438_*_claude-md-nennt-31-von-33-dateien-…`), und diese Runde verschiebt beide Zahlen erneut. Das Nachziehen gehört an den Schluss der Runde und nicht in einen ihrer Schritte.
...
Der Plan behält Namen, Ort und Marker. Der Abschnitt `## Was aus den zwei Diagrammprüfungen in diesen Plan eingegangen ist` bleibt als Aufzeichnung des damaligen Standes stehen; er trägt nur einen Verweis auf diesen Nachtrag. Die offene Frage zur Menügliederung (`decisions/260813-0159_*_darf-das-menue-die-eine-gliederung-umsortieren-und-umbenennen.md`) bleibt offen, und die fünf Datensätze unter `## Offene Fragen` sind unverändert. Die Zeile **Decidability** im Kopf gilt weiter: die tragende Frage wird aus gelesenen Eingaben beantwortet, und nach dieser Änderung liest sie ein Mechanismus statt zweier.

---

## Reconciliation Log

**Abgleich 260813-0647, reconciler, Domäne `code`.** Stand `1cd7788`, Sitzungsspanne
`188b81a..HEAD`, 16 Commits.

### Was geprüft ist

**Alle fünfzehn Schritte stehen auf `[DONE]`, und der Baum hält, was sie zusagen.**
`cargo test --workspace` und `cargo clippy --workspace --all-targets -- -D warnings` laufen am
260813 grün, exit 0. Die Zahl „1003 Proben über 19 Ziele" aus der Commit-Nachricht von
`dff167a` entsteht durch naives Zählen der Ergebniszeilen; der Workspace führt **16** Ziele
(eine Bibliothek, drei Binärprogramme, zwölf Prüfziele) mit **1001** Proben, dazu fünf
übersprungene Kindproben in `tests/ablage.rs` und zwei Kindprozessläufe, die sich selbst noch
einmal aufrufen. Beide Zählweisen sind vertretbar; grün ist der Baum in beiden.

**Die vier Zahlen aus C4 stimmen exakt**, von Hand nachgezählt: `Kommando` 75 → 76,
`Wirkungsbereich` 7, `Bereich` 5, `Fokus` 5, `Funktionsbereich` 9 — vier davon unverändert wie
zugesagt. `resources/default-keymap.toml` führt 82 Funktionen mit 88 Kombinationen, und die
Zählzeile im Kopf nennt beide. `opt+cmd+n` war vorher unbelegt; der Diff über alle `id`- und
`tasten`-Zeilen zwischen `188b81a` und HEAD zeigt genau zwei hinzugefügte Zeilen und keine
geänderte. `#![deny(unsafe_code)]` steht an allen drei Kistenwurzeln, die Ausnahme tragen
weiterhin genau zwei Dateien.

**Die tragende Regel der Runde hält.** `zulaessig` ist genau einmal erklärt
(`crates/krk-ui/src/kommandos/zulaessigkeit.rs:113`) und hat außerhalb ihres Moduls genau zwei
Aufrufstellen, `crates/krk-ui/src/appkit/anwendung.rs:741` (`validateMenuItem:`) und `:2600`
(`kommando_ausfuehren`). Das ist C2.16 in beiden Hälften.

### Drift, die der `[DONE]`-Marker nicht trägt

**S3 hat seine eigene Halteregel überstimmt.** Der Schritt sagt: „Findet sich einer, der etwas
erreicht, hält der Schritt an und meldet ihn, statt die Regel trotzdem zu setzen." Die Zählung
hat genau einen solchen Fall gefunden — `esc` mit dem Fokus im Editor —, und der Schritt ist
nicht angehalten worden. Die Abweichung ist nicht still geblieben: sie steht in
`decisions/260813-0320_*_esc-im-editor-erreicht-heute-die-textflaeche-und-wird-nach-s3-geschluckt.md`
unter „Warum trotzdem gebaut", und der Datensatz ist offen. Der Schritt trägt trotzdem
`[DONE]`, und das ist die richtige Auskunft über den Code und die unvollständige über das
Verfahren.

**Die Abnahmeliste für den Lauf am Bündel ist unvollständig.** Weder der Verlust an `esc` im
Editor noch der Klick in die Bereichsleiste während einer Umbenennung steht darauf; die
Wörter „Bereichsleiste", „Zusammensetzung", „Eingabemethode" und „Vorschau-Schalter" kommen im
ganzen Plan nicht vor. Abgelegt als
`issues/260813-0642_*_zwei-hingenommene-verluste-stehen-auf-keiner-abnahmeliste.md`, samt der
Feststellung, dass die Spec-Randbedingung „Kein Verlust gegenüber heute" damit zweimal
verletzt und nicht nachgezogen ist.

**Neun Kriterien versprechen eine Probe und haben keine**, acht sind nur teilweise gedeckt.
Kein Kriterium ist falsch — alle sind nachgeprüft und treffen zu —, aber die Kennzeichnung
`(Probe)` sagt Abnehmbarkeit zu, die neunmal nicht eingelöst ist. Abgelegt als
`issues/260813-0647_*_neun-abnahmekriterien-versprechen-eine-probe-und-haben-keine.md`.

### Zwei tote Verweise, berichtigt

Die Zeilen 318 und 505 zitierten `issues/260813-0201_o_ein-kommentar-in-blaetter-mod-rs-…md`
mit ausgeschriebenem Marker. Die Datei trägt seit Turn 2 `_c_`, beide Verweise waren tot. Sie
stehen jetzt in Sternform. Das ist derselbe Fehler, den die Runde 6 zweimal gemacht hat
(`shared/issues/260812-2253_*_sieben-verweise-im-circle-datensatz-der-runde-5-…md`); der
Circle-Datensatz dieser Runde zitiert dagegen durchgehend in Sternform und ist sauber.

Vierzehn weitere Verweise in Spec und Plan nennen die vier Fragen vom 260813-0053 mit
ausgeschriebenem `_o_`. Sie leben heute, und sie sterben in dem Augenblick, in dem der Nutzer
eine davon beantwortet.

### Warum der Plan trotzdem `_c_` bekommt

Der Marker steht für den Plan und nicht für die Runde. Alle fünfzehn Schritte sind ausgeführt
und einzeln gegen den Baum gelesen; die vier offenen und die fünf neuen Datensätze führen die
Arbeit, die dahinter noch liegt. Die Runde selbst schließt als **beschränkter Abschluss**, wie
