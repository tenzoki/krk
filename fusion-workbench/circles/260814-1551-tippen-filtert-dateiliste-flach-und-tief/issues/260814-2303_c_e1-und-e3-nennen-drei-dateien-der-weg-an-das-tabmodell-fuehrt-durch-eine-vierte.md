...
---
Resolved: Als Nachsatz am Plan berichtigt (`planning/260814-2102_c_plan-…`, Punkt 1), nach der Nutzerentscheidung zu `shared/decisions/260906-0203_*`, und zwar gemeinsam mit den vier weiteren Faellen desselben Musters, wie der Schwesterdatensatz `260814-2357_*` es empfiehlt: eine Berichtigung ist billiger als fuenf. Die fehlende Datei ist `crates/krk-ui/src/appkit/tabelle.rs`, weil die Tabliste im modulprivaten Ivar `QuelleIvars::tabs` steht. Am Baum belegt mit `grep -n "pub fn tiefe_suche_umschalten\|pub fn filter_steht\|pub fn letztes_filterzeichen_weg" crates/krk-ui/src/appkit/tabelle.rs` → `:2989`, `:3104`, `:3134`, die drei oeffentlichen Methoden, die die Umsetzer dort angelegt haben, neben `verstecke_umschalten` (`:2959`).
