...
---
Resolved: Die Probenhaelfte von C1.7 hat einen Traeger. Gebaut sind zwei Zaehlproben am Quelltext im Modul `abbruchrangfolge` von `crates/krk-ui/src/kommandos/operationen.rs`, also an dem Ort, den dieser Befund vorgeschlagen hat — neben `waehrend_blatt_erlaubt` und `abbruchzeile`, den zwei anderen reinen Stuecken desselben Befehls; eine sechste Datei unter `kommandos/` entsteht nicht. `die_drei_raenge_stehen_in_der_zugesagten_reihenfolge` liest den Rumpf von `Anwendungsdelegierter::abbrechen` und haelt die Fundstellen von `offenes_blatt`, `ivars().vorgang` und `filter_leeren()` in dieser Ordnung; `die_zwei_vorderen_raenge_springen_frueh_zurueck` zaehlt die zwei `return true;` vor dem dritten Rang und dass der dritte genau einmal dasteht. **Es ist der dritte Weg und nicht einer der zwei, die der Befund vorlegt**: gemessen wird die Reihenfolge, und genau die sagt C1.7 zu. Damit entsteht weder ein siebter Typ der Runde noch holt ein Zweig seinen Wert ein zweites Mal, und die Entwurfsfrage, ob die Rangfolge eine reine Funktion werden soll, bleibt beim Planer und ist mit dieser Probe nicht entschieden. Was jeder Rang **tut**, misst weiterhin die Buendelhaelfte in G2. Der Nachsatz am Spec (`planning/260814-1830_o_spec-…`, Punkt 2) haelt es fest. `make check` exit 0.
