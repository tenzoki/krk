...
---
Resolved: Als Nachsatz am Spec berichtigt (`planning/260814-1830_o_spec-…`, Punkt 1), nach der Nutzerentscheidung zu `shared/decisions/260906-0203_*`. Die Begruendung "denn der Nutzer navigiert in sie hinein" trifft am 260906 unveraendert nicht zu: `grep -n -A4 "fn ist_ordner" crates/krk-core/src/verzeichnis/eintrag.rs` → `:115-117`, `self.typ == Typ::Ordner`, und `crates/krk-ui/src/appkit/tabelle.rs:461` fragt genau diese Funktion beim Doppelklick. Der Nachsatz nennt die tragende Regel statt der Begruendung: `Ordnermodell::sichtbar` zaehlt **jede** Verknuepfung als Ordner (`grep -n "ist_ordner() || .*ist_verknuepfung()" crates/krk-core/src/verzeichnis/modell.rs` → `:847`), gehalten von `eine_verknuepfung_zaehlt_fuer_die_sichtbarkeit_als_ordner`. Die zwei Entwurfsfragen des Befunds — ob eine Verknuepfung auf eine Datei bei flacher Suche stehen bleibt, und ob KRK in eine Verknuepfung hineinnavigiert — sind damit nicht entschieden; die zweite fuehrt `shared/issues/260814-1612_o_eine-verknuepfung-auf-einen-ordner-laesst-sich-nicht-betreten.md` weiter.
