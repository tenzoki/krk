...
- Die Zählung zeigt `mindestens {Treffer} (Lesung bei {HOECHSTENS_EINTRAEGE} Einträgen abgebrochen)`. Sie sagt beides, was die Teillesung entscheidet: dass es mindestens so viele sind wie die getroffenen, und dass die Lesung abgebrochen ist. Dass es **mehr** sind, sagt sie nicht, denn ein Treffer hinter dem Abbruch ist möglich und nicht gesichert. *(Hier stand „Die Zählung zeigt `über 2.000` statt einer Zahl. Sie kann sagen, dass es mehr sind."; berichtigt am 260824-1722. Ein Muster, auf das in 2.101 Einträgen genau einer passt, ergäbe aus der Konstanten „über 2.000 offene Defekte" und damit eine falsche Aussage. Die Befunde sind `issues/260824-1215_*_die-abgeschnittene-zaehlung-zeigt-ueber-treffer-und-c6-5-verlangt-ueber-2000.md` für den Bau und `issues/260824-1651_*_c6-5-a5-und-planschritt-6-sagen-weiter-ueber-2-000-und-kein-offener-datensatz-traegt-es.md` für die Buchführung; die Berichtigung von C6.5 und A5 steht im Spec unter der Kriterienliste von C6.)*
...
Sechs Orte, fünf Profile: die Speicherordner aus C5.2 und C5.3 fallen in eines. **Das Speicherprofil zählt seit der Antwort vom 260824-1505 neun Speichernamen auf und nicht sechs**; die drei zusätzlichen trägt Schritt 14 nach. Vier der neun, nämlich `backlog`, `consult`, `investigations` und `memos`, gibt es nach `rules/fusion-workbench-conventions.md:78` allein unter `shared/`, also stehen in einer Runde fünf davon. Wie viele Ordner das Profil trifft, sagt der Bestand und nicht diese Zeile: gemessen am 260824-1739 mit `regex` 1.13.1 gegen alle 154 Verzeichnisse unter `fusion-workbench/` sind es 99, davon 9 unter `shared/` und 90 in den achtzehn Runden. Die Zahl wächst mit jeder Runde. *(Hier stand „Es sind seit der Antwort vom 260824-1505 achtzehn und nicht zwölf, je neun im gemeinsamen Speicher und im Circle"; berichtigt am 260824-1751. Es ist dieselbe Rechnung, die der Befund `issues/260824-1651_*_der-kopf-des-speicherprofils-nennt-achtzehn-orte-und-neun-speicher-je-runde-es-sind-99-und-fuenf.md` im Kommentarkopf der Auslieferungsfassung berichtigt hat; geräumt war bis dahin nur der Kommentar.)*
...
| Defektspeicher | Pfad `fusion-workbench/(shared\|circles/[^/]+)/issues$` | vier Zählungen (Datensätze, offen, geschlossen, zurückgestellt), jüngste zehn (hier stand „zwei Zählungen (offen, geschlossen)"; die Gesamtzahl und die zurückgestellten kommen mit der Räumung der Durchsicht `reviews/260824-1700-ontorev-…` dazu, Schritt 7 hat die zwei gebaut) |
...
7. [DONE] **Die Auslieferungsfassung mit fünf Profilen und ihren Kommentarzeilen**
   ...
   - Changes: Die fünf Profile aus der Tabelle unter `### Die fünf mitgelieferten Profile`, in der Reihenfolge Wurzel, Speicher, Defektspeicher, Circle-Verzeichnis, einzelner Circle. Die Ausdrücke im Einzelnen: die Wurzel über das Kennzeichen `^\.fusion-setup$` mit den Feldmustern `"setup_pwd":"[^"]*/([^"/]+)"`, `"setup_at":"([^"]*)"`, `"plugin_version":"([^"]*)"` auf `^\.fusion-setup$`, `^([^\n]+)` auf `^\.active-circle$` und `(?s)## Current\n\s*(.+?)\n` auf `^orchestrator-live\.md$`, dazu zwei Zählungen auf `circles` (ohne Muster) und `shared/issues` (`_o_.*\.md$`). Der Speicher und der Defektspeicher über die Pfadmuster aus der Tabelle, mit `\.md$` als Zählmuster und `anzahl = 10`. Der einzelne Circle über das Kennzeichen `^_._circle\.md$`, mit den drei Zustandszeilen `^_a_circle\.md$`, `^_t_circle\.md$`, `^_[cb]_circle\.md$`, dem Feldmuster `(?sm)^## Directive\s*\n+(.+?)\n\n` auf `^_._circle\.md$`, zwei Vorhandensein auf `ordner = "planning"` mit `_._spec-` und `_._plan-`, einer Zählung auf `ordner = "decisions"` mit `\.md$` und den jüngsten zehn auf `ordner = "history"`. **Die Datei ist zur Hälfte Kommentar**, wie `resources/default-settings.toml` es ist: ein Kopf, der sagt, wofür die Datei da ist, dass KRK sie nach dem ersten Start nie wieder anfasst und dass sie von Hand gepflegt wird; ein Abschnitt, der **alle vier Bausteinnamen** nennt und je einen an einem kurzen Beispiel zeigt (C5.10); ein Abschnitt über die Vorrangregel aus C2, ausgeschrieben in ihren drei Schritten; und die Zahlen des Haushalts mit dem Satz, dass eine Zeile über der Grenze ihren Platzhalter zeigt. Die deutschen Umlaute stehen als Umlaute; die Datei ist Prosa für den Nutzer, kein Bezeichner. Jedes Muster steht in einfachen Anführungszeichen, also als literale Zeichenkette in TOML, damit die Rückstriche nicht zu maskieren sind. **Der Bausteinabschnitt trägt daneben den Satz zur Verankerung:** ein Feldmuster läuft über den ganzen Dateiinhalt, und wer eine Zeile verankern will, schreibt `(?m)`.
     ...
     **Nachtrag 260824-1751: zwei der hier vorgeschriebenen Angaben sind nach dem Bau ersetzt worden, und der Wortlaut dieses Schrittes bleibt trotzdem stehen.** Er hält den gebauten Stand vom 260824-1313 fest, wie es der Abschnitt `### Die fünf mitgelieferten Profile` für die vierte Zustandszeile schon sagt. Eine Anweisung, die die gebaute Datei nicht erfüllt, entstünde erst durch das Umschreiben, und der Ersatz gehört nicht diesem Schritt: die vierte Zustandszeile und die drei zusätzlichen Speichernamen sind Schritt 14, die zwei Angaben hier sind die Räumung der Durchsicht `reviews/260824-1700-ontorev-auslieferungsfassung-der-leseprofile.md`, gebaut mit `942172b`.

     **Erstens das Feldmuster der Zeile „Sitzung".** Hier steht `(?s)## Current\n\s*(.+?)\n`; gebaut ist seit `942172b` `## Current\n(?:[^\S\n]*\n)*[^\S\n]*([^#\n][^\n]*)`. Das alte Muster hatte zwei falsche Ausgänge: bei leerem `## Current`-Abschnitt fing sein gieriges `\s*` die nächste Überschrift und zeigte „Sitzung: ## This Turn", und ohne Zeilenende am Dateiende traf es gar nicht. **Der Vorschlag des Befundes ist ausdrücklich nicht übernommen worden**, weil er gegen neun Gestalten von `orchestrator-live.md` gemessen einen dritten falschen Ausgang hatte: auf eine Leerzeile unter `## Current` und darunter den Wert liefert er den Platzhalter, obwohl der Wert dasteht, und genau diese Gestalt schreibt `crates/krk-core/tests/leseprofil.rs` in `werkbankwurzel`. Nachgewiesen und nicht abgeleitet: mit dem Vorschlag in der Datei fällt die Probe `die_zwei_groessten_mitgelieferten_profile_bleiben_unter_ihren_zahlen`. Der gebaute Ausdruck hat dafür einen Preis, den die Kommentarzeilen der Auslieferungsfassung dem Nutzer sagen: `regex` kennt kein `(?!…)`, also trennt das erste Zeichen der Fanggruppe (`[^#\n]`) die Überschrift vom Wert, und eine Wertzeile, die selbst mit `#` beginnt, wird nicht gelesen. Der Befund ist `issues/260824-1650_*_die-zeile-sitzung-liefert-bei-leerem-current-abschnitt-die-naechste-ueberschrift-statt-des-platzhalters.md`.

     **Zweitens die jüngsten zehn auf `ordner = "history"`.** Sie tragen seit `942172b` zusätzlich `muster = '\.md$'`, wie die zwei anderen `juengste`-Zeilen der Datei. Ohne das Muster nahm die Zeile die `.gitkeep` in `circles/260804-0933-eingebauter-web-betrachter-im-vorschaufenster/history/` auf und führte sie als jüngsten Verlauf. Der Befund ist `issues/260824-1654_*_die-verlaufszeile-des-rundenprofils-traegt-kein-muster-und-listet-eine-gitkeep-als-verlauf.md`.

     **Der Haushalt ist von beiden Änderungen unberührt**, gemessen am 260824-1739: die Wurzel bleibt bei drei Verzeichnisleseläufen und fünf Dateiöffnungen, das Rundenprofil bei fünf und elf, und die Zahlen der Tabelle unter `### Die fünf mitgelieferten Profile` stimmen weiter.
   - Dependencies: Schritt 3

8. [DONE] **Die Ablagehälfte, nach der Vorlage der Einstellungen**
   - Executor: `coder`
...
14. [DONE] **Die vierte Zustandszeile und die drei zusätzlichen Speichernamen**
    - Executor: `ontocoder`
    - Files: `resources/default-readers.toml`
...
10. [DONE] **Der Anzeigezweig und der eine Weg, auf dem die Profile hereinkommen**
    - Executor: `coder`
...
11. [DONE] **Die Anwendung lädt die Profile im selben Durchgang und übergibt sie**
    - Executor: `coder`
...
13. [DONE] **Die falsche Kostenangabe wird berichtigt und der Defekt geschlossen**
    - Executor: `analyst`
    - Files: `circles/260823-2208-vorschau-zeigt-profil-zusammenfassung-statt-metadaten/decisions/260824-0541_*_wie-zieht-der-baustein-ein-feld-aus-einer-datei-und-traegt-er-auch-einen-abschnitt.md`, `circles/260823-2208-vorschau-zeigt-profil-zusammenfassung-statt-metadaten/issues/260824-0600_*_der-entscheidungsdatensatz-zum-regulaeren-ausdruck-sagt-der-baum-fuehre-keine-solche-kiste-er-fuehrt-eine.md`
...
- Der Defektdatensatz `issues/260824-0600_*_…` ist geschlossen, und die berichtigte Kostenangabe steht im Datensatz `decisions/260824-0541_*_wie-zieht-der-baustein-…`.
...
- Der Entscheidungsdatensatz `decisions/260824-0634_*_bekommt-das-circle-profil-eine-vierte-zustandszeile-…` ist beantwortet oder zurückgestellt.
...
|---|---|
| `260823-2208_*_gilt-ein-profil-nur-fuer-ordner-oder-auch-fuer-einzelne-dateien` | Schritt 9 (der Zusammenfassungszweig steht vor dem Ordnerzweig und greift auf keine Datei) |
| `260823-2208_*_liefert-krk-ein-fertiges-fusion-workbench-profil-mit` | Schritt 7 und Schritt 8 |
| `260824-0541_*_wie-zieht-der-baustein-ein-feld-aus-einer-datei-…` | Schritt 1, Schritt 3 und Schritt 6; die Kostenangabe berichtigt Schritt 13 |
| `260824-0541_*_was-heisst-die-juengsten-zehn-und-was-ist-ihr-titel` (Sortierhälfte) | Schritt 6 |
| `260824-0541_*_was-zeigt-die-zusammenfassung-wenn-ein-baustein-ins-leere-greift` | Schritt 6 (`Wert::Nicht`) und Schritt 3 (die abgewiesene Zeile) |
| `260824-0600_*_der-titel-aus-der-ueberschriftenzeile-…` (Titelhälfte) | Schritt 6 |
| `260824-0600_*_welche-form-hat-das-pfadmuster-und-welche-die-kennzeichendatei` | Schritt 3 und Schritt 5 |
| `260824-0600_*_woher-nimmt-die-wurzelzusammenfassung-ihre-sitzungsinfo` | Schritt 7 |
| `260824-0634_*_bekommt-das-circle-profil-eine-vierte-zustandszeile-…` | Schritt 14 |
| `260824-1313_*_deckt-das-speicherprofil-auch-decisions-memos-und-investigations-ab` | Schritt 14 |
...
**Marker `_o_` → `_p_`, und nicht auf `_c_`.** Alle vierzehn Schritte stehen auf `[DONE]`, und
jede behauptete Erledigung ist gegen den Baum gelesen und nicht gegen den Plantext. Geschlossen
ist der Plan trotzdem nicht: sein eigener Abschnitt `## Nutzerarbeit` führt sieben Kriterien mit
Bündelanteil, sein Abhängigkeitsgraph endet auf dem Knoten „Nutzerarbeit am laufenden Bündel",
und dieser Durchgang ist nicht gefahren. Dazu kommt, dass
`shared/decisions/260819-1440_*_was-sagt-der-marker-c-an-einem-spec-gebaut-oder-abgenommen.md`
offen ist. Dasselbe Vorgehen hat der Abgleich der Runde 14 am 260820-0834 gewählt; dort ist `_c_`
gesetzt worden, nachdem der Nutzer die Bündelabnahme gefahren hatte.

**`make check` läuft grün**, am 260824-1852 in einem Zug gefahren: `cargo build --workspace`,
`cargo test --workspace` (1520 Proben in 22 Zielen, 0 rot, 11 Kindproben übersprungen),
`cargo clippy --workspace --all-targets` ohne Beanstandung, `cargo fmt --all --check` ohne
Ausgabe.

**Die vierzehn Schritte, einzeln am Baum belegt:**

| Schritt | Commit | Beleg am Baum |
|---|---|---|
| 1 `regex` im Arbeitsbereich | `abecfb2` | `regex = "1"` (`Cargo.toml:108`) mit der Begründung darüber (`:58-106`), darunter die gemessene Zahl „`Cargo.lock` waechst von 97 auf 98 Eintraege" (`:81`); `regex = { workspace = true }` (`crates/krk-core/Cargo.toml:21`) |
| 2 `Datei::Leser`, die siebte | `ed893a4` | `pub const ALLE: [Datei; 7]` (`crates/krk-core/src/ablage/pfade.rs:226`), `Datei::Leser => "readers.toml"` (`:247`), `Format::Toml` (`:266`), `Leerbefund::Vorgabe` (`:309`), `Ersatz::Nichts` (`:330`); Namensliste `crates/krk-core/tests/ablage.rs:280` |
| 3 Gestalt und Prüfschritt | `f013227` | `Profildatei` (`crates/krk-core/src/leseprofil/datei.rs:99`), `Bausteindatei` als unmarkierte Auswahl (`:214`), `pruefen` (`:286`); neun Proben in `crates/krk-core/tests/leseprofil.rs:99-546`, darunter die fünfte Abweisung `::ein_verschriebener_schluessel_nennt_sich_in_der_meldung` (`:491`) |
| 4 `lesen_hoechstens` und `anlesen` | `b76800b` | `Lesestand` (`crates/krk-core/src/verzeichnis/leser.rs:169`), `lesen_hoechstens` (`:234`), und `lesen` (`:194`) ist auf eine Zeile geschrumpft: `lesen_hoechstens(pfad, usize::MAX)`; `anlesen` (`crates/krk-core/src/text/datei.rs:691`) an derselben Tür `sys::ohne_warten_oeffnen` (`:692`) |
| 5 Erkennung, zwei Durchgänge | `a327d08` | `erkennen` (`crates/krk-core/src/leseprofil/erkennung.rs:99`); sieben Proben (`tests/leseprofil.rs:586-874`), darunter `::der_erste_durchgang_ruft_den_abschluss_nicht` (`:728`), die die Bauart hält, aus der die Zahlen aus C6.7 fallen |
| 6 Bausteine, Haushalt, Teillesung | `abe1a31` | `zusammenfassen` (`crates/krk-core/src/leseprofil/bausteine.rs:142`), Sortierung nach `geaendert` mit Namen als zweitem Schlüssel (`:363`), `titelzeile` (`:486`), `anlesen`-Ruf mit `HOECHSTENS_BYTES` (`:497`); `Wert::als_text` (`leseprofil/mod.rs:571`) bildet den Satz der Teillesung aus `HOECHSTENS_EINTRAEGE` und keiner zweiten Zahl |
| 7 `default-readers.toml`, fünf Profile | `8433935` | Fünf `[[profil]]`-Blöcke (`resources/default-readers.toml:188`, `:235`, `:258`, `:283`, `:303`); die zwei berichtigten Feldmuster stehen auf `^([^\n]+)` (`:206`) und `(?sm)^## Directive…` (`:325`). Die zwei Ersetzungen aus `942172b` sind im Schritttext als Nachtrag geführt und am Baum belegt: das Sitzungsmuster (`:210`) und `muster = '\.md$'` an der Verlaufszeile (`:341`) |
| 8 Ablagehälfte | `4516f4e` | `AUSLIEFERUNGSTEXT` über `include_str!` (`crates/krk-core/src/ablage/leseprofile.rs:74`), `laden` (`:101`), `anlegen_falls_fehlt` mit `atomar::schreiben` (`:131`); die siebte Zeile in `crates/krk-core/tests/baum.rs:197`; vier Fehlerwegproben in `tests/ablage.rs:2030`, `:2096`, `:2138`, `:3385` |
| 9 Der siebte Inhalt | `b60988f` | `Inhalt::Zusammenfassung` (`crates/krk-ui/src/vorschaumodell.rs:296`), `zeigt_dateitext` antwortet `false` (`:564`), der Zweig vor dem Ordnerzweig in `laden` (`:708`); Proben zu C2.5 und C2.6 (`:1276`, `:1363`) |
| 10 Anzeigezweig und `profile_setzen` | `a77bb77` | `profile_setzen` (`crates/krk-ui/src/appkit/vorschau.rs:914`) auf `profile: OnceCell<Arc<Profile>>` (`:612`), der eine Anzeigezweig (`:1127`), `einzufaerben` antwortet `None` (`:1474`); Zählprobe `:1670` |
| 11 Die Anwendung lädt und übergibt | `7de937f` | Merkfeld `profile: RefCell<Arc<Profile>>` (`crates/krk-ui/src/appkit/anwendung.rs:545`), `leseprofile::laden(zugang)` im einen Durchgang (`:1559`), `vorschau.profile_setzen` (`:1221`); Zählprobe `::die_leseprofile_werden_im_baum_genau_einmal_geladen` (`:8718`) |
| 12 Die Zählproben zu C6 | `f9e34e7` | Neun Proben in `crates/krk-core/tests/leseprofil.rs:1587-2470`, darunter die Kindprobe unter `ulimit -n 24` (`:2370`, `:2406`) und die Messung gegen die eingebettete Fassung mit den genauen Zahlen `(5, 11)` und `(3, 5)` (`:2121`) |
| 13 Kostenangabe berichtigt | `c15f99b` | Absatz `**Berichtigung 260824-0910.**` (`decisions/260824-0541_*_wie-zieht-der-baustein-…`:26); der Defekt trägt seine `Resolved:`-Zeile und steht auf `_c_` |
| 14 Vierte Zustandszeile, drei Namen | `b5bf2e3` | Zeile „Abgelegt" auf `^_[sd]_circle\.md$` (`resources/default-readers.toml:320`), neun Speichernamen im Pfadmuster (`:237`) |

**Nachgerechnet und nicht übernommen: die Wirkung der Erkennung am echten Bestand.** Der Abgleich
hat die fünf Profile aus `resources/default-readers.toml` gegen alle 154 Verzeichnisse unter
`fusion-workbench/` gehalten. Ohne Profil bleiben 16, davon sind zehn keine Speicher
(`.guard-state` dreimal, `archive` selbst, die zwei Laufordner, deren zwei `shared`-Hüllen,
`fusion-workbench/shared` als Hülle, `stilwerk`) und sechs Speicher unter `archive/`. Das ist
genau der Bestand, den `issues/260824-1655_*_sechs-speicher-unter-archive-…` beschreibt; der
Datensatz bleibt offen, weil er eine Nutzerentscheidung stellt und keinen Mangel.

**Zehn Entscheidungsdatensätze stehen jetzt auf umgesetzt.** Acht standen auf beantwortet,
obwohl ihre Umsetzung im Baum steht; jeder trägt seit diesem Abgleich seine `Implemented:`-Zeile
mit Commit und Beleg. Zwei standen schon auf umgesetzt. Damit ist die Buchführung geschlossen,
die dieser Plan unter `## Welcher Schritt welchen Datensatz realisiert` angekündigt hat.

**Fünf neue Defektdatensätze sind gefallen**, alle unter `issues/` dieser Runde, alle vom Typ
„die Sache stimmt, die Zusage ist ungehalten":
`260824-1852_*_die-meldung-der-teillesungsprobe-…`,
`260824-1852_*_c3-14-nennt-seinen-eigenen-nachweis-…`,
`260824-1852_*_zwei-abnahmekriterien-aus-c5-…`,
`260824-1852_*_die-probe-zu-c5-10-…` und
`260824-1852_*_zwei-aussagen-in-claude-md-…`. Der letzte holt die zwei Stellen aus der
Risikotabelle dieses Plans in einen eigenen Datensatz; der Absatz in der Tabelle bleibt stehen.
