...
- [ ] Die dritte Spalte trägt eine ausgeschriebene Beschriftung und keinen Kurznamen aus dem Programmtext. Eine Legende, die Kurznamen erklärt, entsteht nicht, weil es keine Kurznamen gibt.
...
- [ ] Die sechs vom Hauptmenü zugestellten Textbefehle tragen in der dritten Spalte "Textfelder und Editor". Sie haben kein Kommando und damit keinen Wirkungsbereich; die Beschriftung entsteht aus der Prüfung im nächsten Kriterium und nicht aus einer Eigenschaft im Kern.
- [ ] **Vor dem Schreiben dieser Beschriftung ist für jeden der sechs Textbefehle einzeln geprüft, welche Flächen von KRK ihn tatsächlich beantworten.** Trifft "Textfelder und Editor" für einen von ihnen nicht zu, wird seine Zelle berichtigt oder bleibt leer. Eine Spalte, die eine falsche Zusicherung gibt, ist schlechter als eine leere. Die Begründung und der konkrete Verdachtsfall stehen unten unter `## Was die Abnahme mitentscheidet`.
...
**Die sieben Beschriftungen:**

| Wert in `Wirkungsbereich` | Beschriftung in der Datei | Woher |
|---|---|---|
...
Zwei Punkte gehören zur Abnahme dieses Specs und sind keine der zwölf Antworten oben. Dazu kommen sechs Vorbelegungen, die der Nutzer am Gate umstoßen kann.

**Eine Ableitung des Shapers, die vor dem Bau zu prüfen ist.** Die Beschriftung "Textfelder und Editor" für die sechs vom Menü zugestellten Textbefehle steht auf einer Ableitung und nicht auf einer Messung. Der Shaper hat sie aus dem Aufbau der Antwortkette geschlossen: die sechs Einträge tragen die Standardselektoren `undo:`, `redo:`, `cut:`, `copy:`, `paste:` und `selectAll:` und bekommen kein Ziel gesetzt (`crates/krk-ui/src/appkit/menue.rs:210-254`), also beantwortet sie, wer den Ersthelfer hält und den Selektor kennt. Ein Feldeditor kennt sie, die Textfläche des Editors kennt sie, und der Kommentarblock in `resources/default-keymap.toml` hält fest, dass im Dateifenster heute niemand sie beantwortet und der Eintrag dort deshalb grau ist.

**Der Verdachtsfall, an dem die Ableitung brechen könnte, ist benannt.** `text_alles_auswaehlen` liegt auf `selectAll:`, und die Lesezeichen- und Geräteleiste ist eine `NSTableView` (`crates/krk-ui/src/appkit/leiste.rs:3`). `inference:` Eine `NSTableView` beantwortet `selectAll:` von sich aus. Steht der Fokus in der Leiste, weist der stumme Fokusvorbehalt das Kommando `alle_markieren` ab und reicht den Tastendruck unverändert an AppKit weiter; von dort erreicht er den Menüeintrag und die Antwortkette. Trifft die Ableitung zu, wählt `Cmd+A` in der Leiste alle Zeilen aus, und die Zelle "Textfelder und Editor" wäre eine falsche Zusicherung. Das zwölfte Abnahmekriterium von C3 macht die Prüfung deshalb zur ersten Arbeit an diesem Schritt: **wer den Schritt baut, prüft alle sechs, bevor er die Spalte schreibt.** Eine leere Zelle ist besser als eine falsche.

Der Grund, aus dem dieser Punkt hier steht und nicht nur im Kriterium: die Durchsicht der vorigen Sitzung hat genau diese Fehlerform zweimal gefunden (`shared/issues/260810-1751_*_zwei-zusicherungen-ueber-den-auswahlversuch-halten-so-nicht-wie-sie-geschrieben-sind.md`). Eine Zusicherung, die im Text stärker ist als im Code, ist teurer als eine fehlende, weil niemand sie nachprüft.
