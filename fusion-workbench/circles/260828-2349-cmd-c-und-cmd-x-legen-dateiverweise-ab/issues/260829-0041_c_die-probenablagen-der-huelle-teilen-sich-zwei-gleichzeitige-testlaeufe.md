...
---
Resolved: Die Bedingung des Datensatzes ist eingetreten — parallele Testlaeufe sind mit den gleichzeitig gefahrenen Bahnen der Regelfall, zuletzt beobachtet am 260908-0030 —, und der Weg ist gefahren, in der Fassung, die weniger kostet als die vorgeschlagene: **eine** Ablage je Prozess statt einer je Probe. `probenablage()` nimmt keinen Zweck mehr entgegen, benennt sich `com.krk.probe.<pid>` und gibt einen Griff heraus, der einen `Mutex` haelt; alle zehn Proben laufen damit nacheinander auf derselben Ablage. Der Preis ist eine stehenbleibende, geleerte Ablage je Lauf und nicht zehn — `objc2-app-kit 0.3.2` bindet `releaseGlobally` weiterhin nicht. Die Sperre ist dieselbe Form, mit der `appkit/editor.rs` seine AppKit-Proben serialisiert, und sie ist noetig, weil die Proben untereinander sonst waeren, was zwei Prozesse vorher gegeneinander waren.
