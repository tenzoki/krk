...
---
Resolved: Alle drei als Nachsatz am Spec berichtigt (`planning/260829-0005_c_spec-…`), nach der Nutzerentscheidung zu `shared/decisions/260906-0203_*`, und jede Zahl am 260906 neu gemessen. **Punkt 1:** `grep -rn "betroffene(\|betroffene_eintraege()" crates/krk-ui/src` → **18** Zeilen in vier Dateien; der Nachsatz nennt wie vom Datensatz verlangt das Kommando und keine Ordnungszahl. **Punkt 2:** `grep -n "fileURLWithPath" crates/krk-ui/src/appkit/zwischenablage.rs` → `:210` im Untergrenzen-Abschnitt und `:456` der Aufruf; die Spec-Aussage ist seit `3764fb6` wahr und war es am 260829 nicht, und der Nachsatz sagt genau das. **Punkt 3:** `grep -rn "NSPasteboard" crates/krk-ui/src | grep -v appkit/zwischenablage.rs` → **30** Zeilen in **sieben** Dateien statt der drei des Spec, saemtlich in Kommentaren; die Aussage, die C5.1 halten will, bleibt wahr.
