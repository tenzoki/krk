...
**Date:** 2026-08-29
**Status:** Complete — alle neun Schritte `[DONE]`, jeder am 260829-0734 einzeln gegen den Baum gelesen (`## Reconciliation Log`), Abnahmelauf vom Nutzer am 260829 gefahren, alle zehn Punkte halten
...
**260829-0734** — Abgleich gegen HEAD `35b95b3` (Anker `4bd0084`; `701412c`/`9facb1e` dazwischen sind die Auslieferung 1.2.2 und kein Gegenstand der Runde). `cargo test --workspace` grün (krk-ui 851 passed, 0 failed), `cargo clippy --workspace --all-targets -- -D warnings` exit 0, `cargo fmt --all --check` exit 0.

| Schritt | Beleg |
|---|---|
| 1 | `4455af7`; `operationen.rs:1143` `pub enum Dateiablage`, `:1165` `namenszeilen`, `:1185` `ablagemeldung` (Zusatz `– verschieben tut das Ziel (Finder: opt+cmd+v)` `:1193`), `:1211` `verweise_abgewiesen`; Proben `:2066-2106` |
| 2 | `dfde98c`; `zulaessigkeit.rs:205` `zulaessig`, `:230` `dateiablage_zulaessig`, `:247` `enum Anspruch`, `:290` `fn gestattet`; Proben `:419` `die_zwei_frager_der_dateiablage_rufen_dieselbe_regel`, `:444` `die_dateiablage_wirkt_genau_mit_dem_fokus_im_dateifenster`, `:903` `waehrend_eines_blattes_kommen_genau_diese_vier_durch` |
| 3 | `3764fb6`; `zwischenablage.rs:359` `dateiverweise_auf_ablage_schreiben`, `:395` `dateiverweise_schreiben`, Modulkopf `:152` „Seit der Runde 22 schreibt die Huelle zwei Sorten", Untergrenzen `:193` `fileURLWithPath:` (`NSURL.h:52`), `:196` `NSPasteboardWriting`; Proben `:537`, `:563`, `:585`; `dateien_ablegen` nur noch Probenhelfer `:507` |
| 4 | `3764fb6`; `tabelle.rs:1940` `dateiverweise_ablegen`, leere Menge `:1943` `nichts_zu_kopieren`, `:1948` `ablagemeldung`, `:1950` `verweise_abgewiesen` |
| 5 | `1644ada`; `anwendung.rs:892` `#[unsafe(method(copy:))]`, `:905` `cut:`, `:967-968` Zweig in `validateMenuItem:`, `:3188` `dateiablage_ausfuehren`; Probe `:9855-9863` (`copy:`/`cut:` ja, `paste:` nein); `grep -rn 'method(paste:)' crates/` leer |
| 6 | `1644ada`; `menue.rs:23-24`, `:92`, `:100`, `:126-131`, `:890-896` (Doc `GEMESSEN`) |
| 7 | `1644ada`; `betrachter.rs:731` `nspasteboard_steht_nicht_im_betrachter_und_copy_und_cut_stehen_an_genannten_stellen`, Modulkopf `:65` |
| 8 | `38aa652`; `git diff 83e011c HEAD -- resources/default-keymap.toml Cargo.lock Cargo.toml` leer; `grep -n 'name = "cc"\|-sys"' Cargo.lock` allein `windows-sys` (108, 862, 872); Menü-Diff und Prüfsummen `8355633a…` beidseits in `history/260829-0210-coder-schritt-8-…md`; `make tasten` dort als nicht kopflos fahrbar begründet, ersetzt durch den leeren Diff der Belegungsquelle und ihrer Leser |
| 9 | `35b95b3`; Nutzerlauf, in Schritt 9 und im Turn log des Circle-Datensatzes gebucht; Terminal fügt den Namen ein (`decisions/260829-0053_a_…`, `Answered:`) |

`## Where this Circle stops`, Klausel für Klausel: neun `[DONE]` gelesen (oben); `make check`-Kommandos grün (oben); 40 Kriterien mit Stelle — der Spec zählt 43 Kästchen, drei davon Nutzerfragen, jedes C-Kriterium steht in einer `Kriterien:`-Zeile der Schritte 1–9 (nicht einzeln gegen den Spec nachgezählt, Zuordnung des Planers); Belegung und Menü unverändert (Schritt 8; `Kommando` zählt vor und nach der Runde 82 Varianten); `Cargo.lock` unverändert; `NSPasteboard` als Codezeile außerhalb der Hülle allein `abwurf.rs:132-175`/`:339` (`sorten`, Sortenkonstanten) und `vorschau.rs:302`/`:536-537` (`auswahl_ablegen`), beide nach dem präzisierten Constraint erlaubt — `writeObjects`, `setString_forType`, `generalPasteboard` als Codezeilen allein in `zwischenablage.rs`; `method(copy:)` je einmal in `anwendung.rs` und `betrachter.rs`, `method(cut:)` einmal, `method(paste:)` nirgends; `grep -oE '"L[0-9]+"' crates/krk-bench/src/messen.rs | sort -u` auf `83e011c` und HEAD dieselben zehn; Untergrenzen-Abschnitte in allen fünf angefassten `appkit/`-Dateien vorhanden, keine über macOS 15; Defekt `issues/260829-0006_o_…` offen und keine Vorbedingung; `circles/260828-1041-…/decisions/260828-1041_o_…` weiter `_o_`; Abnahmelauf gefahren, also die `_c_`-Klausel und nicht die `_b_`-Klausel; die Auslieferung 1.2.2 (`701412c`) liegt vor der ersten Codeänderung der Runde und hat ihr nichts vorweggenommen.

Abweichung ohne Handlungsbedarf: der Plan (Entscheidung 3, Risikotabelle) nennt `public.url` als zweite Sorte eines Datei-`NSURL`; die Messung der Durchsicht (`reviews/260829-0051-…`, „Was ich verifiziert habe") sagt allein `public.file-url`. Der Modulkopf der Hülle behauptet die Sorte nicht. Offen bleiben die vier Defekte des Circles (`260829-0006`, `260829-0041`, `260829-0051`, `260829-0052`), alle Low oder Prosa, keiner blockiert. Marker `_p_` → `_c_`.
