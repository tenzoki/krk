...
Implemented: 260824-1849, Commit `b60988f`, Schritt 9 des Plans. In `krk-ui/src/vorschaumodell.rs::laden` steht der Zusammenfassungszweig hinter der Typprüfung: nur ein Eintrag, der keine gewöhnliche Datei ist, erreicht ihn. `krk_core::leseprofil::zusammenfassen` weist eine Datei seit `06dbb4c` zusätzlich selbst ab, am aufgelösten Pfad, damit die Zusage nicht am Aufrufer hängt. Belegt durch `crates/krk-core/tests/leseprofil.rs::auf_eine_datei_greift_kein_profil_auch_bei_passendem_pfadmuster` und die Probe zu C2.6 in `vorschaumodell.rs`.
