...
Implemented: 260824-1849, Commits `8433935` (Schritt 7) und `4516f4e` (Schritt 8). `resources/default-readers.toml` trägt fünf Profile; `krk-core/src/ablage/leseprofile.rs` bindet sie über `include_str!` als `AUSLIEFERUNGSTEXT` ein und legt sie beim ersten Start wörtlich an, wie `einstellungen.rs` es für `settings.toml` tut. Belegt durch `crates/krk-core/tests/ablage.rs::eine_fehlende_readers_toml_entsteht_byteweise_und_bleibt_beim_zweiten_start_liegen`.
