...
---
Resolved: Der Nachweis ist gebaut, und er misst weniger als das Kriterium woertlich sagt; das steht an ihm. Neue Probe `der_leseweg_der_leseprofile_oeffnet_keine_datei_ueber_ihren_pfad` (`crates/krk-core/tests/baum.rs`): sie sammelt aus `quelldateien()` alles unter `krk-core/src/leseprofil/` und `krk-core/src/ablage/leseprofile.rs` und verlangt, dass keine Code-Zeile davon `File::open`, `fs::read(`, `read_to_string` oder `OpenOptions` traegt. Eine leere Menge und keine Aufzaehlung: eine erlaubte Ausnahme gibt es hier nicht, also steht keine Liste da, die jemand stillschweigend verlaengern koennte. Die Nadeln stehen mit `concat!` zusammengesetzt, wie jede Zaehlprobe dieser Datei. Am 260906 negativ geprueft: ein probeweise angehaengtes `std::fs::read_to_string` in `leseprofil/defaultprofil.rs` laesst sie mit dem Dateinamen rot werden, danach ist die Datei unveraendert wiederhergestellt. Was C3.14 fuer den ganzen Baum sagt, haelt die Probe ausdruecklich nicht: Kopieren, Entpacken, der Verzeichnisleser und die Proben oeffnen ueber ihren Pfad, jeder aus eigenem Grund, und eine Aufzaehlung darueber liefe ueber zwei Kisten und wuerde bei jeder Aenderung in der anderen rot, ohne dass an ihr abzulesen waere, warum. Der Doc-Kommentar der Probe schreibt diese Grenze aus.
