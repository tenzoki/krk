...
---
Resolved: Weg 1 gegangen, wie der Nachtrag vom 260826-1225 ihn eroeffnet hat. `Zugang::laden` (`crates/krk-core/src/ablage/mod.rs`) liest jetzt mit `fs::read` und wandelt selbst mit `String::from_utf8` um; scheitert die Umwandlung, gehen die Bytes ueber `Zugang::beiseite_legen` zur Seite und die Ersetzung traegt `Grund::Beschaedigt("keine gueltige UTF-8-Folge")`, woertlich derselbe Satz wie im Zettelweg unter `Unlesbarkeit::KeinText`. Kein zweiter Schreibweg, kein vierter Wert an `Grund`, kein Rufer in `krk-ui` beruehrt: `grep -rn 'Grund::' crates/krk-ui/src` bleibt ohne Fundstelle. Der Zweig `Grund::NichtLesbar` traegt jetzt, was er behauptet, naemlich ein gescheitertes Lesen. Nachgezogen sind die drei Prosastellen, die der Datensatz nennt: der Modulkopf von `ablage` (neuer Absatz "Eine Gestalt weniger seit dem 260906") und der Doc-Kommentar von `Zugang::laden`; die dritte liegt im Plan vom 260812-1145 und wird als eingefrorener Text nicht nachgezogen. Neue Probe `eine_ablagedatei_mit_ungueltigem_utf_8_geht_zur_seite` (`crates/krk-core/tests/ablage.rs`) mit `b"...Caf\xe9..."` als Bytefolge, genau wie der Datensatz sie vorschlaegt; sie prueft Grund, `Beiseite::Gesichert` und dass die Sicherung die Bytes der Datei traegt. Die Nachbarprobe `eine_fehlende_und_eine_nicht_lesbare_datei_werden_nicht_zur_seite_gelegt` bleibt gruen, denn ihr Ordner an der Stelle der Datei ist weiterhin `NichtLesbar`. `cargo test -p krk-core` Exit 0.
