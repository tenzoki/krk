...
---
Resolved: Was der Datensatz unter „Was zu tun ist" verlangt, ist getan, und zwar an allen drei Stellen, an denen die Luecke stand. Der Kommentar in `crates/krk-ui/src/appkit/textmerkmale.rs` sagt jetzt zum Satz „der Quelltext in einer Ueberschrift bekommt seine feste Schrift" dazu, dass er dabei auf die Grundgroesse faellt, nennt die 41 Prozent bei Stufe 1 und die zwei weiteren Formen (`## Ein **fetter** Teil`, `## *kursiver* Teil`); daneben steht, dass ein blosses Zusammenlegen der Schnitte den Groessenverlust nicht behebt. Der zurueckgestellte Datensatz `260812-1851_d_*` traegt eine `Also seen:`-Zeile mit demselben Inhalt, damit die Frage mit drei Faellen und einem Zustand aus Schnitt **und** Groesse wieder aufgenommen wird. Der geschlossene `260812-1805_c_*` traegt eine `Revised by:`-Zeile, weil sein Abschnitt „Was fehlt: Punkt 2" zwei Paarungen zaehlt und diese dritte uebergeht. **Ob der Fall behoben wird, ist damit nicht vorweggenommen**; er haengt an der zurueckgestellten Nutzerfrage, wie der Datensatz sagt.
