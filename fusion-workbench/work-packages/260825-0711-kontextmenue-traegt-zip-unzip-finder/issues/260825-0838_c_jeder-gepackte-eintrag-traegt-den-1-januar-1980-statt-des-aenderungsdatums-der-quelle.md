...
---
Revised by: `acc9671` — der Satz „und bekommt eine Zeile in der Abschlussliste" in der `Resolved:`-Notiz oben gilt seit dem 260825-2216 nicht mehr. `zeit_uebernehmen` (`crates/krk-core/src/operation/zippen.rs`) nimmt keine `Steuerung` mehr entgegen und meldet an keiner der drei Stellen: ein unlesbares Änderungsdatum, ein Zeitpunkt außerhalb von 1980 bis 2107 und ein nicht angehängtes Zusatzfeld lassen den Eintrag mit dem Vorgabedatum stehen und erzeugen keine Zeile. Der Grund: der Eintrag liegt in allen drei Lagen vollständig im Archiv, „übersprungen" war also die falsche Vokabel, und das andere Ende derselben Runde, das Entpacken, trifft dieselbe Wahl. Herleitung in `shared/issues/260825-2127_*_ein-gepackter-eintrag-mit-ersatzdatum-steht-in-der-liste-der-uebersprungenen.md`; die Probe heißt `ein_zeitpunkt_vor_1980_faellt_auf_das_vorgabedatum_und_bleibt_aus_der_abschlussliste` (`crates/krk-core/tests/operation.rs:1559`). Der Defekt bleibt geschlossen; allein die Begründung ist umgezogen. Nachgetragen vom reconciler am 260826-0149; die dritte Stelle derselben Aussage, das Abnahmekriterium im Plan, ist unter `shared/issues/260825-2230_*_der-plan-der-runde-18-verlangt-in-schritt-3-…` nachgezogen.
