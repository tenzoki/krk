...
---
Resolved: Als Nachsatz am Plan berichtigt (`planning/260814-2102_c_plan-…`, Punkt 1), nach der Nutzerentscheidung zu `shared/decisions/260906-0203_*`. Der Nachsatz folgt der Sammlung dieses Datensatzes und berichtigt alle fuenf Faelle in einem, nach ihren drei Ursachen getrennt: C2, E1 und E3 nennen `crates/krk-ui/src/appkit/tabelle.rs` in keinem Teil des Schritts, obwohl der Weg an das Tabmodell durch sie fuehrt; B1 nennt `crates/krk-ui/src/kommandos/navigation.rs` im Fliesstext und nicht in der Zeile (`grep -n "fn ersatzzeile" crates/krk-ui/src/kommandos/navigation.rs` → `:91`); D1s zwei weitere Dateien sind die Folge einer in die Mitte eingefuegten Rangstelle und keine Abweichung. Die Auskunft des Datensatzes, dass `Files:` in diesem Plan bei fuenf von vierzehn Schritten zu kurz misst, steht als Satz im Nachsatz.
