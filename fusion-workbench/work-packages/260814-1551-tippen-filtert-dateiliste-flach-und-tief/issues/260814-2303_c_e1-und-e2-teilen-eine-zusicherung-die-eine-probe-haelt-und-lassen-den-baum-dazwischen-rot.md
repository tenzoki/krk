...
---
Resolved: Als Nachsatz am Plan berichtigt (`planning/260814-2102_c_plan-…`, Punkt 2), nach der Nutzerentscheidung zu `shared/decisions/260906-0203_*`. Der Nachsatz uebernimmt den Vorschlag des Befunds und sagt, welche Reihenfolge richtig ist: erst der Eintrag in `resources/default-keymap.toml`, dann die Kennung. Die Belegungsdatei vertraegt einen Eintrag ohne zugehoeriges `Kommando` — `jede_kennung_der_kommandos_steht_in_der_auslieferungsbelegung` prueft diese Richtung nicht, und `Kommando::aus_kennung` antwortet dafuer ausdruecklich `None` mit der Bedeutung "noch nicht gebaut"; umgekehrt geht es nicht. Dass die zwei Schritte zwei Ausfuehrenden gehoeren, steht dabei.
