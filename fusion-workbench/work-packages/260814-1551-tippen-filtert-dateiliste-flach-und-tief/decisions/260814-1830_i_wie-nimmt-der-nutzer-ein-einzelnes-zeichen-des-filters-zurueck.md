...
---
Implemented: `2ff4b5a` — die Regel steht als reine Funktion mit ausgeschriebener Tafel in `crates/krk-ui/src/kommandos/rueckschritt.rs` und liefert `Rueckschritt::ZeichenZurueck`, sobald ein Filtertext steht (`:99-135`). Sie hat genau einen Rufer, gehalten von der Probe `die_regel_hat_genau_einen_aufrufer`. Die sicherheitsrelevante Hälfte der Antwort ist mit umgesetzt: bei stehendem Filtertext erreicht die nackte Taste `in_papierkorb` nicht, `cmd+delete` räumt in jeder Lage. Der Planner hat die vom Datensatz offen gelassene Frage nach dem Ort entschieden — die Regel steht **nicht** in `kommandos/zulaessigkeit.rs`, weil `delete` und `cmd+delete` dort dasselbe `Kommando` sind und eine Antwort dort den Menüeintrag ausgrauen würde. Abgeglichen am 260820-2056 gegen `f5300f4`.
