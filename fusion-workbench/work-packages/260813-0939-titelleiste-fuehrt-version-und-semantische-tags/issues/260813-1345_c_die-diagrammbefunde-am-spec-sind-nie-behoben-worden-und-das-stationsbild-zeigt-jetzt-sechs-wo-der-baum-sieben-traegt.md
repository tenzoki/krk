...
---
Resolved: Beide Haelften stehen als Nachsatz am Spec (`planning/260813-1037_c_spec-…`, Punkte 1 und 2), nach der Nutzerentscheidung zu `shared/decisions/260906-0203_*`: der Bestandstext bleibt unangetastet, die Berichtigung steht darunter. **Die Stationszahl ist heute acht und nicht sieben** — der Titel dieses Datensatzes ist selbst ueberholt: `grep -rn "sechs Stationen\|sieben Stationen\|acht Stationen" README.md Makefile xtask/src/*.rs` liefert sieben Treffer, alle "acht" (`README.md:236`, `:274`, `:276`; `xtask/src/main.rs:89`; `xtask/src/version.rs:11`, `:61`; `xtask/src/release.rs:3`). Der Spec zeigt den Stand vor seinem eigenen Plan und ist damit zwei Zaehlungen hinter dem Baum. B1, B2 und B6 sind im Nachsatz einzeln richtiggestellt, ohne dass an einem Bild ein Knoten oder eine Kante verschoben waere.
