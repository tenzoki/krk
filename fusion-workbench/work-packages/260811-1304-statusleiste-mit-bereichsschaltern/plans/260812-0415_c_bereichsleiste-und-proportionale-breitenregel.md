...
4. [DONE] **Fünf neue Funktionen in der Auslieferungsbelegung**
   ...
   - Dependencies: keine.
   - Anmerkung: die gewählten Kombinationen sind eine Empfehlung und eine Nutzerfrage; siehe `## Offene Fragen` und den Datensatz `decisions/260812-0415_*_welche-kombinationen-bekommen-die-beiden-neuen-umschalter.md`. Eine andere Antwort kostet in diesem Schritt je eine Zeile. **Am 260812-0430 beantwortet: die Empfehlung wird übernommen**, der Schritt setzt sie unverändert um.
   - Befund bei der Ausführung: `make check` läuft nach diesem Schritt **nicht** durch, und das ist kein Fehlschlag des Schrittes. Grün sind die beiden hier genannten Prüfungen in `crates/krk-core/src/tasten/belegung.rs`. Rot sind achtundzwanzig Proben in `krk-ui`, die den vom Plan benannten Zwischenstand messen (fünf Kennungen ohne Kommando, aufgelöst durch die Schritte 5 und 7), und drei weitere, die **kein** Schritt dieses Plans wieder grün macht; sie stehen in `issues/260812-0533_*_drei-proben-stehen-gegen-die-neuen-belegungseintraege-und-keine-gehoert-zu-einem-planschritt.md` samt Vorschlag.
...
| Der erste Anteil eines nie sichtbaren Bereichs hängt an der Fensterbreite im Augenblick des ersten Aufgehens. | Bereits im Datensatz `260811-1305_*_welchen-anteil-bekommt-ein-bereich-der-noch-nie-sichtbar-war.md` benannt und dort als klein bewertet. Der Plan ändert daran nichts. |
...
| Die Aufzählungen `Spalte` (Anzeige) und `Schluessel` (Sortierung) nennen dieselben vier Dinge, und nichts hält sie zusammen. | Nicht Gegenstand dieser Runde. Abgelegt als `issues/260812-0415_*_die-spalten-und-die-sortierschluessel-sind-zwei-aufzaehlungen-derselben-vier-dinge.md`. |
...
- [ ] **Welche Kombinationen bekommen die beiden neuen Umschalter, und was wird aus `opt+cmd+e`?** `decisions/260812-0415_*_welche-kombinationen-bekommen-die-beiden-neuen-umschalter.md`. Der Plan setzt `opt+cmd+left` und `opt+cmd+b` und lässt `editor_schliessen` auf `opt+cmd+e`. Kosten einer Umkehr: je eine Zeile in Schritt 4.
- [ ] **Was tut der Editorschalter, wenn der Editor keine Datei hält?** `decisions/260812-0415_*_was-tut-der-editorschalter-ohne-datei-im-editor.md`. Der Plan weist ihn ohne Meldung ab, wie es `fokus_editor` schon tut. Kosten einer Umkehr: der Rumpf von `editor_umschalten` in Schritt 5.
- [ ] **Was geschieht, wenn der Nutzer das Fenster unter die Summe der Mindestbreiten zieht?** `decisions/260812-0415_*_was-geschieht-wenn-das-fenster-unter-die-summe-der-mindestbreiten-faellt.md`. Der Plan lässt alle sichtbaren Bereiche mit demselben Faktor schrumpfen. Der Datensatz führt daneben die Möglichkeit, `MINDESTGROESSE` in der Breite auf 940 zu heben; sie machte die eben beantwortete sechste Frage dieses Circles gegenstandslos und ist deshalb eine Nutzerentscheidung und keine Planentscheidung.
...
(`issues/260812-0801_*_zwei-modulkoepfe-nennen-aufteilung-sichtbar-im-das-es-nicht-mehr-gibt.md`).
