...
---
Resolved: Auf dem zweiten der zwei vorgeschlagenen Wege behoben, dem, der den schwereren Teil
trifft und keine Voraussetzung der Kette ändert. Die Meldung des gescheiterten Anlegens ist
als reine Funktion `anlegen_gescheitert_meldung` (`xtask/src/veroeffentlichung.rs`)
herausgezogen und trägt keinen Satz mehr für eine Ursache: sie stellt die
Standardfehlerausgabe von `gh` voran, benennt beide Lagen — vorübergehend gegen stehend — und
nennt zu jeder den Handgriff, `cargo xtask veroeffentlichen <zahl>` für die erste und
`gh repo view` für die zweite. Die Probe
`das_gescheiterte_anlegen_nennt_beide_lagen_und_je_ihren_handgriff` hält beide Zweige und das
Verschwinden des alten Satzes.

**Die Maschine entscheidet dabei nicht zwischen den zwei Lagen, und das ist Absicht.** Der
Rückgabewert trennt sie nicht, und die Standardfehlerausgabe täte es allein über ihren
Wortlaut — genau die Grundlage, die dieses Modul für jede Entscheidung ablehnt, weil sie mit
der nächsten Fassung des fremden Werkzeugs eine andere ist. Der Datensatz nennt als ersten Weg
eine dritte Frage in `gh_pruefen`; sie ist **nicht** gebaut, und damit bleibt es dabei, dass
ein unerreichbares Vorhaben erst nach dem Packen und Schieben auffällt. Was sich geändert hat,
ist die Auskunft an dieser Stelle, nicht der Zeitpunkt.

Der Nachtrag vom 260826-1440 ist mitbehoben: keine der Meldungen sagt mehr „Derselbe Aufruf
noch einmal", ohne den Befehl zu nennen. `spaet_ohne_gh_meldung` nennt jetzt ebenfalls
`cargo xtask veroeffentlichen <zahl>` und bekommt die Zahl dafür gereicht; die Probe
`jede_gh_meldung_nennt_den_stand_der_an_ihrer_stelle_gilt` hält es.

**Was offen bleibt:** `release_steht` hat zwei Rufer mit verschiedenem Stand des Laufs und
gibt beiden dieselbe späte Meldung; an Station 1 nennt sie damit „gepackt und geschoben", wo
nichts geschehen ist. Eigener Datensatz
