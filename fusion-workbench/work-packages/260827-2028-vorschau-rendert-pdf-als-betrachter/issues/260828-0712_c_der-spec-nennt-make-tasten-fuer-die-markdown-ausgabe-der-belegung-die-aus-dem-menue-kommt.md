...
---
Resolved: Als Nachsatz am Spec berichtigt (`planning/260828-0649_c_spec-…`), nach der Nutzerentscheidung zu `shared/decisions/260906-0203_*`. Am 260906 nachgemessen und unveraendert: `grep -n -A3 "^tasten:" Makefile` → `tasten: bundle ## Tastencodes protokollieren, Beenden mit Cmd+Q` und `$(BINAER) --tasten-protokoll`, also der interaktive Tastenlogger und kein Markdown. Die Markdown-Datei entsteht aus dem Menueeintrag `tastenbelegungSichern:` (`crates/krk-ui/src/appkit/anwendung.rs:901`) ueber `crates/krk-ui/src/belegungsausgabe.rs`, dritte Spalte aus `Wirkungsbereich::beschriftung` (`:271`). Der Abnahmelauf von Schritt 11 (`48cd818`) ist bereits ueber den Menueeintrag gefahren, wie dieser Datensatz es vorgegeben hat.
