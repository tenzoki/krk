...
**Date:** 2026-08-28
**Status:** Complete — gebaut, vom Nutzer am 260828 abgenommen, alle elf Schritte `[DONE]` gegen den Baum gelesen (Abgleich 260828-1044)
...
1. **Das Tastenalphabet trägt `plus` und `minus`** [DONE]
   - Executor: `coder`
...
2. **`Wirkungsbereich::Vorschau` und die drei Kommandos** [DONE]
   - Executor: `coder`
...
3. **Die drei Einträge in der Auslieferungsbelegung** [DONE]
   - Executor: `ontocoder`
...
4. **Die Kiste `objc2-pdf-kit` mit Begründung** [DONE]
   - Executor: `coder`
...
5. **Der vierte Weg im Modell: `Inhalt::Pdf`** [DONE]
   - Executor: `coder`
...
6. **Das Modul `appkit/betrachter.rs`: die Klasse `Pdfbetrachter`** [DONE]
   - Executor: `coder`
...
7. **Die dritte Ansicht in der Vorschau** [DONE]
   - Executor: `coder`
...
8. **Der siebte Rang der Statuszeile** [DONE]
   - Executor: `coder`
...
9. **Die drei Ausführungszweige beim Anwendungsdelegierten** [DONE]
   - Executor: `coder`
...
10. **Die Berührung von C6 der Runde 1 buchen** [DONE]
    - Executor: `analyst`
...
11. **Der Abnahmelauf am laufenden Bündel** [DONE] — vom Nutzer am 260828 gefahren, alle zwölf Punkte halten (Bündel auf 8a8e638; ein erster Lauf auf 03af590 fand den Delegat-Absturz, behoben in 8a8e638)
...
**260828-1044 — Abschluss-Abgleich der Runde 20** (Anker `2033626`, HEAD `48cd818`)

Jede der elf `[DONE]`-Behauptungen einzeln gegen den Baum gelesen; alle halten.

| Schritt | Beleg |
|---|---|
| 1 | `1df8b8d`: `zeichen_des_namens` (`crates/krk-core/src/tasten/parser.rs:211-222`), `TASTEN` trägt `plus`/`minus` mit `kVK_ANSI_KeypadPlus`/`Minus` (`:364-365`), `zeichen_der_taste` ruft `taste.zeichen()` (`crates/krk-ui/src/appkit/menue.rs:612-613`) |
| 2 | `2aee690`: `Wirkungsbereich` acht Werte, `Vorschau` (`crates/krk-core/src/tasten/belegung.rs`, Beschriftung `:362`); drei Kommandos (`:733-747`), `KENNUNGEN: [...; 82]` (`:753`), `wirkungsbereich` (`:1037-1039`); `fokus::wirkt` (`crates/krk-ui/src/kommandos/fokus.rs:370`), Tafel „acht" (`:400`); `ACHT_BESCHRIFTUNGEN` über `varianten_der_aufzaehlung` (`crates/krk-core/tests/belegung.rs:1977`, `:2033`); Zulässigkeitsproben (`zulaessigkeit.rs:868-931`) |
| 3 | `22b8442`: `resources/default-keymap.toml:705-717` die drei Einträge, `:34` „88 Funktionen mit 93 Kombinationen", `:698-700` Kommentar zu `plus`/`minus` |
| 4 | `ae349d1`: `Cargo.toml:284` `objc2-pdf-kit` ohne Vorgabemerkmale, `crates/krk-ui/Cargo.toml:38`, `Cargo.lock:500` 0.3.2; `grep 'name = "cc"\|-sys"' Cargo.lock` liefert allein `windows-sys`, wie auf `2033626` |
| 5 | `9d2e457`: `Inhalt::Pdf` (`crates/krk-ui/src/vorschaumodell.rs:320`), `PDFENDUNG` (`:238`), `endung_klein` (`:876`), `ist_pdfpfad` (`:887`), Ladezweig (`:830`), `zeigt_dateitext` (`:642`), Modulkopf „vier Wege" (`:36`); Proben `:1206`, `:1228`, `:1439` |
| 6 | `5ff1ee4` + `8a8e638`: `crates/krk-ui/src/appkit/betrachter.rs` (711 Zeilen), Untergrenzen-Abschnitt (`:114`), `ZOOM_MIN`/`ZOOM_MAX` (`:194`, `:202`), `Zoom` (`:216`), `Deutung` (`:233`), `copy:` (`:359`), `menuForEvent:` (`:383`), `dokument_setzen` `#[must_use]` (`:498-499`), `zoomen` (`:547`), `seitenstand` (`:578`), `PDFViewPageChangedNotification` (`:450`, `:619`); `mod betrachter;` (`appkit/mod.rs:197`, „Einunddreissig Module" `:10`). Abweichung vom Planwortlaut: der Delegat ist seit `8a8e638` eine eigene Klasse `Verweisdelegierter` (`:259-280`) und nicht mehr der Betrachter selbst — der Plan sah „als `PDFViewDelegate` beantwortet die Klasse" vor; die Bauart war der Absturz (`analyses/260828-0912-absturzbericht-pdfview-delegate-rekursion.txt`, `-[PDFView PDFViewWillChangeScaleFactor:toScale:]` unbegrenzt rekursiv). Keine Nachbesserung nötig, der Bugfixer hat den Modulkopf nachgezogen (`:141`) |
| 7 | `5ff1ee4`: `Flaeche` (`crates/krk-ui/src/appkit/vorschau.rs:351`), `flaeche_zeigen` als einzige `setHidden`-Stelle (`:1406-1417`, Probe `:2439-2475`), `pdf_zeigen` (`:1444`), `zoomen` (`:1113`), `seitenzaehler` (`:1134`), `seitenmelder_setzen`/`seiten_melden` (`:1147`, `:1164`), `anzeigen`-Zweig (`:1310`), `einzufaerben` (`:1768`) |
| 8 | `5ff1ee4`: `Rang::Seitenzaehler` (`crates/krk-ui/src/appkit/statuszeile.rs:235`, `ALLE` `:281`, `art` `:299`), `Herkunftsart` (`:250`), `Herkunft` (`:547`), `seitenzaehler_text` (`:536`); `anwendung.rs:1163` Seitenmelder, `:5073` fünfte Eingabe |
| 9 | `5ff1ee4`: die drei Zweige `crates/krk-ui/src/appkit/anwendung.rs:3439-3441`, keiner in `tabelle.rs`; Zählprobe in `zulaessigkeit.rs:913-915` |
| 10 | `03af590`: `issues/260828-0744_o_c6-der-runde-1-…` steht, `_o_`, fremder Spec unverändert |
| 11 | `48cd818`: Nutzerlauf auf dem Bündel `8a8e638`, alle zwölf Punkte laut Turn log des Circle-Datensatzes und Schritt 11; nicht von einem Agenten prüfbar, als Nutzeraussage übernommen |

`## Where this Circle stops`, zwölf Klauseln: (1) elf `[DONE]`, siehe Tabelle; (2) `cargo build`, `cargo test --workspace` (alle Ziele grün, u. a. 840 + 196 + 155 Proben), `cargo clippy --all-targets`, `cargo fmt --check` am 260828-1044 auf `48cd818` grün; (3) 45 Kriterien den Schritten zugeordnet, C3.2 US-Hälfte offen (`decisions/260828-0712_o_*`); (4) `Cargo.lock` allein `windows-sys`, identisch mit `2033626`; (5) `grep -rn 'ohne_warten_oeffnen(' crates/krk-core/src` zehn Treffer, identisch mit `2033626`; (6) `grep -oE '"L[0-9]+"' crates/krk-bench/src/messen.rs | sort -u` L1–L10 vor und nach der Runde gleich; (7) Untergrenzen-Abschnitt in jeder Datei unter `appkit/` außer `koordinaten.rs` und `mod.rs`, `betrachter.rs` eingeschlossen, höchste Angabe 10.13 (`betrachter.rs:114-160`); (8) `shared/issues/260826-1302_c_ein-achter-…` und `260826-1423_c_…` tragen `Resolved:` und `_c_`, C6-Datensatz steht; (9) `decisions/260827-2028_*` jetzt `_i_` mit `Implemented:` (dieser Abgleich); (10) `decisions/260828-0712_o_*` offen, keine Vorbedingung; (11) Abnahmelauf gefahren, also `_c_` möglich — die Umbenennung des Circle-Datensatzes gehört dem Orchestrator; (12) keine Auslieferung gefahren, `git tag -l` unverändert.

Plan `_p_` → `_c_`; Spec `_o_` → `_c_`.
