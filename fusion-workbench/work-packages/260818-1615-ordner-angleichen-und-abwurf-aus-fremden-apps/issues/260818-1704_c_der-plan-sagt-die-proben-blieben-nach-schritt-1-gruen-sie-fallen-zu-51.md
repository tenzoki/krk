...
---
Resolved: Als Nachsatz am Plan berichtigt (`planning/260818-1633_c_plan-…`, Punkt 1), nach der Nutzerentscheidung zu `shared/decisions/260906-0203_*`. Der Nachsatz sagt, was die Abnahmezeile von Schritt 1 sagen muesste: der rote Zwischenstand ist erwartet und wird vom gemeinsamen Commit mit Schritt 2 getragen; die brechende Stelle ist `belegungsmodell.rs` und nicht `belegungsausgabe.rs`, und eine Funktion ohne `Kommando` ist kein bekannter Zustand des Modells. Die Meldung steht heute in `crates/krk-ui/src/belegungsmodell.rs:896` und `:1029` (`grep -rn "hat keinen Funktionsbereich" crates/krk-ui/src/belegungsmodell.rs`); die Zeilennummer `:831` des Befunds ist gewandert. Dass die Kopfzusage des Plans — gruen nach jedem einzelnen Schritt — fuer Schritt 1 nicht haelt, steht ausdruecklich dabei.
