...
22. [DONE] **5.4a Das Modell: Schutz, Laden mit PIN, verschlüsseltes Sichern**
    - Umgesetzt am 260926, `make check` grün. `Editormodell::oeffnen` nimmt die PIN als zweites Argument (`Option<Pin>`); `Editorbereich::datei_oeffnen` reicht bis 5.4b `None` herein, so dass F4 und `cmd+e` auf `.secrets.txt` jetzt sofort mit „… lässt sich nicht im Editor öffnen: sie ist verschlüsselt und öffnet sich allein mit der PIN“ abgewiesen werden, ohne Faden und ohne Lesen. Mit PIN liest der Faden über `datei::bis_zur_grenze_lesen` (Editorgrenze plus Kopf plus Prüfwert), null Bytes leiten über `neuer_schluessel` ab, sonst `tresor::oeffnen`. Gesichert wird über `Chiffrat::verschliessen` (Sicherungsform, dann `tresor::verschliessen`) und `Chiffrat::schreiben` (`atomar::schreiben`); der Schreibweg nimmt allein den Typ `Chiffrat`. **Drei Abweichungen:** (1) Die Abweisungen sind `Abweisung::KeinGueltigesZiel` mit dem Satz aus `Oeffnungsfehler::meldung` als Grund, kein neuer Wert in `Abweisung` oder `Ladeausgang`; der Satz lautet damit „<Pfad> lässt sich nicht im Editor öffnen: PIN falsch oder Datei verändert“. (2) Zwei Sicherungen gegen eine Erkennung, die sich zwischen Öffnen und Sichern ändert (ein F2 erneuert die aufgelöste Form): `uebernehmen` nimmt eine im Klartext gelesene `.secrets.txt` nicht auf, und der Klartextweg von `sichern` fragt die Erkennung vor `datei::sichern` noch einmal und schreibt nicht. (3) Eine PIN für eine andere Datei weist ab, statt die Datei im Klartext zu öffnen. `Schutz::Verschluesselt::pin` trägt bis 5.5 ein `expect(dead_code)`. `crates/krk-core/tests/baum.rs` nennt `krk-ui/src/editormodell.rs` in `nur_benannte_dateien_erreichen_das_atomare_schreiben`. **Probes:** die zehn aus diesem Schritt im Prüfmodul von `editormodell.rs`, darunter `kein_weg_schreibt_klartext_nach_secrets_txt` und `die_sperre_fragt_die_sonderdatei_vor_dem_lesen`; gegen vier Mutationen gefahren (Klartextfrage im Sichern entfernt, Klartext als Chiffrat, Sperre in `oeffnen` entfernt, Prüfung in `uebernehmen` entfernt), jede macht mindestens eine Probe rot. **Übergabe an 5.4b:** `Editorbereich::geheimnisse_oeffnen(pfad, pin)` ruft `modell.oeffnen(pfad, Some(pin))` über denselben Takt wie `datei_oeffnen`; die Form des Blattes (festlegen oder eingeben) entscheidet die Dateigröße, das Modell nimmt in beiden Fällen dieselbe PIN.
    - Executor: `code-implementer`
...
23. [DONE] **5.4b Das PIN-Blatt und der Weg in den Editor**
    - Umgesetzt am 260926, `make check` grün. `Editorbereich::datei_oeffnen` fragt nach der Zelle `heim.sonderdatei(pfad)` über einen eigenen Griff in `EditorIvars::heim`; die reine Regel `oeffnungsweg(herkunft, sonderdatei, haelt_bereits)` gibt `Laden`, `PinErfragen` oder `Unterlassen`. Aus der Sitzung geschieht nichts; auf einen Befehl hin gibt das Modell ein laufendes Lesen auf (`laden_aufgeben`), ein `stat(2)` entscheidet `Pinform::nach_groesse` (0 Bytes: `Festlegen`, sonst und ohne Größe: `Eingeben`), und der Bedarf geht als neuer Wert `Ladeausgang::PinVerlangt { pfad, form }` durch dieselbe Senke. Die schon gehaltene `.secrets.txt` fragt nicht erneut und nimmt die Abkürzung `SchonOffen`. Der Delegierte zeigt `blaetter::pin::zeigen` (`pin_erfragen`), der Griff geht in `offenes_blatt`, und die PIN geht an `Editorbereich::geheimnisse_oeffnen(pfad, pin)` (Herkunft ausdrücklich `Befehl`, derselbe Takt). Das Blatt trägt `NSSecureTextField`, `Festlegen` zwei Felder, `Eingeben` eines, `HINWEIS` im Wortlaut aus C7. **Abweichungen:** (1) „Eine abgewiesene Eingabe schließt das Blatt nicht“ ist über die bestätigende Schaltfläche gebaut, die abgeschaltet bleibt, solange `eingabe_pruefen` nein sagt, und über eine Prüfung im Eingabewächter (`Blatt::bestaetigung_pruefen`, `Blatt::bestaetigende_schaltflaeche` in `blaetter/mod.rs`); der Grund steht in einer Zeile unter den Feldern. (2) `pin_aenderbar` liest keinen Kopf von der Platte, sondern eine Buchführung im Schutz (`Schutz::Verschluesselt::kopf_auf_platte`: gesetzt beim Lesen nicht leerer Bytes und nach jedem gelungenen Sichern), weil `lage()` bei jedem Tastendruck läuft. (3) Eine falsche PIN lässt das Blatt nicht wieder aufgehen; sie meldet „PIN falsch oder Datei verändert“ wie in 5.4a. (4) Eine vorgemerkte Textmarke fällt beim PIN-Bedarf. `Editorform::Geheimnisse` zeigt die Notiztabelle, `eintragsart_der_form` gibt `Notizen`, `esc` übernimmt wie in der Notizzelle, `form_passt` behandelt sie wie `Notizen`. **Probes:** `der_wortlaut_des_blattes`, `das_blatt_nimmt_nur_vier_ziffern_an`, `die_wiederholung_muss_gleichen`, `der_grund_beim_tippen`, `festlegen_fragt_zweimal_und_eingeben_einmal` (`blaetter/pin.rs`); `secrets_txt_geht_ueber_das_blatt_und_aus_der_sitzung_nirgends_hin`, `datei_oeffnen_fragt_die_sonderdatei_vor_dem_ladeauftrag`, die Formenableitung in `die_form_folgt_aus_ansicht_und_dateityp` (`appkit/editor.rs`); `die_form_des_pin_blattes_folgt_der_groesse` (`editormodell.rs`); `JEDE_FORM` mit vier Einträgen samt Tafel und Sechs-Befehle-Probe (`zulaessigkeit.rs`). **Neu gemeldet:** `260926-1004_*_eine-textmarke-in-secrets-txt-schreibt-eine-klartextzeile-in-die-lesezeichendatei.md`. **Übergabe an 5.5:** `Lage::pin_aenderbar` wird erhoben und nicht gelesen; `pin_erfragen` und `blaetter::pin` kennen zwei Formen, die dritte (alte PIN, neue zweimal) kommt dazu, und `eingabe_pruefen` nimmt dann ein drittes Feld.
    - Executor: `code-implementer`
...
24. [DONE] **5.5 Der Befehl „PIN ändern“**
    - Umgesetzt am 260926, `make check` bis auf die vier Proben, die jede Kennung in der Auslieferungsbelegung verlangen: `tasten::belegung::tests::jede_kennung_der_kommandos_steht_in_der_auslieferungsbelegung` (`krk-core`, Bibliothek), `jedes_gebaute_kommando_haengt_an_seiner_ausgelieferten_taste` (`crates/krk-core/tests/belegung.rs`), `belegungsausgabe::tests::die_dritte_spalte_haelt_die_begruendungslagen_auseinander` und `belegungsmodell::tests::der_bereich_editor_fuehrt_genau_die_befehle_des_editors` (`krk-ui`); 5.6 macht sie gruen. `Kommando::PinAendern` (`pin_aendern`) traegt den neuen `Wirkungsbereich::Geheimnisse` („Geheimnisse im Editor“), `fokus::wirkt` verlangt `Fokus::Editor`, und `zulaessigkeit::gestattet` fragt ueber die neue vollstaendige Funktion `datei_passt` `Lage::pin_aenderbar`; die Form fragt der Bereich nicht, auch die Rohansicht der Datei laesst den Befehl zu. `Funktionsbereich::Editor`, eigener Zweig `Kommando::PinAendern => self.pin_aendern_erfragen()` samt Eintrag in der Zweigprobe. `Editorbereich::pin_aendern` ruft als erstes `zelle_uebernehmen` (Probe `die_zellenuebernahme_hat_genau_diese_rufer` nennt sieben Rufer) und fragt dann `Editormodell::pin_aenderung_pruefen` (Kopf auf der Platte, kein laufender Wechsel, keine fremde Aenderung), bevor das Blatt aufgeht. `Pinform::Aendern` mit drei Feldern; das Blatt liefert eine `Pineingabe { pin, alte }` ueber `felder_pruefen`, das `eingabe_pruefen` fuer die neue PIN und ihre Wiederholung nutzt. `Editormodell::pin_aendern(alte, neue)` vergleicht die alte gegen `Schutz::Verschluesselt::pin` (das `expect(dead_code)` ist gefallen) und startet `tresor::neuer_schluessel` auf dem Faden `krk-pin`; `pinwechsel_einziehen` laeuft am Einzugstakt neben Laden und Einfaerben, fragt `fremd_geaendert`, liest die Platte, entschluesselt mit dem gehaltenen Schluessel, verschliesst ueber `Chiffrat::verschliessen` mit dem neuen, schreibt ueber `Chiffrat::schreiben`, setzt den Stempel neu und ersetzt den Schutz mit `kopf_auf_platte: true`. Der Ausgang geht als `Editormeldung::PinGeaendert` oder `PinNichtGeaendert` ueber die Meldungssenke in die Statuszeile. **Abweichungen:** (1) `crates/krk-core/src/heimordner/tresor.rs` steht nicht in der Dateiliste und hat `tresor::oeffnen_mit(bytes, &Schluessel)` bekommen: der Plan verlangt das Entschluesseln mit dem gehaltenen Schluessel, und die Kiste bot dafuer keinen Weg ohne Ableitung; ein Schluessel mit anderem Salz oder anderen Parametern gibt `PinFalschOderVeraendert`. (2) Die Ableitung laeuft auf dem Faden, Lesen, Entschluesseln, Verschluesseln und Schreiben danach auf dem Hauptfaden, wie beim Sichern: so ist der umgeschluesselte Stand der, der im Augenblick des Schreibens auf der Platte steht, und ein `cmd+s` waehrend der Ableitung geht nicht verloren. (3) Eine falsche alte PIN prueft nicht das Blatt, sondern das Modell nach dem Blatt, und sie meldet sich in der Statuszeile; das Blatt kennt die gehaltene PIN nicht (dieselbe Bauart wie Abweichung (3) in 5.4b). (4) Der laufende Wechsel ueberlebt Dateiwechsel und Schliessen und meldet dann „… ist nicht mehr offen; die PIN bleibt, wie sie war“, statt still zu fallen. (5) `eingabe_pruefen` hat keinen dritten Parameter bekommen; die dritte Form geht ueber das neue `felder_pruefen` und `grund_der_felder`, damit die zwei aelteren Formen und ihre Proben unveraendert bleiben. **Probes:** `nach_dem_aendern_oeffnet_allein_die_neue_pin`, `eine_falsche_alte_pin_weist_ab_und_laesst_alles_stehen`, `eine_fremd_geaenderte_datei_weist_das_aendern_ab`, `ein_sichern_waehrend_des_aenderns_geht_nicht_verloren`, `eine_nie_gesicherte_pin_laesst_sich_nicht_aendern`, `ein_schliessen_waehrend_des_aenderns_meldet_und_schreibt_nichts`, `das_aendern_leitet_auf_einem_faden_ab_und_schreibt_allein_chiffrat` (`editormodell.rs`); `aendern_fragt_die_alte_einmal_und_die_neue_zweimal`, `eingeben_und_festlegen_liefern_keine_alte_pin` (`blaetter/pin.rs`); `pin_aendern_wirkt_allein_mit_dem_fokus_im_editor_und_pin_aenderbar`, `allein_pin_aendern_fragt_pin_aenderbar`, Tafel und Stellvertreter um die Zeile `Geheimnisse` (`zulaessigkeit.rs`); Tafel in `fokus.rs`; `der_gehaltene_schluessel_oeffnet_allein_seine_datei` (`tresor.rs`); `pin_aendern_traegt_den_bereich_der_geheimnisse` und das Beschriftungsfeld (`crates/krk-core/tests/belegung.rs`). **Uebergabe an 5.6:** `pin_aendern` fehlt allein in `resources/default-keymap.toml`; `EDITORBEFEHLE` in `belegungsmodell.rs` fuehrt es schon. **Uebergabe an 5.7:** `HowTo.md` beschreibt „PIN ändern“ mit der Regel, dass der Befehl erst nach dem ersten Sichern wirkt, dass ungesicherte Aenderungen ungesichert bleiben, und dass die alte PIN nach dem Blatt geprueft wird.
    - Executor: `code-implementer`
...
25. [DONE] **5.6 Die Belegung trägt „PIN ändern“**
    - Executor: `data-implementer`
    - Files: `resources/default-keymap.toml`
