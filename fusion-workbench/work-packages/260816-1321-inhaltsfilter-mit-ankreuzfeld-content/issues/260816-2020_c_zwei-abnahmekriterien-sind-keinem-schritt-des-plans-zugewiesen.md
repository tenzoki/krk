...
---
Resolved: Als Nachsatz am Plan berichtigt (`planning/260816-1359_c_plan-…`), nach der Nutzerentscheidung zu `shared/decisions/260906-0203_*`. Der Nachsatz weist C4.3 und C6.2 ihre Nachweise zu und nennt den Grund der Luecke: der Weg beider stammt vollstaendig aus der Runde 10 und hat in dieser Runde keine Zeile bekommen. **C6.2 traegt dabei seit der Runde 21 einen anderen Namen**: `filter_klein` und `Ordnermodell::filter_uebernehmen` aus dem Befund gibt es in dieser Form nicht mehr; kleingeschrieben wird einmal je Aenderung in `Muster::aus` und einmal je Vergleich in `traegt_die_folge`, beide in `crates/krk-core/src/verzeichnis/filter.rs`. Nachgemessen mit `grep -rn "to_lowercase\|to_ascii_lowercase"` ueber die fuenf vom Befund genannten Dateien → **zwei** Stellen, beide dort (`:146`, `:193`); die Zusage haelt unveraendert. C4.3 belegt ueber `filter_leeren` in `appkit/tabelle.rs` und `verzeichnis/modell.rs` und `durchlauf_nachziehen_an` in `tabs.rs`, gehalten von `ohne_seine_drei_bedingungen_beginnt_kein_durchlauf`. Der Vorschlag des Befunds, einen Schritt zu bauen, der `Erfuellt:`-Felder gegen die Kriterien abgleicht, ist damit nicht erledigt und gehoert nicht in diese Runde.
