...
---
Resolved: Als Nachsatz am Spec berichtigt (`planning/260830-1251_p_spec-…`, Punkt 5), nach der Nutzerentscheidung zu `shared/decisions/260906-0203_*`, und in der vom Datensatz verlangten Form: C3.8 sagt dasselbe wie C10.3, und das tragende Pruefmittel ist die Frage nach dem **Aufruf**. Am 260906 gemessen: `grep -rn "write_changes" crates/` → **drei** Treffer, alle in Prosa (`crates/krk-core/src/git/mod.rs:17`, `crates/krk-core/src/git/leser.rs:56`, `:496`); `grep -rn "write_changes(" crates/` → **null**. Der zweite, aeltere Riss ist mitgemessen: `grep -rn "NeedsUpdate" crates/` → 15 Treffer, davon **elf** auf `menuNeedsUpdate:`; das Muster gehoert an eine Wortgrenze gebunden.
