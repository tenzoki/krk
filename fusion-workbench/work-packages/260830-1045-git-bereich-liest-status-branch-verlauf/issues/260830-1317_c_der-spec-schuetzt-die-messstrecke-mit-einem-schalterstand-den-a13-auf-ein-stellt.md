...
---
Resolved: Als Nachsatz am Spec berichtigt (`planning/260830-1251_p_spec-…`, Punkt 2), nach der Nutzerentscheidung zu `shared/decisions/260906-0203_*`. Der Nachsatz begruendet die Unberuehrtheit der zehn Zusagen mit dem **Ort des Messplatzes** statt mit einem Schalterstand, wie der Datensatz es als Abnahme verlangt hat, und fuehrt die von ihm genannte Pruefung: `git -C ~/Library/Caches/krk-messplatz rev-parse --show-toplevel` bricht am 260906 mit **exit 128** ab. Die widersprochene Haelfte ist ebenfalls neu gemessen: `grep -n "marke: true" crates/krk-core/src/ablage/sitzung.rs` → `:344` in `Spaltensichtbarkeit::default`, die Markenspalte steht ab Werk auf ein; `Anwendungsdelegierter::gitbedarf_nachziehen` (`crates/krk-ui/src/appkit/anwendung.rs:4823`) rechnet die Oder-Verknuepfung aus Git-Bereich und Markenspalte. Die Warnung fuer den Fall, dass der Messplatz eines Tages in einem Repository liegt, steht im Nachsatz.
