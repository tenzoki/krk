...
---
Resolved: Als Nachsatz am Plan berichtigt (`planning/260814-0656_c_plan-…`, Punkte 2 und 3), nach der Nutzerentscheidung zu `shared/decisions/260906-0203_*`. Die dritte Eingabe der `Decidability:`-Zeile lautet "der gehaltene Stand beider Zettel"; belegt mit `grep -n -A6 "fn zu_sichern" crates/krk-ui/src/zettelmodell.rs` (`:250`, laeuft ueber `Zettel::ALLE` und liest je Zettel `stand.gehalten`). Der vierte Gegenstand des Pruefmoduls ist die Regel "der getippte Stand gewinnt" mit ihren drei Proben; `grep -n` auf ihre Namen in derselben Datei → `:419`, `:440`, `:458`.
