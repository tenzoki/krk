...
---
Resolved: Ohne Änderung geschlossen, gemeinsam mit dem Datensatz, dessen Erhebung er
berichtigt.

Dieser Datensatz verlangt eine Ergänzung an der Tabelle oder eine Zeile über den Umfang der
Erhebung von `260814-0912_*`. Beides ist gegenstandslos geworden: keine der Stellen steht
mehr da. Nachgemessen am 260908 mit demselben Suchmuster über `crates/` — sieben Zeilen, alle
in Prüfdateien, keine davon eine Aussage über den Ablageordner; `crates/krk-core/tests/ablage.rs`,
das dieser Datensatz als den übersehenen Teil des Heuhaufens nennt, trägt heute keine
einzige.

Der blinde Fleck, den er beschreibt — eine Erhebung, die `src/` liest und `tests/` weglässt,
ohne es zu sagen —, ist damit an dieser Stelle keiner mehr, und die Sorte Fehler ist an der
Wurzel gefasst: `keine_prosastelle_der_ablage_nennt_eine_andere_zahl_von_ablagedateien`
(`crates/krk-core/tests/baum.rs`) rechnet die Zahl aus `Datei::ALLE` statt sie zu suchen, und
ihr Doc-Kommentar schreibt aus, wo sie blind ist.
