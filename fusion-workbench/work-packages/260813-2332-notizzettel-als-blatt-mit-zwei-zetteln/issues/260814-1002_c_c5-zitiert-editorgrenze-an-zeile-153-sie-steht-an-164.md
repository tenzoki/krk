...
---
Resolved: Als Nachsatz am Spec berichtigt (`planning/260813-2348_o_spec-…`, Punkt 3), nach der Nutzerentscheidung zu `shared/decisions/260906-0203_*`. **Weder 153 noch 164 trifft heute:** `grep -n "pub const EDITORGRENZE" crates/krk-core/src/text/datei.rs` → `:186`. Die Konstante ist seit diesem Befund ein zweites Mal gewandert, und der Nachsatz zieht daraus den Schluss, den der Befund selbst als Ausweg genannt hat: die Zeilennummer gehoert nicht in das Kriterium, der Name findet die Stelle eindeutig. Die Zusage des Kriteriums — genau eine Stelle im Baum — haelt unveraendert.
