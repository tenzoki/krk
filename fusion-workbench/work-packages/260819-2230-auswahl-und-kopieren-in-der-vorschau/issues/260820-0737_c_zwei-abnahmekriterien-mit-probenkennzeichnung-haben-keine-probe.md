...
---
Resolved: C2.3 und C2.4 haben ihre Proben. Gebaut ist das Modul `durchgaenge` in `crates/krk-ui/src/markdown.rs`, also dort, wo die Proben des Renderns stehen: `genau_ein_durchgang_liest_die_markdown_quelle` (C2.4) zaehlt den Einstieg in den Ereignisstrom der Markdown-Kiste — genau einmal in dieser Datei und in keiner anderen des Baums —, `der_quelltext_wird_kein_zweites_mal_von_der_platte_gelesen` (C2.3) haelt zwei Enden fest: diese Datei ruft keinen Weg des Dateisystems (weder `read_to_string` noch `File::open` noch die Huelle des Kerns noch `bis_zur_grenze_lesen`), und der Quellbezug nimmt seine Quelle aus dem Stand des laufenden Durchgangs. Alle Nadeln stehen zusammengesetzt und die Pruefmodule sind abgeschnitten, damit der Modulkopf, der den Aufruf in seinem Bild abzeichnet, nicht mitzaehlt. Gemessen vor dem Bau mit `grep -rn "into_offset_iter" crates/` → zwei Zeilen, `crates/krk-ui/src/markdown.rs:27` (Modulkopf) und `:582` (der eine Aufruf). **Damit hat der Verzicht auf einen Abnahmelauf gegen L7 seine zugesagten Ersatzpruefer**; ob `shared/decisions/260819-2216_a_schuldet-diese-runde-einen-abnahmelauf-gegen-die-zusage-l7.md` daraufhin auf `_i_` geht, entscheidet dieser Datensatz nicht. `make check` exit 0.
