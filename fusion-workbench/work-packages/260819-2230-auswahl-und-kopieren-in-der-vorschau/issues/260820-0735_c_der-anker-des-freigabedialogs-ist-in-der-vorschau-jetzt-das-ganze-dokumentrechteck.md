...
---
Resolved: Die „Richtung, falls nicht" ist gefahren, und zwar ohne auf den Buendellauf zu warten, weil sie unter **beiden** Ausgaengen richtig ist: `Anwendungsdelegierter::teilen` reicht `flaeche.visibleRect()` statt `flaeche.bounds()` als Ankerrechteck an `teilen::anbieten`. Fuer jede ganz sichtbare Ansicht sind beide Rechtecke dasselbe, fuer die mitwachsende Textanzeige der Vorschau ist der sichtbare Ausschnitt das, was der Anker eines `NSPopover` braucht. Die eine Zuordnung von Fokuswert auf Ansicht aus C1.8 ist unberuehrt; geaendert ist das Rechteck. Damit ist der beschriebene Zusammenhang aufgeloest, und die Buendelbeobachtung entscheidet nichts mehr.
