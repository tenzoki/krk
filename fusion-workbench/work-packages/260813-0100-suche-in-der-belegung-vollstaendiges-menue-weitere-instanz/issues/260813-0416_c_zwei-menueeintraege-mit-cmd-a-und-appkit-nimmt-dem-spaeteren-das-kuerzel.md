...
---
Resolved: Geschlossen: der Defekt war im selben Schritt behoben, und die Nutzerfrage, die den Datensatz offen hielt, ist beantwortet **und** umgesetzt. `decisions/260813-0430_*_wer-bekommt-das-menuekuerzel-wenn-zwei-funktionen-sich-eine-kombination-teilen.md` steht auf `_i_`: bei geteilter Kombination zeigt die Textfeld-Funktion das Menuekuerzel und der KRK-Befehl keines, also genau das, worauf die Runde gefahren ist. Am Baum nachgelesen am 260908: `menuemodell::zugestellte_kuerzel` steht, der Modulkopf von `crates/krk-ui/src/menuemodell.rs` schreibt die Regel aus und nennt keine Zahl von Doppelungen mehr — ausgeliefert sind es seit dem 260907 zwei, `cmd+a` und `cmd+f`. Die zwei Saetze in `resources/default-keymap.toml`, die der `ontorev` am 260813-0532 empfohlen und die dieser Datensatz wegen der damals offenen Frage zurueckgestellt hat, stehen dort weiterhin nicht; sie sind jetzt schreibbar, gehoeren aber dem `ontocoder` und stehen als eigener Datensatz `260908-0719_*_zwei-saetze-zur-kuerzelregel-fehlen-in-default-keymap-toml-seit-die-frage-dahinter-beantwortet-ist.md`. Der zweite Rest — was ein ausgegrauter Eintrag mit einer Tastenentsprechung tut — ist am Baum nicht entscheidbar und bleibt Nutzerarbeit am Buendel, wie der Datensatz selbst sagt.
