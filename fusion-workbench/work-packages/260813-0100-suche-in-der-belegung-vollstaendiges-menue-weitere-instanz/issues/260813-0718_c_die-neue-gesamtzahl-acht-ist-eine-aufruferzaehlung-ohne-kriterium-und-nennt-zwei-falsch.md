...
---
Resolved: Behoben auf dem Weg, den der Datensatz vorschlaegt: "Die zweite Zahl streichen und die erste ehrlich beschriften." Die zweite Zusicherung ist aus `der_delegierte_wird_an_genau_drei_stellen_um_einen_befehl_gebeten` (`crates/krk-ui/src/appkit/menue.rs`) entfernt; die Zusicherung auf die drei Frager bleibt, weil C2.14 diese Zahl selbst zusagt. Der Doc-Kommentar schreibt jetzt aus, warum die Gesamtzahl fiel: sie war der Stellvertreter, den der Kopf von `crate::quellbaum` ausschliesst, und ihre Erklaerung zaehlte die zwei Aufrufe des Messmodus an `Tabellenquelle::kommando_ausfuehren` als Weiterreichungen mit, obwohl sie am Delegierten vorbeigehen. Nachgemessen am `2fa1d0e`: die Gesamtzahl stand inzwischen auf neun, zusammengesetzt aus drei Fragern, vier Weiterreichungen (Leiste, Vorschau, Git, Tabelle) und den zwei Messmodus-Aufrufen; die Beschriftung sprach von sechs Weiterreichungen. Die benannte Blindheit der verbliebenen Zahl (der Empfaengername) steht weiter dort und nennt jetzt auch den Git-Bereich. Geprueft: cargo test -p krk-ui 908 gruen, clippy/fmt/doc je Exit 0.
