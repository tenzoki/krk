...
**Date:** 2026-08-27
**Status:** Complete — gebaut und vom Nutzer am 260827 abgenommen; alle acht Schritte auf [DONE], Abgleich `history/260827-1907-reconciliation.md`
...
5. **Die Proben, die die Zusagen ohne Fenster halten** [DONE]
   - Executor: `coder`
...
**260827-1907, reconciler:** Alle acht `[DONE]` einzeln gegen den Baum gelesen, jede hält. Schritt 1: `3ee2638`; `Baustein::Zaehlung` trägt `typ: Option<Typ>` und `versteckt: bool` (`crates/krk-core/src/leseprofil/mod.rs:331,336`), `Wert::ZahlMitVersteckten` (`mod.rs:748`) mit dem Zweig `"{zahl} ({versteckt})"` (`mod.rs:814`), `zaehlen` mit den zwei Kriterien und `UeberGrenze` vor `ZahlMitVersteckten` (`bausteine.rs:796-812`), `trifft` als private Einfallfrage (`bausteine.rs:827`). Schritt 2: `9f91f92`; `Zaehlungsdatei { typ: Option<Typdatei>, versteckt: Option<bool> }` (`datei.rs:250,258`), `Typdatei` mit `rename_all = "lowercase"` (`datei.rs:281-282`), `fn typ` ohne Auffangzweig (`datei.rs:575-580`), `unwrap_or(false)` (`datei.rs:451`). Schritt 3: `bf3a91d`; `defaultprofil.rs` mit `LazyLock` (`:81`), den drei Beschriftungen in der Reihenfolge Dateien, Ordner, Verknüpfungen (`:67-69`), `Ortsangabe::wurzel()` und `versteckt: true` (`:88,91`), `pub fn defaultprofil` (`:103`); `Auskunft` (`mod.rs:645-653`), `zeilen_als_text` mit `#[must_use]` (`mod.rs:674-675`); `zusammenfassen`/`zusammenfassen_gezaehlt` liefern `Auskunft` (`bausteine.rs:258,281`), Rückfallzweig über `symlink_metadata` am ausgewählten Pfad (`bausteine.rs:304,322`). Schritt 4: `5e506e6`; `Inhalt::Metadaten { metadaten, zaehlzeilen }` (`vorschaumodell.rs:305`), vollständige Verzweigung über `Auskunft::Erkannt`/`Auskunft::Default`/`None` (`vorschaumodell.rs:749-757`), `metadaten_text` hängt `zeilen_als_text` hinter „Typ" an (`appkit/vorschau.rs:1384,1414`), Doc-Kommentar von `sitzung_laden` berichtigt und nennt den Datensatz `260827-1322_o_` (`appkit/anwendung.rs:1518-1526`). Schritt 5: `891f313`; die Proben stehen unter den im Plan genannten Namen (`tests/leseprofil.rs:2324,2459,2508,2548,2592,2669,2729,705`; `tests/baum.rs:183,230`; `ablage/leseprofile.rs:182,192`; `vorschaumodell.rs:1377,1431`; `appkit/vorschau.rs:1704`). Schritt 6: `c072de7`; der Diff an `resources/default-readers.toml` besteht allein aus Kommentarzeilen, die zwölf `[[profil]]`-Blöcke stehen unverändert (`:141-159`, `:236-251`). Schritt 7: `162058f`; der Defektdatensatz `issues/260827-1710_o_…` steht, zitiert den Spec der Runde 16 mit gesterntem Marker und bleibt planmäßig offen. Schritt 8: `d444879`; der Nutzer hat den Lauf am Bündel auf `c072de7` (1.2.1) gefahren, der Commit setzt allein die zwei Statuszeilen. `make check` am 260827-1907 grün, 1660 Proben, 0 Fehler. Stop-Bedingungen: `grep -oE '"L[0-9]+"' crates/krk-bench/src/messen.rs | sort -u` liefert auf `a5c7a46` und auf `d444879` dieselben zehn Zusagen L1 bis L10; die zwei beantworteten Entscheidungsdatensätze tragen jetzt ihre `Implemented:`-Zeile und stehen auf `_i_`; der Messmodus-Datensatz `260827-1322_o_` bleibt offen und ist keine Vorbedingung. Dateimarker `_p_` → `_c_`.
