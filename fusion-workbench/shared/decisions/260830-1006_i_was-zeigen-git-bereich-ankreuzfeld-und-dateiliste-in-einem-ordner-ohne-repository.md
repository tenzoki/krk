...
Implemented: 7264daf — alle drei Anzeigen tragen je die erste Möglichkeit. Der Satz steht als `krk_core::git::texte::KEIN_REPOSITORY` („Dieser Ordner liegt in keinem Git-Repository.", `crates/krk-core/src/git/texte.rs:29`) und kommt über `Kopf::KeinRepository` in die Kopffläche. Das Ankreuzfeld zeigt eine Einstellung und keine Lage: `Fenstermodell::sichtbar_setzen` bleibt der eine Schreiber und hat außerhalb von `fenstermodell.rs` keinen Rufer, im Gitweg keinen einzigen (`crates/krk-ui/src/appkit/git.rs:41` schreibt es aus). Die Markenspalte steht und bleibt leer: `Spalte::Marke` ist ab Werk sichtbar (`Spaltensichtbarkeit::default`, `crates/krk-core/src/ablage/sitzung.rs:344`) und `Tabellenquelle::beschriften` liefert ohne Befund die leere Zeichenkette. Die Anzeigehälfte (C6.1, C6.2, C6.3) ist Nutzerarbeit aus Schritt 17 und noch nicht abgenommen.
