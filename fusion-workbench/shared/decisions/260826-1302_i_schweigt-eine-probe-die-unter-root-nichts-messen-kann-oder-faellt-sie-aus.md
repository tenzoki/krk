...
---
Implemented: 260907-0858-drei-kleine-nutzerantworten-in-code.md — die Antwort steht als eine Funktion in `crates/krk-core/tests/gemeinsam/mod.rs`, `rechtesperre_haelt_oder_abbruch`, deren Doc-Kommentar die Begruendung und die Voraussetzung „dieser Baum wird nie unter root geprueft" traegt; acht Aufrufstellen in fuenf Pruefdateien des Kerns. **Der realisierte Bestand ist groesser als die vier dieses Datensatzes:** still uebersprungen wurde an drei Stellen und nicht an zweien (die dritte ist `eine_datei_ohne_leserecht_traegt_nichts` in `tests/verzeichnis.rs`), ungeprueft blieben ebenfalls drei (die dritte ist `ein_nicht_lesbarer_ordner_gilt_als_kein_treffer` in derselben Datei), und die zwei bestehenden Eigenformen in `tests/ablage.rs` und `tests/leseprofil.rs` gehen jetzt auf dieselbe Funktion. Eine dritte Klasse, die dieser Datensatz nicht kennt — grün unter root und dabei ohne Beleg —, ist nicht mitentschieden und steht als `260907-0858_*_zwei-proben-bleiben-unter-root-gruen-und-verlieren-dabei-ihren-beleg.md`; `krk-ui` erreicht die Funktion nicht und steht als `260907-0858_*_fuenf-proben-in-krk-ui-fahren-die-rechtesperre-frage-ohne-die-eine-antwort-des-kerns.md`.
