...
---
Resolved: Der zweite Weg, der mit der Vorlage im selben Modul.
`kind_mit_deskriptorgrenze` (`crates/krk-core/tests/gemeinsam/mod.rs`) nimmt jetzt als
erstes Argument `zusage: &str` und stellt den Satz seiner Abbruchmeldung voran, wie
`mit_zeitschranke` es mit seinem `was` vorführt; damit steht die fachliche Zeile an der
einen Stelle, an der sie auch ausgelöst wird. Der Starter gibt nichts mehr zurück, und die
Rufer haben ihr totes `assert!` verloren. **Es sind sieben und nicht sechs:** seit `fc829c8`
ist `ein_deskriptormangel_laesst_den_gitbefund_unentschieden` (`tests/git.rs`) als siebter
dazugekommen, mit derselben Bauart und demselben Befund. Modulkopf und Doc-Kommentar des
Starters nennen die alte Bauart und den Grund ihres Wegfalls; die drei Prosastellen, die die
fachliche Zeile beim Rufer behaupteten, sagen es nicht mehr.
