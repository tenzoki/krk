...
---
Resolved: Die erste der zwei Fassungen, die "Richtung" nennt.
`das_msdos_feld_traegt_die_ortszeit_des_quelldatums` (`crates/krk-core/tests/operation.rs`)
haelt am Anfang `zonenversatz(SOMMER) != zonenversatz(WINTER)`; der Doc-Kommentar nennt die
Bedingung, die drei Zonenlagen, in denen sie faellt, und den Umzug in einen Kindprozess mit
gesetztem `TZ` als die Fassung, die ueberall messen wuerde.

Der Versatz wird **gerechnet und nicht abgefragt**, denn `Ortszeit` fuehrt kein Versatzfeld:
der neue Helfer `zonenversatz` stellt die buergerliche Tagesminute gegen die UTC-Tagesminute
aus den Epochensekunden. Ein Vergleich der blossen Stunden traege nicht — die zwei
Zeitpunkte liegen auf verschiedenen UTC-Stunden (12:30:45 und 09:30:45) und waeren auch
unter `TZ=UTC` verschieden; die Rechnung in Minuten deckt daneben Zonen mit halbem Versatz.

Nachgemessen am 260908: in `Europe/Berlin` gruen, unter `TZ=UTC` rot mit dem neuen Satz
(`left: 0, right: 0`). Die Nachbarprobe `der_rundweg_erhaelt_das_aenderungsdatum_…` bleibt
unangetastet: sie traegt dieselbe Zonenabhaengigkeit, aber nicht diese Zusage.
