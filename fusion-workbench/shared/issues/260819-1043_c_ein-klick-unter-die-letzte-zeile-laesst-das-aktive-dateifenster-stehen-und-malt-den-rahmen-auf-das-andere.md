...
---
Abgleich 260819-1440 (reconciler): **geschlossen bestätigt, am Baumstand `77dcd48` nachgelesen.** Die Behebung steht in `76ceb68`; die `Resolved:`-Notiz darüber beschreibt sie, nennt aber keinen Commit, und das ist hier nachgetragen. Im Baum: `Anwendungsdelegierter::aktives_dem_ersthelfer_nachziehen` (`crates/krk-ui/src/appkit/anwendung.rs:4285`) hängt am Melder des Ersthelferwechsels (`:1130`), und zwar **vor** `fokusanzeige_nachziehen`, damit die Anzeige schon mit dem neuen `aktiv` rechnet. Damit kann der beschriebene Fall nicht mehr eintreten: `aktiv` folgt jedem Rangwechsel in ein Dateifenster, auch ohne Zeilentreffer. Die bindende Antwort trägt seit diesem Abgleich `_i_` (`shared/decisions/260819-1043_*_welche-flaechen-holen-den-fokus-wenn-man-hineinklickt.md`).
