...
---
Resolved: In `crates/krk-ui/src/appkit/belegungsansicht.rs` ist das Feld
`anzeige: &'static str` aus `Schaltflaechentaste` gefallen und durch
`zeichen: Schaltflaechenzeichen` ersetzt. Die neue Aufzählung hat zwei Werte,
`Buchstabe(char)` und `Eingabetaste`, und trägt zwei vollständige
Fallunterscheidungen ohne Auffangzweig: `tastenzeichen()` liefert, was
`setKeyEquivalent` bekommt, `anzeige(mit_befehl)` liefert, was die
Erläuterungszeile nennt. Ein dritter Wert hält damit den Bau an.

Der Weg ist damit nicht der aus dem Vorschlag ("ein Feld `zeichen` daneben, die
Probe hält `anzeige` gegen `zeichen`"), sondern der eine Schritt weiter: die
Anzeigeform wird **gerechnet** statt danebengeschrieben, also gibt es die zwei
Zeichenketten nicht mehr, die auseinanderlaufen könnten, und die Probe braucht
keinen Abgleich zwischen ihnen zu führen. `taste_setzen` nimmt kein
`zeichen`-Argument mehr; die zwei Aufrufstellen mit `ns_string!("t")` und
`ns_string!("r")` sind gefallen. `erlaeuterung()` und die Probe
`die_erlaeuterung_nennt_die_drei_kuerzel_und_die_suche` gehen über
`Schaltflaechentaste::anzeige()`.

Zum Zusatz am Ende des Befunds, "`Funktionsbereich` trägt heute neun Werte, die
Zahl stimmt": **sie stimmte schon beim Filen nicht.** Gemessen mit
`awk '/pub enum Funktionsbereich/,/^}/' crates/krk-ui/src/belegungsmodell.rs`
trägt die Aufzählung zehn Werte; `Git` ist in der Git-Runde dazugekommen. Der
Doc-Kommentar an `TABELLENHOEHE` nennt jetzt keine Zahl mehr, sondern das
Zählkommando, und schreibt beide gefallenen Zahlen — Funktionen und
Überschriften — mit ihrer Herkunft aus.

Beleg: `cargo test -p krk-ui belegungsansicht` → 3 bestanden;
