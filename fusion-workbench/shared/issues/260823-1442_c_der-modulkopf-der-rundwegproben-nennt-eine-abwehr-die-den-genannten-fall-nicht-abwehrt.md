...
---
Resolved: Der erste der beiden Wege, den der Datensatz auch vorzieht. `rundwegproben::das_schliessen_hat_genau_die_zwei_gelesenen_rufer` zaehlt die Aufrufstellen von `editor_schliessen` ueber `quelldateien()` und `aufrufstellen` und verlangt zwei — dieselbe Bauform, die `crate::kommandos::rundweg` fuer `rundweg` schon benutzt, und der Name steht als `concat!` da, damit die Probe sich nicht selbst zaehlt. Der Modulkopf sagt jetzt, dass die vier Proben den dritten Rufer **fuer sich** nicht sehen, nennt die neue Probe als die Abwehr und sagt ausdruecklich, dass `die_regel_hat_genau_einen_aufrufer` es nicht ist: die zaehlt die Rufer der reinen Funktion `rundweg` und sagt ueber `editor_schliessen` nichts.
