...
---
Resolved: Die drei Entscheidungsdatensaetze in `werkbankgestalt`
(`crates/krk-core/tests/leseprofil.rs`) gehen jetzt durch denselben
`geaendert_setzen`-Aufruf wie die vier Verlaufsdateien, mit derselben Staffelung von je
sechzig Sekunden; die Zusicherung auf `Dritte Frage?` steht damit auf einer gesetzten Zahl
statt auf der Geschwindigkeit des Dateisystems. Der Doc-Kommentar von `geaendert_setzen`
sagt jetzt, dass er fuer jeden Bestand mit einer Reihenfolgezusicherung gilt, und nennt fuer
`decisions` die Umkehrung durch den namensaufsteigenden Zweitschluessel. `runde()` bleibt
unangetastet: auf die Reihenfolge seiner drei Entscheidungsdatensaetze zeigt keine
Zusicherung, und die Zeile gehoert dort nach dem Datensatz selbst nicht hin.
