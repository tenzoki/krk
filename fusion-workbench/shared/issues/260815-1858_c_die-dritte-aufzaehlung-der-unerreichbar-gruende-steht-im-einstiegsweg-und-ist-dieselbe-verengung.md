...
---
Resolved: Derselbe Halbsatz nachgezogen, den `260815-1845` fuer die anderen zwei Stellen vorgibt: der Kommentar am `Unerreichbar`-Zweig von `in_zeile_einsteigen` (`crates/krk-ui/src/appkit/tabelle.rs`) nennt „ins Leere, im Ring oder ohne Recht" jetzt als **Beispiele** und sagt, dass der Zweig jeden Fehlschlag von `stat(2)` faengt — samt den zwei Faellen, die unter keines der drei fallen (ein zu langer Name, `errno 63`, und `ELOOP` ab 32 aufgeloesten Verknuepfungen ohne Ring). Der zweite Satz, die Statuszeile aus C1 als die eine Meldeflaeche, steht unveraendert.
