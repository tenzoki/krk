...
---
Resolved: `lesezeichen_aendern` liefert `bool` mit `#[must_use]`: `true` heisst, dass es **nichts** in die Befehlsantwort geschrieben hat und der Aufrufer seine Erfolgsmeldung stellen darf. Vier Ausgaenge liefern `false` — die nicht zu nehmende Schreibsperre, ein Hinweis aus dem Laden, ein verschwundenes Lesezeichen, ein gescheitertes Sichern. `lesezeichen_anlegen_ausfuehren` meldet „angelegt" nur noch darauf. Die Frage ist „steht schon eine Meldung" und nicht „ist geschrieben worden": ohne Ablageordner wird gerechnet, nicht geschrieben und nichts gemeldet, wie der Doc-Kommentar es vorsieht. Die drei Geschwister schreiben `let _ =`.
