...
---
Resolved: Der Doc-Kommentar von `sichtbarkeit_aendern` (`crates/krk-ui/src/appkit/anwendung.rs`) sagt jetzt beides, was der Datensatz verlangt: dass die Stelle nicht misst, und warum sie es nicht muss — ihre zwei Rufer `bereich_umschalten` und `bereich_einblenden` werden allein aus `kommando_ausfuehren`, `anlass_ausfuehren` und `editorausgang_behandeln` erreicht, und alle drei messen; zwischen jener Messung und diesem Griff in das Modell kann keine Ziehbewegung liegen. Die Regel an `bildschirmbreiten_uebernehmen` traegt damit an allen fuenf Adressen etwas, und `aktives_setzen` bleibt der einzige, an dem ein offener Befund steht statt einer Begruendung.
