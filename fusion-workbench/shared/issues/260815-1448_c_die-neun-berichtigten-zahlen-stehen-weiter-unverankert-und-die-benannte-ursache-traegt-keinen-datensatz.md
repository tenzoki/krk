...
---
Resolved: Am Baum `8779a25` steht keine der neun Zahlen mehr da. Gewählt ist der zweite der beiden Wege, den dieser Datensatz zur Entscheidung gestellt hat — die Zahl aus der Prosa nehmen, wie `CLAUDE.md` es für dieselbe Familie schon getan hatte —, und zwar durch den Behebungsdurchgang an `krk-ui` (`c5f6fab`, Datensatz `260905-2046_*_drei-prosastellen-in-menue-rs-nennen-85-funktionen-die-belegung-fuehrt-92.md`). `crates/krk-ui/src/appkit/menue.rs` nennt im Abschnitt „Wer die sechs Textbefehle beantwortet" statt einer Zahl die zwei Zählkommandos `grep -c '^id = ' resources/default-keymap.toml` und `grep -c 'gehalten_von = "menue"' resources/default-keymap.toml`; `crates/krk-ui/src/belegungsausgabe.rs` hält im Abschnitt „Die vier Begründungslagen der dritten Spalte" die erste Lage an `Kommando::KENNUNGEN.len()` und schreibt dazu, dass eine Zahl deshalb nicht dasteht. Nachgeprüft mit einem Lauf über die Doc-Kommentare beider Dateien: keine zweistellige Zahl dieser Bedeutung mehr, allein Runden- und Datumsangaben. Damit ist auch die im Titel genannte Ursache erledigt — sie trug mit diesem Datensatz ihren eigenen Eintrag und ist über ihn behoben worden.
