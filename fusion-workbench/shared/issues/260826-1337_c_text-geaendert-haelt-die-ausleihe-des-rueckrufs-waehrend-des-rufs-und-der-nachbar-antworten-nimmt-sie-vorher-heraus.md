...
---
Resolved: `Eingabewaechter::text_geaendert` (`crates/krk-ui/src/appkit/blaetter/mod.rs`) klont den Rueckruf aus der Ausleihe heraus, bevor es ihn ruft; das Feld traegt dafuer ein `Rc<dyn Fn()>` statt eines `Box<dyn Fn()>`. `take` waere hier falsch — anders als in `antworten` gilt der Rueckruf fuer jede weitere Aenderung —, und der Kommentar an der Stelle nennt beide Gruende: der Rueckruf schreibt nach AppKit hinein, und `textaenderung_melden` haelt ein `borrow_mut` auf dieselbe Zelle.
