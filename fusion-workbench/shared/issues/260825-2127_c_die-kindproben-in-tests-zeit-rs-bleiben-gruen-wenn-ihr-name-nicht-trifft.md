...
---
Resolved: Moeglichkeit 1. `kind_ist_durchgelaufen` (`crates/krk-core/tests/zeit.rs`) haelt
neben dem Rueckgabewert jetzt, dass die Ausgabe des Kindes `test result: ok. 1 passed;`
traegt; der Satz steht als `EIN_KIND_GELAUFEN` mit seiner Begruendung daneben, unter
demselben Namen, den `gemeinsam::kind_mit_deskriptorgrenze` dafuer fuehrt (diese Datei
fuehrt `gemeinsam` nicht und kann ihn nicht ausleihen). Der Modulkopf sagt jetzt, dass die
abgeschriebene Form aus zwei Haelften besteht. Ein verfehlter Kindname macht beide
Elternproben damit rot statt still gruen.
