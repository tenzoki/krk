...
---
Resolved: Gemessen. `tests::der_verwalter_gibt_den_block_auf_allen_vier_wegen_frei` (`crates/krk-ui/src/appkit/editor.rs`) meldet einen Block an, der eine `Stapellast` haelt, und faehrt die vier Wege des Kopfes je an einem eigenen Verwalter: `removeAllActions`, `undo`, die neue Anmeldung nach einem `undo` und das Fallenlassen des Verwalters. Alle vier geben frei. **Bedingung ist ein offener `autoreleasepool`** — ohne ihn faellt der Zaehler auf keinem der vier Wege; der Kopf von `Stapellast` hielt das bis dahin nur fuer moeglich und traegt es jetzt als gemessen. Weg 4 braucht daneben `setGroupsByEvent(false)`, weil ein Verwalter in der Werksbetriebsart von der Laufschleife des Pruefstands festgehalten wird. Ein Umlauf ueber `runMode:beforeDate:` ersetzt den Freigabeverbund nicht: er kehrt ohne Eingabequelle sofort zurueck.
