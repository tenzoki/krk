...
---
Resolved: Beide Haelften des Vorschlags, und zusammen schliessen sie die Luecke. Erstens: `Blatt::zeigen` liefert den `Blattgriff` zurueck (`#[must_use]`) statt ihn fallen zu lassen; die fuenf Eingabeblaetter reichen ihn durch, und die sechs Aufrufstellen in `anwendung.rs` legen ihn ueber `blatt_oeffnet` in `offenes_blatt`. Zweitens: der erste Rang von `abbrechen` fragt `blatt_steht()`, bevor er den Griff nimmt, und antwortet bei einem stehenden Blatt **ohne** Griff `false` — die Taste geht damit an AppKit zurueck und beantwortet die Schaltflaeche des Blattes, statt auf Rang 2 und 3 zu fallen. Diese zweite Haelfte deckt auch den einen Rest: `pfadeingabe::zeigen` wird aus `tabelle.rs` geoeffnet, das den Schlitz des Anwendungsdelegierten nicht erreicht, und laesst den Griff mit `let _ =` und einem Satz dazu fallen. Der Guertel dazu: ein liegengebliebener Griff eines Eingabeblattes kann den Rang nicht mehr ausloesen, weil dieser zuerst nach einem stehenden Blatt fragt. Die Zusage „wer den Griff nicht braucht, laesst ihn fallen; das schadet nicht" am `Blattgriff` ist berichtigt.
