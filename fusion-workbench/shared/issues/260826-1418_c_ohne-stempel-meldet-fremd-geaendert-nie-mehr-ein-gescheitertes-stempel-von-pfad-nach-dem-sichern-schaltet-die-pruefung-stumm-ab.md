...
---
Resolved: Behoben auf dem im Datensatz genannten ersten Weg. `Editormodell::fremd_geaendert` (`crates/krk-ui/src/editormodell.rs`) trennt jetzt die zwei Lagen: ohne Pfad `false` (der Editor haelt keine Datei), mit Pfad und ohne Stempel `true` (der Vergleich hat seine Bezugsgroesse verloren). Damit meldet `fremdaenderung_melden` weiter und `sichern` ueberschreibt nicht mehr ungeprueft. Der Preis steht am Doc-Kommentar ausgeschrieben: in dieser Lage sichert der Editor bis zum naechsten Oeffnen nicht mehr, und das ist die teurere Seite des Irrtums und die richtige. Neue Probe `eine_gehaltene_datei_ohne_stempel_gilt_als_geaendert`; der ueberholte Kommentar in `der_stempel_kennt_eine_aenderung_von_aussen`, der die alte Antwort `false` als Begruendung fuehrte, ist nachgezogen. Geprueft: cargo test -p krk-ui Exit 0.
