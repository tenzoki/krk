...
---
Resolved: Die Zahl ist an beiden Probenkoepfen gestrichen und durch den Zeiger auf den Iterator ersetzt: `der_namenszusatz_gilt_auf_jedem_rang` sagt jetzt „auf jedem, den `dateifenster_raenge` liefert" und traegt daneben, warum keine Zahl dasteht; `ueber_alle_zwoelf_bewerber_gewinnt_genau_eine_aussage` und `die_art_haengt_am_rang_und_nicht_an_der_seite` ebenso (`crates/krk-ui/src/appkit/statuszeile.rs`). **Die fuenf Stellen der `Also seen:`-Zeile sind mit erledigt**, alle nach derselben Regel entnumeriert: `crates/krk-ui/src/appkit/anwendung.rs` dreimal („auf ihren obersten Rang" bzw. „der oberste der Rangfolge"), `crates/krk-ui/src/appkit/editor.rs` einmal („ihren obersten Rang") und `crates/krk-ui/src/appkit/tabelle.rs` einmal („der oberste Rang der Zeile"). Gepruefte Abnahme: `cargo test -p krk-ui` — exit 0.
