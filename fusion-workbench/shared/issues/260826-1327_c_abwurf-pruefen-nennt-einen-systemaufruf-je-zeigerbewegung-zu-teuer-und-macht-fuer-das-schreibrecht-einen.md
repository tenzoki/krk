...
---
Resolved: Der erste Grund ist gestrichen, wie der Datensatz es als eine der zwei Moeglichkeiten nennt. Der Doc-Kommentar von `abwurf_pruefen` (`crates/krk-ui/src/appkit/tabelle.rs`) sagt jetzt, dass nicht der Preis von `st_dev`/`st_ino` entscheidet, sondern dass selbst dann eine Vorhersage bliebe; daneben steht, dass das Preisargument bis zum 260907 dort stand und nicht trug, weil dieselbe Funktion drei Zeilen tiefer `abwurf::beschreibbarkeit` fragt und das ueber `resourceValuesForKeys:` genau so einen Aufruf je Zeigerbewegung macht. Der Vergleich selbst bleibt unveraendert.
