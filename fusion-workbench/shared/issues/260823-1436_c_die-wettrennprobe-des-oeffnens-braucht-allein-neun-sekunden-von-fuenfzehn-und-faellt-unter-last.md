...
---
Resolved: Beide Vorschlaege, und der erste in der zweiten der zwei dort genannten Formen —
„den Lesefaden seinen Fortschritt melden lassen und die Frist auf ‚seit N Sekunden kein
Fortschritt' stellen", weil die andere Form die Frist an der gemessenen Dauer bemesse und
damit die Wanduhr behielte.

`ein_wechsel_der_art_unter_dem_oeffnen_haelt_nichts_an` (`crates/krk-core/tests/text.rs`)
hat kein `recv_timeout` ueber den ganzen Lauf mehr. Der Lesefaden schreibt vor jedem
`datei::oeffnen` die Nummer seines Durchlaufs in einen Zaehler; der Elternteil wacht alle
`STILLSTANDSFRIST` (zehn Sekunden) auf und urteilt allein dann, wenn sich der Zaehler in der
ganzen Frist nicht bewegt hat. Damit misst die Probe die Frage, die sie stellt — haengt ein
einzelner Aufruf —, statt der Belegung der Maschine. Die Meldung nennt jetzt den erreichten
Durchlauf und die Zahl der Tausche und schreibt aus, was sie **nicht** trennt: eine
Maschine, die eine ganze Frist lang gar nichts rechnet, saehe aus wie ein Haenger.

Gemessen am 260908 an diesem Baum: allein gefahren 2,4 s, unter 24 Lastschleifen auf 16
Kernen 10,5 s, beide Male gruen. Unter der alten Bauart waere der zweite Lauf mit 10,5 s von
15 s knapp an der Frist gewesen. Die Lastschleifen sind nach der Messung beendet.

Der Datensatz `260823-1210_*_ein-make-check-von-neun-ist-mit-2-abgebrochen-*` ist weiter
**nicht angefasst**, wie dieser Datensatz es vorsieht.
