...
---
Resolved: Beide Halbsaetze.

**Die Probe.** `ein_abbruch_im_stapel_kehrt_binnen_100_ms_zurueck_und_meldet_die_umbenannten`
(`crates/krk-core/tests/operation.rs`) nimmt jetzt `ZEITMESSUNG` und faehrt fuenf Versuche,
in der Bauform der Nachbarin: die drei Zusicherungen ueber das Verhalten des Kerns gelten in
jedem einzelnen Versuch, allein die 100-ms-Frist darf ein Versuch reissen, und der
Fehlschlag nennt jede gemessene Spanne. Die Zahl aus C4 ist nicht gedehnt. Der Abbruch
selbst hing schon vorher an einem Ereignis (die Fortschrittsmeldung ueber 1.000 Eintraege)
und nicht an einer Wanduhr; was an der Wanduhr hing, war die Messung, und dafuer ist die
Bauform der Nachbarin die passende und nicht die des Entpackabbruchs.

**Der Modulkopf.** Die Zahl ist gestrichen statt auf vier gesetzt, wie es dieser Baum an
einem Dutzend Stellen schon getan hat; an ihre Stelle tritt
`grep -n 'ZEITMESSUNG$' crates/krk-core/tests/operation.rs` (heute fuenf Zeilen, denn die
Stapelprobe ist als fuenfte dazugetreten). Daneben steht jetzt, was die Sperre **nicht**
leistet: sie trennt die Zeitmessungen voneinander und nicht von den uebrigen Proben der
Datei.
