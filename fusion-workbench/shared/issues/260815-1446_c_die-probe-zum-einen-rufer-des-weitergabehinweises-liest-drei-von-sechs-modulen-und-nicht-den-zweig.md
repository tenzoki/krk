...
---
Resolved: Auf dem zweiten der beiden vorgeschlagenen Wege behoben, strukturell. Der
Zweigkörper des Unterbefehls `bundle` steht als benannte Funktion `bundle_fahren`
(`xtask/src/main.rs`) statt als Stelle im `match`; sein Doc-Kommentar schreibt aus, warum.
`allein_der_unterbefehl_bundle_gibt_den_hinweis_aus` (`xtask/src/sign.rs`) läuft jetzt über
**jede** `.rs`-Datei unter `xtask/src` statt über drei benannte — ein neues Modul kommt von
selbst dazu, statt stumm zu bleiben — und hält danach, dass die eine Fundstelle im Rumpf von
`bundle_fahren` liegt. Damit trägt sie beides, was der alte Doc-Kommentar zusagte: dass es
genau einen Ausgabeort gibt und wo er liegt. `sign.rs` bleibt mit Grund außen vor, dort steht
die Funktion selbst. Die Verschiebung, gegen die die Probe geschrieben ist — die Zeilen des
Aufrufs aus dem `bundle`-Zweig in den `release`-Zweig —, lässt sie jetzt rot werden.
