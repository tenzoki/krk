...
---
Resolved: Am 260908 am Baum nachgemessen: `grep -rn "sieben Stationen"` über den Quellbaum
ohne `.git`, `target` und `fusion-workbench` liefert **keinen** Treffer. Die sieben Stellen
der ersten vier Zeilen der Tafel oben — `README.md`, `xtask/src/version.rs`,
`xtask/src/main.rs`, `xtask/src/release.rs` — sind nachgezogen, und gehalten wird das von
`der_quellbaum_nennt_die_alte_stationszahl_nicht_mehr` (`xtask/src/release.rs`), deren Nadel
als `concat!("sieben ", "Stationen")` steht und sich damit nicht selbst mitzählt.

Die Zusage des Plans vom 260821-1221 — enger als C6.3, nämlich auf den Quellbaum begrenzt —
ist damit erfüllt. Das Kriterium C6.3 im Spec und die drei Aufzeichnungen eines Standes
behalten ihren Wortlaut nach der Ortsregel; dieser Datensatz ist der Beleg, warum sie nicht
mitzuzählen sind, und er hat seinen Zweck erfüllt.
