...
---
Resolved: Moeglichkeit 1 und 2 zusammen, wie dieser Datensatz sie empfiehlt; Moeglichkeit 3
ist ausdruecklich nicht genommen.

Beide Meldungen von `die_drei_groessten_mitgelieferten_profile_bleiben_unter_ihren_zahlen`
(`crates/krk-core/tests/leseprofil.rs`) nennen jetzt die geaenderte Zeilenreihenfolge in
`resources/default-readers.toml` als zweiten moeglichen Grund neben dem, den sie behaupten —
am Speicherprofil, dass die Zaehlungszeile nicht mehr vor der Datumszeile steht, am
Projektwurzelprofil, dass die sieben Zeilen umgestellt wurden.

Der Doc-Kommentar der Probe traegt die Kopplung an einer Stelle ausgeschrieben, unter der
Ueberschrift "Zwei Behauptungen haengen an der Zeilenreihenfolge der Profildatei": beide
Stellen, die Bauart des `step_by(2)` und der ausgeschriebenen Werteliste, warum die
Beschriftungsliste darueber eine Umstellung nicht abfaengt, und warum die Kopplung nicht
aufgehoben wird, solange die Richtung rot bleibt.
