...
---
Resolved: Der Kopf von `einfaerbung_anfordern` benennt die zweite Abschrift als angenommenen Preis, mit dem Abstand zur ersten (ein `memcpy` gegen eine Umschreibung aus UTF-16), mit der Einschraenkung, dass sie nur der Anschlag zahlt, der einen Lauf **startet**, und mit dem Grund, aus dem ein `Arc<str>` am `Editormodell` nicht in Frage kommt. Eine Zahl steht dort nicht: gemessen ist sie nicht, und der Datensatz verlangt sie nicht.
