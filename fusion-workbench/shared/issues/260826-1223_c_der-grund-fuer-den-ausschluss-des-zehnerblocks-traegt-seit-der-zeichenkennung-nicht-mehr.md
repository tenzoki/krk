...
---
Resolved: Die vier falschen oder irrefuehrenden Prosastellen sind richtiggestellt; die Auskunft steht jetzt an genau einer Stelle, im Modulkopf von `crates/krk-core/src/tasten/parser.rs` unter „Was der Zehnerblock ausloest, und was nicht". Sie nennt die Regel statt einer Zahl: eine Taste des Blocks loest genau dann aus, wenn das Zeichen, das sie meldet, ein Name der Tabelle traegt. Die drei uebrigen Stellen (`TASTEN`-Kopf und die Zifferzeile in derselben Datei, `Zuweisung::OhneNamen` in `krk-ui/src/belegungsmodell.rs`, der Zuweisungszweig in `krk-ui/src/appkit/belegungsansicht.rs`) verweisen darauf. Punkt 2 des Befunds — ob der halbe Anschluss gewollt ist — hat der Nutzer am 260907-0823 beantwortet: es bleibt dabei. Einzelheiten in `260907-0858-drei-kleine-nutzerantworten-in-code.md`.
