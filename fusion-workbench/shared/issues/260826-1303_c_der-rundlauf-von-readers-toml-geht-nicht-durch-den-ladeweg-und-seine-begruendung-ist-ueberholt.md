...
---
Resolved: `alle_toml_dateien_ueberstehen_schreiben_und_wiedereinlesen`
(`crates/krk-core/tests/ablage.rs`) holt `readers.toml` jetzt ueber `geladene_leseprofile`
zurueck, haelt `!ist_ersetzt()` und vergleicht die Profilnamen gegen `["eigener Ordner"]` —
derselbe Weg, den die vier anderen TOML-Dateien gehen. Der Wortlautvergleich bleibt daneben
stehen und traegt jetzt die Meldung, die er meint ("der Ladeweg hat die von Hand gepflegte
readers.toml umgeschrieben"). Der ueberholte Doc-Absatz ist durch einen ersetzt, der die
Vorgeschichte nennt und auf `toml_dateien` verweist, wo derselben Datei schon seit Schritt 8
der Runde 16 der Ladeweg zugesagt ist.
