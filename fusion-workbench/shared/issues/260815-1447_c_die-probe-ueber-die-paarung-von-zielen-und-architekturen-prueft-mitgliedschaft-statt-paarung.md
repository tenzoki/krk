...
---
Resolved: `jedes_ziel_tripel_bekommt_einen_namen_aus_den_architekturen`
(`xtask/src/release.rs`) fragt nach der Stelle statt nach der Mitgliedschaft, in der im
Datensatz vorgeschlagenen Form: `ZIELE.into_iter().zip(ARCHITEKTUREN)` und
`assert_eq!(lipo_name(praefix), erwartet)`. Damit fängt sie jedes Vertauschen an jeder
Stelle, auch bei zehn Zielen, und nicht mehr nur das, was `const _: () = assert!(len == len)`
ohnehin schon fängt. `die_beiden_ziele_tragen_die_namen_die_lipo_dafuer_meldet` steht
unverändert daneben: sie hält die zwei Namen als Tatsache über `lipo` fest, diese hier die
Paarung. Der Doc-Kommentar der Probe schreibt die alte Lücke und den Grund für die neue Form
aus.
