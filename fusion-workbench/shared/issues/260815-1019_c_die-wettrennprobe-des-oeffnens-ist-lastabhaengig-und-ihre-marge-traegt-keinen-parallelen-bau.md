...
---
Resolved: Alle drei Punkte aus „Was zu tun ist", der dritte dadurch, dass er wegfaellt.

1. Der Ausfallzweig nennt jetzt den erreichten Durchlauf und die Zahl der Tausche.
2. Die Meldung behauptet keine Ursache mehr: sie sagt, dass der Lesefaden eine ganze Frist
   lang keinen Schritt gemacht hat, und schreibt aus, dass eine Maschine, die in dieser
   Zeit gar nichts rechnet, genauso aussaehe.
3. **Ueber die Marge ist nicht zu entscheiden, denn es gibt keine mehr.** Das
   `recv_timeout(15 s)` ueber den ganzen Lauf ist durch einen Stillstandszaehler ersetzt
   (`crates/krk-core/tests/text.rs`, `STILLSTANDSFRIST`): gemessen wird, ob **ein einzelner**
   Aufruf haengt, und nicht, wie lange 20.000 Aufrufe zusammen brauchen. Eine langsame
   Maschine verlaengert den Lauf und macht ihn nicht rot. Damit sind die vier Moeglichkeiten
   dieses Punktes — hoehere Schranke, weniger Durchlaeufe, `#[ignore]`, an der gemessenen
   Dauer bemessen — alle gegenstandslos: jede haette die Wanduhr behalten.

Der Satz „`make check` ist auf diesem Geraet nicht verlaesslich gruen, sobald ein zweiter
Bau daneben laeuft" gilt fuer diese Probe nicht mehr. Gemessen am 260908: allein 2,4 s,
unter 24 Lastschleifen auf 16 Kernen 10,5 s, beide gruen. Gemeinsam behoben mit
`260816-0055_*` und `260823-1436_*`.
