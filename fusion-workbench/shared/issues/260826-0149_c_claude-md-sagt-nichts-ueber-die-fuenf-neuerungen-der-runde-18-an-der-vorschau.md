...
---
Resolved: `CLAUDE.md`, Abschnitt „Was man nicht sieht, wenn man es nicht weiß", zwei neue Absätze hinter dem ersten. Der erste trägt den Handgriff mit der `readers.toml` als zweite Regel derselben Bauart wie die Betriebsregel gegen den Datenverlust: KRK legt die Datei beim ersten Start aus `resources/default-readers.toml` an und schreibt sie danach nie wieder, der Handgriff ist Beiseitelegen statt Löschen, und der Weg im Einzelnen steht in `README.md` unter „Neue Leseprofile übernehmen". **Keine Profilzahl in Prosa**, wie der Datensatz verlangt: gezählt wird mit `grep -c '^\[\[' resources/default-readers.toml` (am Baum `8779a25`: zwölf). Der zweite Absatz trägt die vier Änderungen am Mechanismus — die Vorschau beschreibt ohne ausgewählte Zeile den angezeigten Ordner (`zu_beschreiben`, `nach_lesebeginn` in `krk-ui/src/appkit/tabelle.rs`), ein Ort wird je Zusammenfassung höchstens einmal gelesen samt dem Hinweis, dass das eine Festlegung der Runde 16 umkehrt, der Platzhalter in der Ortsangabe mit der Abweichung zwischen Leseläufen und Verzeichnisöffnungen (Zeiger auf den Modulkopf von `krk-core/src/leseprofil/bausteine.rs`, Abschnitt „Ein Platzhalterlauf ist ein Ort und kostet einen Leselauf", und auf die Messung `shared/analyses/260825-2107-was-die-zwoelf-leseprofile-an-der-wirklichen-werkbank-kosten.md`), und der Schlüssel `zeigt` an `juengste` mit `titel` und `datum`. Die Tabellenzeile 18 stand schon seit `fb50fcd`.
