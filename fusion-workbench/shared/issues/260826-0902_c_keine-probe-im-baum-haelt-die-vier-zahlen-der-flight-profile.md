...
---
Resolved: Eine Schwesterprobe steht daneben, wie dieser Datensatz sie beschreibt:
`die_zwei_flight_profile_bleiben_unter_ihren_zahlen` (`crates/krk-core/tests/leseprofil.rs`),
mit den Prüfordner-Helfern `flightbestand`, `flightwurzel` und `flightprojektwurzel`.

Gehalten wird je Profil **beides**, wie `96e32cb` es für das fusion-Projektwurzelprofil
vormacht: die Zahl selbst — `(5, 3)` an der Werkbankwurzel, `(6, 3)` an der Projektwurzel —
und ihre Herleitung, `leselaeufe == orte.len()` dort und `leselaeufe == orte.len() + 1` hier.
Daneben hält sie, dass wirklich das genannte Profil gegriffen hat (Beschriftungen gegen
`profil.zeilen()`), dass keine Zeile auf `Wert::Nicht` fällt, und C6.4.

**Der Ordner in `archive` ist da**, und die Probe belegt an ihm, was der Datensatz verlangt:
die Zeile „Ablagen, zuletzt" liefert mit `zeigt = "datum"` ein Datum. Der Prüfordner trägt
außerdem `stilwerk`, dem das Profil bewusst keine Zeile gibt — damit er die Gestalt einer
wirklichen Werkbank hat und nicht die des Profils.

Die vier Zahlen sind damit unabhängig nachgemessen und stimmen: die Probe war beim ersten
Lauf grün. Wer den vier flight-Speichern einen fünften hinzufügt, lässt die
Gleichheitszusicherung rot werden und muss den Kommentar mitziehen — genau die Vorkehrung,
