...
---
Resolved: Weg 2. `das_modell_verwirft_stapel_einer_alten_generation`
(`crates/krk-core/tests/verzeichnis.rs`) schickt die Stapel der Generation 3 jetzt durch
dieselbe Abfrage, die die Oberflaeche stellt (`gehoert_dazu` vor `anhaengen`), und haelt
danach `zeilenzahl()` des Modells der Generation 4 auf 0. Daneben steht ein zweites Modell
der Generation 3, das dieselben Stapel aufnimmt und auf 10 gehalten wird — ohne das waere
die Probe auch dann gruen, wenn der Leser nichts geliefert haette. Das `filter`-Praedikat
mit der Schleifeninvariante ist weg; der Doc-Kommentar nennt die alte Bauart und warum sie
nichts gemessen hat. Ein `anhaengen`, das den fremden Stapel aufnaehme, macht die Probe
jetzt rot.
