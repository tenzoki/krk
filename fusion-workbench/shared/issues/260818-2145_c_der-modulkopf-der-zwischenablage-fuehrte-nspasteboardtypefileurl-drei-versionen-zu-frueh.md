...
---
Resolved: **Die Ursache war bereits gefallen, am Code nichts geaendert.** Nachgesehen am heutigen Baum: `crates/krk-ui/src/appkit/zwischenablage.rs` fuehrt „**`NSPasteboardTypeFileURL` steht seit 10.13** (`NSPasteboard.h:39`) und nicht seit 10.6, wie diese Stelle bis zur Runde 13 sagte", und `NSPasteboardURLReadingFileURLsOnlyKey` steht richtig unter den Symbolen seit 10.6. Beide Abgleiche vom 260819 hatten das schon festgestellt und den Marker nur wegen der zweiten Haelfte auf `_o_` gelassen. **Diese zweite Haelfte ist keine Stoerung, sondern eine Frage, und sie hat ihren eigenen Datensatz:** wie eine **falsche** Untergrenzenangabe kuenftig auffaellt, entscheidet `decisions/260811-2050_*_wird-die-untergrenzen-angabe-pruefbar-gemacht.md` (offen, drei Stufen mit Kosten). Geschlossen auf dem Weg, den der Abgleich vom 260819-0057 unter „Anmerkung zur Form" selbst als den sauberen benennt: `_c_` mit Verweis auf jenen Entscheid, statt einer Mischform aus `Resolved:` und `_o_`, die jede Erhebung ueber `Resolved:`-Zeilen falsch zaehlt.
