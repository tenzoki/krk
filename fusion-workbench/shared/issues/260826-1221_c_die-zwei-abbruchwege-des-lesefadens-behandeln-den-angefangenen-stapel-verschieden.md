...
---
Resolved: „an beiden nicht", mit einer Zeile Begruendung an der ersten Stelle und einem
Verweis darauf an der zweiten (`crates/krk-core/src/verzeichnis/leser.rs`,
`lesen_und_senden`).

**Die Wahl ist gemessen und nicht abgewogen.** Der erste Versuch ging die andere Richtung —
`rest_senden` auch an der zweiten Stelle — und machte
`abgebrochener_leser_liefert_teilbestand_und_meldet_den_abbruch`
(`crates/krk-core/tests/verzeichnis.rs`) rot: „der Abbruch kam zu spaet: 5000 von 5.000
Eintraegen gelesen". Der Grund ist die Stellung der zweiten Pruefung. Sie steht **innerhalb**
der Stapelschleife, wo `gesammelt` alles traegt, was ein einziger `getdirentries` geliefert
hat; ein `rest_senden` dort schickt bei einem Ordner mit 5.000 Eintraegen alle 5.000 los, und
der Abbruch hoert auf, einer zu sein. Der Kommentar unmittelbar darueber sagt dasselbe von
der anderen Seite: die Pruefung steht dort, damit ein abgebrochener Ordner nicht doch
vollstaendig durchlaeuft.

Der Datensatz nennt fuer „an beiden senden", dass der Rest bereits gelesen sei und das
Senden nichts koste als einen Kanalplatz. Das trifft fuer die erste Stelle zu und fuer die
zweite nicht, und weil die zwei Stellen dieselbe Antwort bekommen sollten, gibt die zweite
den Ausschlag. `Abschluss::Abgebrochen` traegt beide Ausgaenge; ein Senden weniger ist
daneben ein Kanalplatz weniger, an dem der Abbruch haengen kann.

`cargo test -p krk-core --test verzeichnis`: 100 passed, 0 failed.
