...
---
Resolved: Geschlossen ohne Aenderung: am Baum steht nichts mehr aus. Nachgelesen am 260908 an HEAD `fe5fe9c` samt den Aenderungen dieses Durchgangs. Der Modulkopf von `crates/krk-core/src/verzeichnis/verweisziel.rs` traegt das Wort `wortlos` nicht mehr und nennt stattdessen den Weg ueber den Leser samt der Zeitzusagen L3 und L10, die unberuehrt bleiben. `crates/krk-ui/src/kommandos/pfadeingabe.rs` und `crates/krk-ui/src/appkit/tabelle.rs` tragen je einen Absatz, der `tabs::lesemeldungen_einziehen` als Herkunft der Meldung nennt. Der Entscheidungsdatensatz `260815-1749_*` steht auf `_i_` und sein `Implemented:`-Nachsatz schreibt den Irrtum der Praemisse aus. Die Probe `tabs::tests::ein_ordner_ohne_leserecht_meldet_sich_aus_dem_lesevorgang` steht im Baum. Was bleibt, ist keine Aufgabe dieses Datensatzes, sondern die allgemeine Frage `260906-0203_*_darf-ein-agent-den-spec-oder-plan-einer-geschlossenen-runde-berichtigen.md`, die er selbst als offene Nachbarschaft nennt.
