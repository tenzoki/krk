...
---
Resolved: Der Satz „hier vertretbar und sonst nirgends" steht nicht mehr an `verwalter_ohne_fenster`. An seiner Stelle stehen die drei Stellen des Teilbaums namentlich, die Erhebung, die sie zaehlt (`grep -rn new_unchecked crates/krk-ui/src`), und die Auskunft, dass ausserhalb der Pruefmodule keine steht. Die Frage nach der Sperre bleibt offen und ist dort als offen benannt; sie haengt an `260810-1044_*_ziehen-die-vier-instanzproben-in-ein-pruefziel-ohne-libtest-harness-um.md` (zurueckgestellt). CLAUDE.md ist nicht angefasst: seine Aussage ist auf „die, die eine `NSTextView` bauen" verengt und trifft damit zu.
