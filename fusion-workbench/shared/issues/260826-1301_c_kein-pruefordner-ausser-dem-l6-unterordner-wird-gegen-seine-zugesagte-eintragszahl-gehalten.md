...
Reconciled: 260826-2205 — gegen den Baum `bc5991d` geprueft und zutreffend, mit einer Einschraenkung, die schon einen eigenen Datensatz traegt: der Commit ist `960900d`, `pruefordner_pruefen` steht an `crates/krk-bench/src/messen.rs:1587` und wird fuer A, B und 100k gerufen (`:1063-1077`), die gelesene Zahl erreicht den Bericht (`:1266-1268`, `crates/krk-bench/src/bericht.rs:259` und `:266`). Die Zeile sagt "jeder Pruefordner"; gegen die **gelesene** Zahl gehalten werden nur A und der grosse Ordner, B und der L6-Unterordner allein gegen ihren Steckbrief, und `Durchstich::fahren` (`messen.rs:763`) prueft weiter nichts (`shared/issues/260826-2154_*_der-durchstich-prueft-seine-pruefordner-ueberhaupt-nicht-und-verspricht-in-prosa-weiter-zehntausend.md`, `shared/issues/260826-2155_*_pruefordner-b-und-der-l6-unterordner-werden-nur-gegen-ihren-steckbrief-gehalten-und-der-kommentar-sagt-b-werde-nicht-gelesen.md`). `make check` ueber `bc5991d` gruen.
