...
---
Resolved: `vorgang_zeichnen` laesst Konfliktfrage und Abschlussbericht im `Vorgangszustand` liegen, solange `blatt_steht()` wahr ist — der Weg aus dem Abschnitt „Denkbarer Weg". Abgeholt werden sie beim naechsten Durchgang, und der kommt: `blatt_geschlossen` weckt den Hauptfaden ueber `hauptfaden_wecken`, also ueber denselben Weg, den der Vermittlerfaden nimmt, und einen Durchgang spaeter — der Abschlussblock laeuft, waehrend AppKit das Blatt noch abbaut, ein unmittelbarer Ruf traefe deshalb denselben Vorbehalt noch einmal. `offenes_blatt` hat seither genau einen Schreib- und einen Loeschpunkt (`blatt_oeffnet`, `blatt_geschlossen`); alle sechs Schliessstellen gehen durch den zweiten. Der Preis: der Arbeitsfaden wartet auf die Konfliktantwort, solange der Notizzettel steht — derselbe Preis wie bei einem Nutzer, der die Frage stehen laesst.
