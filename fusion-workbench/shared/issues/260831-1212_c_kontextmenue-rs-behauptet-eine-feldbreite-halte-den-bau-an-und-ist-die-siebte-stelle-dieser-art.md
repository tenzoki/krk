...
---
Resolved: Der Doc-Kommentar an `Kontextbefehl::ALLE` (`crates/krk-ui/src/kommandos/kontextmenue.rs`) sagt jetzt das Gegenteil seiner alten Behauptung: die Feldbreite haelt den Bau **nicht** an, `[Kontextbefehl; 3]` zwingt zu drei Gliedern und sagt nichts darueber, welche drei. Der Halbsatz „wie es die Aufzaehlungen dieses Baums durchweg tun" ist gefallen, beide Belegdatensaetze sind zitiert. **Was den Bau wirklich anhaelt, ist am Baum nachgesehen und namentlich genannt:** die drei vollstaendigen Fallunterscheidungen ueber den Wert, `Kontextbefehl::titel`, `::menuemarke` und der Ausfuehrungszweig beim Anwendungsdelegierten (`crates/krk-ui/src/appkit/anwendung.rs`, `kontextbefehl_ausfuehren`), keine davon mit Auffangzweig. **Der Weg, den der Datensatz vorschlaegt, traegt dabei nicht ganz:** die Probe `die_tafel_nennt_jeden_befehl_genau_einmal` laeuft ueber `ALLE` und haelt die Tafel dagegen, nicht `ALLE` gegen die Aufzaehlung; dass ein vierter Wert auch in dieser Liste landet, erzwingt am Baum nichts. Der Kommentar sagt das jetzt und verweist fuer die Frage nach der Bauform auf `260826-1811_*`, statt eine Sicherung zu behaupten, die es nicht gibt.
