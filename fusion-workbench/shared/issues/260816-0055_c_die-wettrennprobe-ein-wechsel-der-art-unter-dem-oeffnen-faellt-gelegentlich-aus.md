...
---
Resolved: Der dritte Weg, den „Was zu entscheiden ist" nennt — „den Ausfall an der Frage
aufhaengen, die die Probe wirklich stellt, ob **ein einzelner** Aufruf haengt, statt an der
Gesamtdauer". Die Notbremse `recv_timeout(15 s)` ueber den ganzen Lauf ist weg; an ihrer
Stelle steht ein Stillstandszaehler, den der Lesefaden vor jedem `datei::oeffnen`
hochsetzt, und der Elternteil urteilt allein, wenn er sich eine ganze Frist lang nicht
bewegt hat (`crates/krk-core/tests/text.rs`, `STILLSTANDSFRIST`).

Damit ist die Abwaegung dieses Datensatzes gegenstandslos statt beantwortet: es gibt keine
Frist mehr, die anzuheben waere, und die Zahl der Durchlaeufe bleibt bei 20.000, muss also
nicht gesenkt werden. Ein Lauf ohne Defekt endet immer, denn `HOECHSTENS_DURCHLAEUFE`
begrenzt den Lesefaden; die einzige Lage, in der er nicht endet, ist die gesuchte.
Gemeinsam behoben mit `260815-1019_*` und `260823-1436_*`, die dieselbe Probe halten.
