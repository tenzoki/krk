// KRK — Nachmessung zu `rueckgaengig.swift` daneben: trägt die Form aus
// Schritt 4.2 (`spikes/mehrzeilige-zelle/zelle.swift`, Durchgang
// `feldeditorohnenote`) auch mit einem eigenen Feldeditor aus
// `windowWillReturnFieldEditor:toObject:`? Eine Kopie jenes Programms; geändert
// sind allein der Feldeditor und der Name der Belegdatei.
//
// Ursprünglich: KRK — Prüfprogramm zu Schritt 4.2 des Plans
...
final class Zelleneditor: NSTextView {
    let eigenerVerwalter = UndoManager()
    override var undoManager: UndoManager? { eigenerVerwalter }
    @objc func undo(_ sender: Any?) { eigenerVerwalter.undo() }
    @objc func redo(_ sender: Any?) { eigenerVerwalter.redo() }
}
let zelleneditor: Zelleneditor = {
    let e = Zelleneditor()
    ...
    NSTextFieldDelegate, NSWindowDelegate
{
    func windowWillReturnFieldEditor(_ sender: NSWindow, to client: Any?) -> Any? {
        guard let feld = client as? NSView, feld.isDescendant(of: tabelle) else { return nil }
        return zelleneditor
    }
...
func lage(_ marke: String) -> CGFloat {
    ...
    let feldeditorIstErsthelfer = editor != nil && fenster.firstResponder === editor && editor === zelleneditor
...
func abschliessen() {
    ...
    let pfad = URL(fileURLWithPath: CommandLine.arguments[0])
        ...
        .appendingPathComponent("messung-mehrzeilig-\(durchgang.rawValue).txt")
