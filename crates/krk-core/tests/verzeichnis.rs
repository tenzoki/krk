...
fn heimmodell(ohne: Option<&'static str>, ausblenden: bool) -> Ordnermodell {
    ...
    modell.anhaengen([
        ...
        handeintrag("notes.txt", Typ::Datei),
    ]);
    modell.abschliessen();
    modell
}
...
fn die_geheimnisse_bekommen_im_erkannten_ordner_keinen_inhaltsauftrag() {
    for ausblenden in [true, false] {
        let mut modell = heimmodell(Some(GEHEIMNISSE), ausblenden);
        modell.inhalt_setzen(true);
        ...
        let geheim = index_von(&modell, GEHEIMNISSE);
        ...
        assert!(
            auftraege
                .iter()
                .any(|auftrag| auftrag.index == notizen && auftrag.art == Auftragsart::Inhalt),
            "ausblenden={ausblenden}: die gewoehnliche Datei hat keinen: {auftraege:?}"
        );
        assert!(
            !namen(&modell).contains(&GEHEIMNISSE),
            "ausblenden={ausblenden}: ihr Name traegt den Filter nicht, also faellt sie"
        );
        // Ein Befund, der sie dennoch erreichte, stellte sie nicht hin: sie
        // steht unter keinem Vorbehalt.
        modell.befunde_setzen([(geheim, Befund::Treffer)]);
        assert!(!namen(&modell).contains(&GEHEIMNISSE));

        // Traegt ihr Name den Filter, steht sie.
        modell.filtertext_setzen("secret");
        assert!(
            namen(&modell).contains(&GEHEIMNISSE),
            "ausblenden={ausblenden}: der Name traegt den Filter"
        );
    }
    ...
    assert!(
        anderswo
            .auftraege()
            .iter()
            .any(|auftrag| auftrag.index == geheim),
        "ausserhalb des erkannten Ordners liest der Inhaltsfilter sie wie jede Datei"
    );
}
